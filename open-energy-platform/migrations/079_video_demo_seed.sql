...
--   • lender launch board shows "0 facilities / 0 covenants" → seed 6 covenants
...
--
-- One INSERT per row (not UNION ALL chains) — D1's compound SELECT term cap
-- rejects the chained form.
--
-- Safe to re-run (all INSERT OR IGNORE / UPDATE WHERE).
-- ═══════════════════════════════════════════════════════════════════════════

-- ─── 1. Rename admin shell name ───────────────────────────────────────────────
UPDATE participants
...
-- ─── 2. Trader orders — 6 open + 3 partial + 1 closed ─────────────────────────
...
INSERT OR IGNORE INTO trade_orders
  (id, participant_id, side, energy_type, volume_mwh, price_min, price_max,
   delivery_date, delivery_point, market_type, status, created_at)
SELECT 'tord-vid-002', id, 'buy',  'wind',   200, 1100, 1170, date('now','+1 day'), 'EC-Coastal', 'exchange', 'open', datetime('now','-2 hours')
  FROM participants WHERE email='trader@openenergy.co.za';
INSERT OR IGNORE INTO trade_orders
  (id, participant_id, side, energy_type, volume_mwh, price_min, price_max,
   delivery_date, delivery_point, market_type, status, created_at)
SELECT 'tord-vid-003', id, 'buy',  'hybrid', 120, 1240, 1290, date('now','+2 day'), 'NC-Upington','exchange', 'open', datetime('now','-90 minutes')
  FROM participants WHERE email='trader@openenergy.co.za';
INSERT OR IGNORE INTO trade_orders
  (id, participant_id, side, energy_type, volume_mwh, price_min, price_max,
   delivery_date, delivery_point, market_type, status, created_at)
SELECT 'tord-vid-004', id, 'sell', 'solar',  180, 1210, 1260, date('now','+1 day'), 'NC-Upington','exchange', 'open', datetime('now','-75 minutes')
  FROM participants WHERE email='trader@openenergy.co.za';
INSERT OR IGNORE INTO trade_orders
  (id, participant_id, side, energy_type, volume_mwh, price_min, price_max,
   delivery_date, delivery_point, market_type, status, created_at)
SELECT 'tord-vid-005', id, 'sell', 'wind',   220, 1130, 1190, date('now','+1 day'), 'EC-Coastal', 'exchange', 'open', datetime('now','-60 minutes')
  FROM participants WHERE email='trader@openenergy.co.za';
INSERT OR IGNORE INTO trade_orders
  (id, participant_id, side, energy_type, volume_mwh, price_min, price_max,
   delivery_date, delivery_point, market_type, status, created_at)
SELECT 'tord-vid-006', id, 'sell', 'hybrid', 100, 1260, 1310, date('now','+2 day'), 'WC-Inland',  'exchange', 'open', datetime('now','-45 minutes')
  FROM participants WHERE email='trader@openenergy.co.za';
INSERT OR IGNORE INTO trade_orders
  (id, participant_id, side, energy_type, volume_mwh, price_min, price_max,
   delivery_date, delivery_point, market_type, status, created_at)
...
INSERT OR IGNORE INTO trade_orders
  (id, participant_id, side, energy_type, volume_mwh, price_min, price_max,
   delivery_date, delivery_point, market_type, status, created_at)
SELECT 'tord-vid-008', id, 'sell', 'wind',  90,  1140, 1180, date('now'),           'EC-Coastal', 'exchange', 'partial', datetime('now','-3 hours')
  FROM participants WHERE email='trader@openenergy.co.za';
INSERT OR IGNORE INTO trade_orders
  (id, participant_id, side, energy_type, volume_mwh, price_min, price_max,
   delivery_date, delivery_point, market_type, status, created_at)
SELECT 'tord-vid-009', id, 'buy',  'hybrid',60,  1250, 1290, date('now'),           'KZN-South',  'exchange', 'partial', datetime('now','-2 hours')
  FROM participants WHERE email='trader@openenergy.co.za';
INSERT OR IGNORE INTO trade_orders
  (id, participant_id, side, energy_type, volume_mwh, price_min, price_max,
   delivery_date, delivery_point, market_type, status, created_at)
SELECT 'tord-vid-010', id, 'buy',  'solar', 100, 1200, 1240, date('now','-1 day'),  'NC-Upington','exchange', 'closed',  datetime('now','-1 day')
  FROM participants WHERE email='trader@openenergy.co.za';

INSERT OR IGNORE INTO trade_matches (id, buy_order_id, sell_order_id, matched_volume_mwh, matched_price, status, matched_at)
VALUES ('tmatch-vid-001', 'tord-vid-007', 'tord-vid-005', 40,  1210, 'settling', datetime('now','-3 hours'));
INSERT OR IGNORE INTO trade_matches (id, buy_order_id, sell_order_id, matched_volume_mwh, matched_price, status, matched_at)
VALUES ('tmatch-vid-002', 'tord-vid-009', 'tord-vid-006', 30,  1280, 'settling', datetime('now','-90 minutes'));
INSERT OR IGNORE INTO trade_matches (id, buy_order_id, sell_order_id, matched_volume_mwh, matched_price, status, matched_at)
VALUES ('tmatch-vid-003', 'tord-vid-010', 'tord-vid-004', 100, 1220, 'settled',  datetime('now','-23 hours'));

-- ─── 3. Offtaker PPAs — 4 active ──────────────────────────────────────────────
...
INSERT OR IGNORE INTO off_ppa_portfolio
  (id, participant_id, tenant_id, contract_ref, counterparty_name, technology,
   capacity_mw, ppa_term_years, ppa_start_date, ppa_end_date, price_zar_per_mwh,
   indexation, expected_p50_gwh_yr, green_attributes, status)
SELECT 'oppa-vid-002', id, 'default', 'CECL-PPA-02',
...
INSERT OR IGNORE INTO off_ppa_portfolio
  (id, participant_id, tenant_id, contract_ref, counterparty_name, technology,
   capacity_mw, ppa_term_years, ppa_start_date, ppa_end_date, price_zar_per_mwh,
   indexation, expected_p50_gwh_yr, green_attributes, status)
SELECT 'oppa-vid-003', id, 'default', 'CECL-PPA-03',
...
INSERT OR IGNORE INTO off_ppa_portfolio
  (id, participant_id, tenant_id, contract_ref, counterparty_name, technology,
   capacity_mw, ppa_term_years, ppa_start_date, ppa_end_date, price_zar_per_mwh,
   indexation, expected_p50_gwh_yr, green_attributes, status)
SELECT 'oppa-vid-004', id, 'default', 'CECL-PPA-04',
...
INSERT OR IGNORE INTO regulator_surveillance_rules (id, rule_code, rule_name, description, rule_type, severity, enabled)
VALUES ('rsr-vid-02', 'MARKING_CLOSE', 'Marking the close',
...
INSERT OR IGNORE INTO regulator_surveillance_alerts
  (id, rule_id, rule_code, participant_id, entity_type, entity_id, severity, status, raised_at, details_json)
SELECT 'rsa-vid-02', 'rsr-vid-02', 'MARKING_CLOSE', id, 'trade_orders', 'tord-vid-003', 'medium', 'open', datetime('now','-2 hours'), '{"share_of_volume":0.42}'
  FROM participants WHERE email='trader@openenergy.co.za';
INSERT OR IGNORE INTO regulator_surveillance_alerts
  (id, rule_id, rule_code, participant_id, entity_type, entity_id, severity, status, raised_at, details_json)
SELECT 'rsa-vid-03', 'rsr-vid-01', 'WASH_TRADE', id, 'trade_matches', 'tmatch-vid-002', 'high', 'resolved', datetime('now','-1 day'), '{"matches":1,"window_min":12}'
  FROM participants WHERE email='trader@openenergy.co.za';
INSERT OR IGNORE INTO regulator_surveillance_alerts
  (id, rule_id, rule_code, participant_id, entity_type, entity_id, severity, status, raised_at, details_json)
SELECT 'rsa-vid-04', 'rsr-vid-02', 'MARKING_CLOSE', id, 'trade_orders', 'tord-vid-004', 'low', 'false_positive', datetime('now','-2 day'), '{"share_of_volume":0.18}'
  FROM participants WHERE email='trader@openenergy.co.za';
...
INSERT OR IGNORE INTO regulator_surveillance_triage
  (id, alert_id, triaged_by, triaged_at, decision, rationale, next_review_at)
SELECT 'rst-vid-02', 'rsa-vid-04', id, datetime('now','-1 day','-6 hours'), 'false_positive',
       'Volume share within historic norms for low-liquidity session.', NULL
  FROM participants WHERE email='regulator@openenergy.co.za';
INSERT OR IGNORE INTO regulator_surveillance_triage
  (id, alert_id, triaged_by, triaged_at, decision, rationale, next_review_at)
SELECT 'rst-vid-03', 'rsa-vid-01', id, datetime('now','-2 hours'), 'monitor',
       'Two matches in 8-min window — escalate if a third lands today.',
       datetime('now','+8 hours')
  FROM participants WHERE email='regulator@openenergy.co.za';
INSERT OR IGNORE INTO regulator_surveillance_triage
  (id, alert_id, triaged_by, triaged_at, decision, rationale, next_review_at)
SELECT 'rst-vid-04', 'rsa-vid-02', id, datetime('now','-90 minutes'), 'contact_party',
