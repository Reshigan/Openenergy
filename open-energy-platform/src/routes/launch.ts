  ...
  return {
    ...
    workflows: [
      ...
      },
      {
        ...
        description: 'JBCC SA standard construction contract + NEC4 payment assessment + REIPPPP payment milestones + Equator Principles EP4 disbursement certification on oe_ipp_progress_claims: 12-state P6 chain (submitted → quantity_survey_review → pm_review → engineer_certified → approved → payment_processed → closed + disputed / suspended / rejected / partial_payment / final_account branches). INVERTED SLA polarity (major >R10m = 720h — most time for thorough IE/lender review; minor <R100k = 72h — least). W141 SIGNATURE: certify_by_engineer EVERY tier when floor_ie_milestone_payment (IE milestone = lender notification always mandatory); record_final_account EVERY tier (final settlement always notified to lenders); approve_payment when floor_lender_certification_required. SLA breach crosses when major/significant + floor_ie_milestone_payment. 4 claim tiers: major (>R10m) / significant (R1m–R10m) / standard (R100k–R1m) / minor (<R100k). 5 claim types: interim / milestone / final / variation / daywork. 5 floor flags: IE milestone payment / lender certification required / retention release / variation included / defects outstanding. 9 financial fields: claim / QS assessed / certified / approved / retention / VAT / net payable / previous certified total / this period. SLA sweep in shared */15 cron slot. JOINS existing ipp audit namespace. Beats Oracle Aconex (payment as document workflow — no P6 lifecycle, no QS/PM/IE gates, no lender notification crossing).',
        href: '/ipp-lifecycle/workstation?tab=progress-claims',
        cta_label: 'Open progress claims',
        icon: 'receipt_long',
      },
      {
        key: 'subcontractors',
