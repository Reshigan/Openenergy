...
function drawFooter(page: PDFPage, fonts: Fonts, pageNum: number, totalPages: number, refNum?: string): void {
  ...
  // Left: legal notice
  page.drawText('Confidential — Consolidated Energy Cockpit | Issued under NERSA/JSE-SRL regulatory framework', {
...
export async function buildCarbonCertPdf(data: CarbonCertData): Promise<Uint8Array> {
  ...
  page.drawText('Authorised by Consolidated Energy Cockpit', { x: MARGIN, y: y - 12, size: 7, font: regular, color: MID });
...
export async function buildAuditExportPdf(data: AuditExportData): Promise<Uint8Array> {
  ...
  y = drawMetadataGrid(page, fonts, [
    ...
    { label: 'Generated By',     value: data.generated_by ?? 'Consolidated Energy Cockpit' },
