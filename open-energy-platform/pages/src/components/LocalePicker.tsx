...
export function LocalePicker({ compact }: { compact?: boolean }) {
  ...
  return (
    <label className="inline-flex items-center gap-1 text-[11px] text-[var(--ink-2, #6b7685)]">
      {!compact && <Languages size={12}/>}
      <select
        aria-label="Language"
        value={locale}
        onChange={(e) => set(e.target.value as Locale)}
        className="h-7 px-1 rounded border border-[var(--border-subtle, #dde4ec)] text-[11px] bg-surface-v2"
      >
        {available.map((l) => (
