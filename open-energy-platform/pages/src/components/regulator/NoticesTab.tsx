...
const STATUS_TONE: Record<NoticeStatus, { bg: string; fg: string }> = {
  issued: { bg: 'color-mix(in oklab, var(--warn) 15%, var(--s1))', fg: '#a06200' },
  acknowledged: { bg: 'oklch(0.94 0.02 250)', fg: 'oklch(0.46 0.16 55)' },
  satisfied: { bg: 'color-mix(in oklab, var(--good) 15%, var(--s1))', fg: 'var(--good, #1f6b3a)' },
  overdue: { bg: 'color-mix(in oklab, var(--bad) 15%, var(--s1))', fg: 'var(--bad, #9b1f1f)' },
  escalated: { bg: 'color-mix(in oklab, var(--bad) 15%, var(--s1))', fg: 'var(--bad, #9b1f1f)' },
  withdrawn: { bg: 'var(--s2, #f0f3f7)', fg: 'var(--ink-2, #445566)' },
};

const TYPE_TONE: Record<NoticeType, { bg: string; fg: string }> = {
  information_request: { bg: 'var(--s2, #f0f3f7)', fg: 'var(--ink-2, #445566)' },
  remediation: { bg: 'color-mix(in oklab, var(--warn) 15%, var(--s1))', fg: '#a06200' },
  warning: { bg: '#ffe5cc', fg: '#a04200' },
  penalty: { bg: 'color-mix(in oklab, var(--bad) 15%, var(--s1))', fg: 'var(--bad, #9b1f1f)' },
  suspension: { bg: 'color-mix(in oklab, var(--bad) 15%, var(--s1))', fg: 'var(--bad, #9b1f1f)' },
  revocation: { bg: 'color-mix(in oklab, var(--bad) 15%, var(--s1))', fg: 'var(--bad, #9b1f1f)' },
};
...
export function NoticesTab() {
  ...
  return (
    <div data-testid="regulator-notices-tab" className="space-y-4">
      ...
      <div className="flex gap-2 flex-wrap items-center">
        {(['all', 'issued', 'acknowledged', 'overdue', 'satisfied', 'withdrawn'] as const).map((s) => (
          <button type="button"
            ...
            onClick={() => setFilter(s)}
            className={`h-7 px-3 rounded-full text-[11px] font-semibold border ${filter === s ? 'bg-[#c2873a] text-white border-[oklch(0.46_0.16_55)]' : 'bg-surface-v2 text-[var(--ink-2, #445566)] border-[var(--border-subtle, #d8dee6)]'}`}
          >
        ...
        <button type="button" onClick={load} className="h-7 px-3 rounded-full text-[11px] font-semibold border border-[var(--border-subtle, #d8dee6)] bg-surface-v2 text-[oklch(0.46_0.16_55)]">
          Refresh
        </button>
      </div>
      ...
      <div data-testid="regulator-notices-table" className="border border-[var(--border-subtle, #e5e9ee)] rounded-md overflow-hidden">
        <div className="grid grid-cols-[140px_1fr_140px_140px_130px] gap-2 px-3 py-2 bg-[var(--s1, #f7f9fb)] text-[11px] uppercase font-bold text-[var(--ink-2, #6b7685)]">
          <div>Type</div>
          <div>Title</div>
          <div>Status</div>
          <div>Deadline</div>
        ...
        {filtered.map((r) => {
          ...
          return (
            <button type="button"
              ...
              className="w-full grid grid-cols-[140px_1fr_140px_140px_130px] gap-2 px-3 py-2 border-t border-[var(--border-subtle, #e5e9ee)] text-left text-[12px] hover:bg-[var(--s1, #f7f9fb)]"
            >
              <div>
              ...
              <div className="font-mono text-[10px] text-[var(--ink-2, #6b7685)]">{r.licensee_user_id}</div>
            </button>
          );
        })}
      </div>
      ...
      {drillRow && (
        <div data-testid="regulator-notice-drill" className="border border-[oklch(0.46_0.16_55)] rounded-md p-4 bg-[var(--s1, #f7f9fb)] space-y-3">
          ...
          {(drillRow.status === 'issued' || drillRow.status === 'acknowledged' || drillRow.status === 'overdue') && (
            <div className="border-t border-[var(--border-subtle, #d8dee6)] pt-3 space-y-2" data-testid="regulator-notice-actions">
              <input
                value={evidence}
...
function CreateNoticeForm({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div data-testid="regulator-notice-form" className="border border-[oklch(0.46_0.16_55)] rounded-md p-4 bg-surface-v2 space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          <div className="text-[10px] uppercase font-bold text-[var(--ink-2, #6b7685)]">Type</div>
          <select
            value={form.notice_type}
            onChange={(e) => setForm({ ...form, notice_type: e.target.value as NoticeType })}
            className="w-full h-9 px-2 rounded border border-[var(--border-subtle, #d8dee6)] text-[12px]"
          >
        ...
        <div className="col-span-2">
          <div className="text-[10px] uppercase font-bold text-[var(--ink-2, #6b7685)]">Body</div>
          <textarea
            value={form.body}
            onChange={(e) => setForm({ ...form, body: e.target.value })}
            rows={4}
            className="w-full px-2 py-1 rounded border border-[var(--border-subtle, #d8dee6)] text-[12px]"
          />
        </div>
        <div>
          <div className="text-[10px] uppercase font-bold text-[var(--ink-2, #6b7685)]">Remedy deadline</div>
          <input
            type="datetime-local"
      ...
      {err && <div className="text-[12px] text-[var(--bad, #9b1f1f)]">{err}</div>}
      <div className="flex gap-2">
        ...
        <button type="button"
          onClick={onClose}
          className="h-8 px-3 rounded border border-[var(--border-subtle, #d8dee6)] bg-surface-v2 text-[var(--ink-2, #445566)] text-[11px] font-semibold"
        >
          Cancel
        </button>
      </div>
...
function Kpi({ label, value, tone }: { label: string; value: number | string; tone?: 'good' | 'warn' | 'bad' }) {
  const color = tone === 'bad' ? 'var(--bad, #9b1f1f)' : tone === 'warn' ? '#a06200' : tone === 'good' ? 'var(--good, #1f6b3a)' : 'oklch(0.46 0.16 55)';
  return (
