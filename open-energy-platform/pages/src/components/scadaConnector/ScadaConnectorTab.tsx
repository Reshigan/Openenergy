...
const STATE_TONE: Record<SccStatus, { bg: string; fg: string; label: string }> = {
  ...
  endpoints_discovered:         { bg: 'color-mix(in oklab, var(--warn) 18%, var(--s1))', fg: 'var(--warn)', label: 'Endpoints discovered' },
  tls_configured:               { bg: 'color-mix(in oklab, var(--warn) 18%, var(--s1))', fg: 'var(--warn)', label: 'TLS configured' },
  handshake_completed:          { bg: 'color-mix(in oklab, var(--warn) 18%, var(--s1))', fg: 'var(--warn)', label: 'Handshake' },
  telemetry_streaming:          { bg: 'color-mix(in oklab, var(--warn) 15%, var(--s1))', fg: 'var(--warn)', label: 'Streaming' },
  quality_validated:            { bg: 'color-mix(in oklab, var(--warn) 15%, var(--s1))', fg: 'var(--warn)', label: 'Quality validated' },
  alarms_subscribed:            { bg: 'color-mix(in oklab, var(--warn) 15%, var(--s1))', fg: 'var(--warn)', label: 'Alarms subscribed' },
  control_commands_authorized:  { bg: 'color-mix(in oklab, var(--warn) 15%, var(--s1))', fg: 'var(--warn)', label: 'Control authorized' },
  live_operations:              { bg: 'color-mix(in oklab, var(--good) 15%, var(--s1))', fg: 'var(--good, #1f6b3a)', label: 'Live operations' },
  ...
  revoked:                      { bg: 'var(--bad, #7a0e0e)', fg: '#fff',    label: 'Revoked' },
  suspended:                    { bg: 'var(--s2, #eef1f5)', fg: 'var(--ink-2)',    label: 'Suspended' },
  failover_active:              { bg: 'color-mix(in oklab, var(--warn) 15%, var(--s1))', fg: 'var(--warn)', label: 'Failover active' },
};

const TIER_TONE: Record<SccTier, { bg: string; fg: string; label: string }> = {
  pilot:                    { bg: 'var(--s2, #eef1f5)', fg: 'var(--ink-2)',    label: 'Pilot' },
  small_substation:         { bg: 'color-mix(in oklab, var(--warn) 18%, var(--s1))', fg: 'var(--warn)', label: 'Small substation' },
  medium_substation:        { bg: 'color-mix(in oklab, var(--good) 15%, var(--s1))', fg: 'var(--good, #1f6b3a)', label: 'Medium substation' },
  large_substation:         { bg: 'color-mix(in oklab, var(--warn) 15%, var(--s1))', fg: 'var(--warn)', label: 'Large substation' },
  national_grid_backbone:   { bg: 'var(--bad, #7a0e0e)', fg: '#fff',    label: 'National grid backbone' },
};
  ...
  return (
    <div className="text-[12px]" style={{ color: 'oklch(0.46 0.16 55)' }}>
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div>
          <h2 className="text-base font-semibold text-[var(--ink, #0c2a4d)]">SCADA / IEC 61850 connector</h2>
        ...
        <div className="overflow-hidden rounded border border-[var(--border-subtle, #d8dde6)] bg-surface-v2">
          <table className="w-full text-[12px]">
            ...
            <tbody>
                ...
                return (
                  ...
                  >
                    <td className="px-3 py-2 font-mono text-[var(--ink, #0c2a4d)]">
                      ...
                      {r.floor_at_national_grid_backbone_live ? <span className="ml-1 text-[9px] font-semibold text-[var(--bad, #7a0e0e)]">NAT</span> : null}
                    </td>
                    <td className="px-3 py-2 text-[11px]" style={{ color: 'oklch(0.46 0.16 55)' }}>
                      {r.substation_name ?? '-'}
                      {r.substation_capacity_mva != null ? <div className="text-[10px] text-[var(--ink-2, #6b7685)] font-mono">{r.substation_capacity_mva} MVA</div> : null}
                    </td>
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={10} className="px-3 py-6 text-center text-[var(--ink-2, #4a5568)]">No connectors match.</td></tr>
              )}
            </tbody>
          </table>
        </div>
    ...
    <div onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }} className="fixed inset-0 z-50 flex items-stretch justify-end bg-black/40">
      <div className="w-full max-w-3xl overflow-y-auto bg-[var(--s2, #f3f5f9)] p-4">
        ...
        <div className="mb-3 grid grid-cols-4 gap-2 rounded border border-[var(--border-subtle, #d8dde6)] bg-surface-v2 p-3 text-[11px]">
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">Logical nodes</div>
            <div className="font-mono text-[12px] text-[var(--ink, #0c2a4d)]">{row.logical_node_count ?? '-'}</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">Data objects</div>
            <div className="font-mono text-[12px] text-[var(--ink, #0c2a4d)]">{row.data_object_count ?? '-'}</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">Msgs/min</div>
            <div className="font-mono text-[12px] text-[var(--ink, #0c2a4d)]">{row.messages_per_minute ?? '-'}</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">SNR (dB)</div>
            <div className="font-mono text-[12px] text-[var(--ink, #0c2a4d)]">{row.signal_to_noise_db ?? '-'}</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">Latency p50</div>
            <div className="font-mono text-[12px] text-[var(--ink, #0c2a4d)]">{row.latency_p50_ms ?? '-'} ms</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">Latency p99</div>
            <div className="font-mono text-[12px] text-[var(--ink, #0c2a4d)]">{row.latency_p99_ms ?? '-'} ms</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">Jitter</div>
            <div className="font-mono text-[12px] text-[var(--ink, #0c2a4d)]">{row.jitter_ms ?? '-'} ms</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">Packet loss</div>
            <div className={`font-mono text-[12px] ${(row.packet_loss_pct ?? 0) > 1 ? 'text-[var(--bad, #9b1f1f)] font-semibold' : 'text-[var(--ink, #0c2a4d)]'}`}>{row.packet_loss_pct ?? '-'} %</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">TLS cert</div>
          ...
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">IEC 62351</div>
            <div className={`font-mono text-[12px] ${row.iec_62351_cipher_ok ? 'text-[var(--good, #1f5b3a)]' : 'text-[#a06200]'}`}>{row.iec_62351_cipher_ok ? 'OK' : 'NO'}</div>
          </div>
          <div>
  ...
  return (
    <div onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="w-full max-w-2xl rounded bg-surface-v2 p-4 text-[12px] text-[oklch(0.46_0.16_55)]">
        <div className="mb-3 flex items-start justify-between">
          <div>
            <h3 className="text-base font-semibold text-[var(--ink, #0c2a4d)]">Propose SCADA connector</h3>
            <p className="text-[11px] text-[var(--ink-2, #4a5568)]">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Peer id (SCADA counterparty)">
            <input value={peerId} onChange={(e) => setPeerId(e.target.value)} className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]" placeholder="ntcsa-koeberg-220kv" />
          </Field>
          <Field label="Substation name">
            <input value={substation} onChange={(e) => setSubstation(e.target.value)} className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]" placeholder="Koeberg 400kV" />
          </Field>
          <Field label="Capacity (MVA)">
            <input value={capacity} onChange={(e) => setCapacity(e.target.value)} type="number" className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]" placeholder="500" />
          </Field>
          <Field label="Protocol">
            <select value={protocol} onChange={(e) => setProtocol(e.target.value as ScadaProtocol)} className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]">
              {PROTOCOL_OPTIONS.map((p) => <option key={p.key} value={p.key}>{p.label}</option>)}
            </select>
          </Field>
          <Field label="Endpoint URL">
            <input value={endpointUrl} onChange={(e) => setEndpointUrl(e.target.value)} className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]" placeholder="iec61850://10.20.30.40:102" />
          </Field>
          <Field label="Title">
            <input value={title} onChange={(e) => setTitle(e.target.value)} className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]" placeholder="Koeberg 400kV IEC 61850 MMS connector" />
          </Field>
          ...
          <Field label="Transmission outage ref">
            <input value={w110} onChange={(e) => setW110(e.target.value)} className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]" placeholder="outage-tx-2026-0042" />
          </Field>
          <Field label="Reserve activation ref">
            <input value={w50} onChange={(e) => setW50(e.target.value)} className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]" placeholder="rsv-act-2026-0021" />
          </Field>
          <Field label="Grid code compliance ref">
            <input value={w67} onChange={(e) => setW67(e.target.value)} className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]" placeholder="gcc-2026-0011" />
          </Field>
        ...
        <div className="mt-3 rounded border border-[var(--border-subtle, #d8dde6)] bg-[var(--s1, #f8fafc)] p-3 text-[11px]">
          <div className="mb-2 text-[10px] uppercase tracking-wider text-[var(--ink-2, #4a5568)]">Floor flags (FLOOR-AT-LARGE {'≥'}1, FLOOR-AT-NATIONAL {'≥'}3)</div>
