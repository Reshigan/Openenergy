...
const TYPE_STYLE: Record<EntityType, { icon: IconName; bg: string; fg: string; route?: (id: string) => string }> = {
  participant: { icon: 'user',         bg: '#dbecfb', fg: '#1a5d97', route: (id) => `/admin?participant=${encodeURIComponent(id)}` },
  ...
  trade:       { icon: 'bolt',         bg: '#dbecfb', fg: '#3b82c4', route: (id) => `/trading?id=${encodeURIComponent(id)}` },
  ...
  rfp:         { icon: 'doc-stack',    bg: '#dbecfb', fg: '#3b82c4', route: (id) => `/procurement?rfp=${encodeURIComponent(id)}` },
