...
const TYPE_STYLE: Record<EntityType, { icon: IconName; bg: string; fg: string; route?: (id: string) => string }> = {
  participant: { icon: 'user',         bg: 'oklch(0.94 0.02 250)', fg: 'var(--info, #1a5d97)', route: (id) => `/admin?participant=${encodeURIComponent(id)}` },
  contract:    { icon: 'contract',     bg: '#ece4f5', fg: '#5d3a7e', route: (id) => `/contracts/${encodeURIComponent(id)}` },
  loi:         { icon: 'loi',          bg: '#d4e7f6', fg: 'var(--info, #1a5d97)', route: (id) => `/lois/${encodeURIComponent(id)}` },
  ...
  invoice:     { icon: 'invoice',      bg: 'color-mix(in oklab, var(--bad) 15%, var(--s1))', fg: 'var(--bad, #c0392b)', route: (id) => `/settlement?invoice=${encodeURIComponent(id)}` },
  ...
  fault:       { icon: 'alert',        bg: 'color-mix(in oklab, var(--bad) 15%, var(--s1))', fg: 'var(--bad, #c0392b)', route: (id) => `/om?fault=${encodeURIComponent(id)}` },
  generic:     { icon: 'connection',   bg: 'var(--s2, #eef2f7)', fg: 'var(--ink-2, #3d4756)' },
};
