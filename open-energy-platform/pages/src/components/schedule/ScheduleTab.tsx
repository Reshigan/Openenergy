    ...
    <div data-testid="schedule-tab" className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <button type="button"
          ...
          disabled={loading || busy !== null}
          onClick={level}
          className="px-3 py-1.5 text-sm rounded-md border border-[var(--border-subtle, #dde4ec)] hover:bg-[var(--s1, #f8fafc)] disabled:opacity-50"
        >
        ...
        {state?.last_computed_at ? (
          <span className="text-xs text-[var(--ink-2, #4a5568)] ml-2">Last computed {fmtDate(state.last_computed_at)} · v{state.version}</span>
        ) : null}
      </div>

      {/* ── Gantt ──────────────────────────────────────────────────── */}
      <div className="rounded-md border border-[var(--border-subtle, #dde4ec)] bg-surface-v2 p-3 overflow-x-auto" data-testid="schedule-gantt">
        <div className="text-sm font-medium text-[var(--ink, #2d3748)] mb-2">Gantt</div>
        <GanttBoard rows={gantt.rows} totalDays={gantt.totalDays} startDate={kpis.start || ''} />
      </div>

      {/* ── WBS table ─────────────────────────────────────────────── */}
      <div className="rounded-md border border-[var(--border-subtle, #dde4ec)] bg-surface-v2 overflow-x-auto" data-testid="schedule-table">
        <table className="w-full text-sm">
          <thead className="bg-[var(--s1, #f8fafc)] text-[var(--ink-2, #3d4756)] border-b border-[var(--border-subtle, #dde4ec)]">
            <tr>
          ...
          <tbody>
            {activities.map(a => (
              <tr key={a.id} className={`border-b border-[var(--s2, #eef2f7)] ${a.type === 'summary' ? 'font-semibold bg-[var(--s1, #f8fafc)]' : ''}`}>
      ...
      {/* ── Resources ─────────────────────────────────────────────── */}
      <div className="rounded-md border border-[var(--border-subtle, #dde4ec)] bg-surface-v2 p-3" data-testid="schedule-resources">
        <div className="text-sm font-medium text-[var(--ink, #2d3748)] mb-2">Resources</div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-2">
          {resources.map(r => (
            <div key={r.id} className="text-xs border border-[var(--border-subtle, #dde4ec)] rounded p-2">
              <div className="font-medium text-[var(--ink, #1e2a38)]">{r.name}</div>
              <div className="text-[var(--ink-2, #4a5568)]">{r.resource_type} · max {r.max_units}{r.unit ? ` ${r.unit}` : ''}</div>
            </div>
          ))}
          {!resources.length && <div className="text-xs text-[var(--ink-2, #4a5568)]">No resources defined yet.</div>}
        </div>
      </div>

      {/* ── Baselines ─────────────────────────────────────────────── */}
      <div className="rounded-md border border-[var(--border-subtle, #dde4ec)] bg-surface-v2 p-3" data-testid="schedule-baselines">
        <div className="text-sm font-medium text-[var(--ink, #2d3748)] mb-2">Baselines</div>
        {baselines.length ? (
          <table className="w-full text-sm">
            <thead className="text-[var(--ink-2, #4a5568)] text-xs">
              <tr>
        ...
        ) : <div className="text-xs text-[var(--ink-2, #4a5568)]">No baselines saved yet.</div>}
      </div>
    </div>
  );
}
...
function Kpi({ label, value, mono, accent }: { label: string; value: any; mono?: boolean; accent?: 'critical' }) {
  return (
    <div className="rounded-md border border-[var(--border-subtle, #dde4ec)] bg-surface-v2 px-3 py-2">
      <div className="text-[10px] uppercase tracking-wider text-[var(--ink-2, #4a5568)]">{label}</div>
      <div className={`text-lg ${mono ? 'font-mono' : 'font-semibold'} ${accent === 'critical' ? 'text-red-600' : 'text-[var(--ink, #0f1c2e)]'}`}>
        {value}
      </div>
    </div>
  );
...
function GanttBoard({ rows, totalDays, startDate }: {
  ...
  if (!rows.length || !totalDays) return <div className="text-xs text-[var(--ink-2, #4a5568)]">Run “Recompute CPM” to schedule activities.</div>;
  ...
  return (
    <svg width={width} height={height} role="img" aria-label="Gantt chart">
      ...
      {monthTicks.map((t, i) => (
        <g key={i}>
          <line x1={t.x} x2={t.x} y1={20} y2={height} stroke="var(--border-subtle, #e2e8f0)" />
          <text x={t.x + 2} y={14} fontSize="10" fill="var(--ink-2, #64748b)">{t.label}</text>
        </g>
      ))}
      {rows.map((r, i) => {
        const y = 30 + i * rowH;
        ...
        const color = r.is_critical ? 'var(--bad, #dc2626)' : '#0f766e';
        return (
          <g key={r.id}>
            <text x={4} y={y + 14} fontSize="11" fill="var(--ink, #0f172a)">{r.wbs_code} · {r.name.slice(0, 28)}</text>
            {r.type === 'milestone'
