...
const STATUS_COLOR: Record<RiskStatus, string> = {
  ...
  archived:         'bg-[var(--s2, #eef2f7)] text-[var(--ink-2, #6b7685)]',
  escalated:        'bg-red-200 text-red-900',
  deferred:         'bg-[var(--s2, #eef2f7)] text-[var(--ink-2, #3d4756)]',
  cancelled:        'bg-[var(--border-subtle, #e8ecf0)] text-[var(--ink-2, #6b7685)]',
...
function scoreColor(score: number | null): string {
  if (!score) return 'text-[var(--ink-2, #9aa5b4)]';
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="text-xs text-[var(--ink-2, #9aa5b4)] ml-auto">{filtered.length} risks</span>
        {!readOnly && (
      ...
      {loading && <div className="text-xs text-[var(--ink-2, #9aa5b4)]">Loading risk register…</div>}
          ...
          <table className="w-full text-xs">
            <thead className="bg-[var(--s1, #f8fafc)]">
              <tr>
                ...
                <th className="text-left px-3 py-2 font-medium text-[var(--ink-2, #6b7685)]">P×I</th>
                ...
                <th className="text-left px-3 py-2 font-medium text-[var(--ink-2, #6b7685)]">Strategy</th>
                ...
                <th className="text-left px-3 py-2 font-medium text-[var(--ink-2, #6b7685)]">Project</th>
                {!readOnly && <th className="px-3 py-2" />}
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 && (
                <tr><td colSpan={readOnly ? 10 : 11} className="px-3 py-6 text-center text-[var(--ink-2, #9aa5b4)]">No risks in register</td></tr>
              )}
              {filtered.map(row => (
                <tr key={row.id} className="border-t border-[var(--s2, #eef2f7)] hover:bg-[var(--s2, #eef2f7)] cursor-pointer" onClick={() => setSelected(row)}>
                  ...
                  <td className="px-3 py-2 max-w-[180px]">
                    <span className="font-medium text-[var(--ink, #1e2a38)] truncate block">{row.title}</span>
                  </td>
                  ...
                  <td className="px-3 py-2 capitalize text-[var(--ink-2, #3d4756)]">{row.risk_category.replace('_', ' ')}</td>
                  <td className="px-3 py-2">
                  ...
                  <td className="px-3 py-2 capitalize text-[var(--ink-2, #3d4756)]">{row.response_strategy ?? '—'}</td>
                  <td className="px-3 py-2">
                  ...
                  <td className="px-3 py-2 text-[var(--ink-2, #6b7685)] max-w-[100px] truncate">{row.project_name ?? row.project_id}</td>
                  {!readOnly && (
                    <td className="px-3 py-2">
      ...
      {selected && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => { setSelected(null); setActionResult(null); }}>
          <div className="bg-surface-v2 rounded-xl shadow-2xl max-w-2xl w-full p-6 max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <h3 className="font-semibold text-[var(--ink, #0f1c2e)]">{selected.title}</h3>
            ...
            <div className="mb-4 px-3 py-3 bg-[var(--s1, #f8fafc)] rounded-lg">
              <p className="text-[10px] text-[var(--ink-2, #9aa5b4)] uppercase tracking-wide mb-2">Treatment progress</p>
              <ChainStateBar
            ...
            {selected.description && <p className="text-sm text-[var(--ink-2, #3d4756)] mb-4">{selected.description}</p>}
      ...
      {showCreate && !readOnly && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setShowCreate(false)}>
          <div className="bg-surface-v2 rounded-xl shadow-2xl max-w-lg w-full p-6 max-h-[85vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-[var(--ink, #0f1c2e)]">Identify new risk</h3>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <FormField label={`Probability (P${newProb})`}>
                  ...
                  <p className="text-[10px] text-[var(--ink-2, #9aa5b4)] mt-0.5">{['Rare','Unlikely','Possible','Likely','Almost certain'][newProb-1]}</p>
                </FormField>
                <FormField label={`Impact (I${newImpact})`}>
                  ...
                  <p className="text-[10px] text-[var(--ink-2, #9aa5b4)] mt-0.5">{['Negligible','Minor','Moderate','Major','Catastrophic'][newImpact-1]}</p>
                </FormField>
              </div>
              <div className="bg-[var(--s1, #f8fafc)] rounded p-2 text-center">
                <span className="text-xs font-medium">Risk score: </span>
                <span className={`text-sm font-bold ${scoreColor(newProb * newImpact)}`}>{newProb}×{newImpact}={newProb * newImpact}</span>
                <span className="text-xs text-[var(--ink-2, #6b7685)] ml-2">({TIER_LABEL[
...
function KpiCard({ label, value, color }: { label: string; value: number; color: string }) {
  const cls = color === 'red' ? 'text-red-600' : color === 'orange' ? 'text-orange-600' : color === 'blue' ? 'text-[oklch(0.46_0.16_55)]' : 'text-[var(--ink, #2d3748)]';
  return (
