...
const STATUS_COLORS: Record<string, string> = {
  audit_due:                      'bg-gray-100 text-gray-500',
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {items.map(item => {
                ...
                return (
                  <tr key={item.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-2 pr-4 text-xs text-gray-700">{item.reporting_year}</td>
                    ...
                    <td className="py-2 pr-4 text-xs text-gray-700">{item.eco_name ?? '—'}</td>
                    <td className="py-2 pr-4 text-xs text-gray-500 font-mono">{eaRef}</td>
                    <td className="py-2 pr-4">
                    ...
                    <td className="py-2 pr-4 text-xs text-gray-600">{violation}</td>
              ...
              {items.length === 0 && (
                <tr><td colSpan={10} className="py-10 text-center text-gray-400 text-sm">No ECO audit reports found</td></tr>
              )}
            </tbody>
          </table>
        </div>
      ...
      {showCreate && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center" onClick={() => setShowCreate(false)}>
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-xs text-gray-500 mb-4">NEMA / EA condition — records an annual Environmental Compliance Officer audit report. Tier and SLA are derived from capacity.</p>
            ...
            <div className="flex gap-2 mt-4">
              <button onClick={createReport} disabled={createPending || !form.project_id || !form.capacity_mw} className="px-4 py-2 bg-indigo-600 text-white rounded text-sm hover:bg-indigo-700 disabled:opacity-50">
