      ...
      {loading ? <div className="text-sm text-[var(--ink-2, #9aa5b4)] py-4">Loading…</div> : (
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {items.map(ct => (
                <tr key={ct.id} className="border-b hover:bg-[var(--s2, #eef2f7)] cursor-pointer" onClick={() => setSelected(ct)}>
                  ...
                  <td className="py-2 pr-4 text-xs text-[var(--ink-2, #6b7685)]">{ct.test_category ? TEST_LABELS[ct.test_category] ?? ct.test_category : '—'}</td>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex justify-end" onClick={() => setSelected(null)}>
          <div className="bg-surface-v2 w-full max-w-xl h-full overflow-y-auto p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm mb-4">
              ...
              <div><span className="text-[var(--ink-2, #6b7685)]">Test Type:</span> {selected.test_category ? TEST_LABELS[selected.test_category] ?? selected.test_category : '—'}</div>
              {selected.contractor_firm && <div className="col-span-2"><span className="text-[var(--ink-2, #6b7685)]">Contractor:</span> {selected.contractor_firm}</div>}
              {selected.ie_firm && <div className="col-span-2"><span className="text-[var(--ink-2, #6b7685)]">IE:</span> {selected.ie_firm}</div>}
              {selected.pac_issued_at && <div><span className="text-[var(--ink-2, #6b7685)]">PAC Issued:</span> <span className="text-cyan-700">{fmtDate(selected.pac_issued_at)}</span></div>}
              {selected.cert_reference && <div className="col-span-2"><span className="text-[var(--ink-2, #6b7685)]">Cert Reference:</span> <span className="font-mono text-green-700">{selected.cert_reference}</span></div>}
              {selected.performance_cert_issued_at && <div><span className="text-[var(--ink-2, #6b7685)]">Cert Issued:</span> <span className="text-green-700">{fmtDate(selected.performance_cert_issued_at)}</span></div>}
            </div>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center" onClick={() => setShowCreate(false)}>
          <div className="bg-surface-v2 rounded-xl p-6 w-full max-w-md shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <select value={form.test_category} onChange={e => setForm(f => ({ ...f, test_category: e.target.value }))} className="w-full border rounded px-3 py-2 text-sm text-[var(--ink, #2d3748)]">
