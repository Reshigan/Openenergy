          ...
          <div className="bg-surface-v2 w-full max-w-xl h-full overflow-y-auto p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <div className="text-xs text-[var(--ink-2, #6b7685)] mt-1">{selected.milestone_tier} · {fmtZar(selected.milestone_value_zar)}</div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm mb-4">
              ...
              <div><span className="text-[var(--ink-2, #6b7685)]">Milestone:</span> {selected.milestone_category ? CATEGORY_LABELS[selected.milestone_category] ?? selected.milestone_category : '—'}</div>
              {selected.ie_firm && <div className="col-span-2"><span className="text-[var(--ink-2, #6b7685)]">IE Firm:</span> {selected.ie_firm}</div>}
              {selected.lender_reference && <div><span className="text-[var(--ink-2, #6b7685)]">Lender Ref:</span> <span className="font-mono text-xs">{selected.lender_reference}</span></div>}
              {selected.cert_number && <div><span className="text-[var(--ink-2, #6b7685)]">Cert Number:</span> <span className="font-mono text-green-700">{selected.cert_number}</span></div>}
              {selected.cert_issued_at && <div><span className="text-[var(--ink-2, #6b7685)]">Issued:</span> <span className="text-green-700">{fmtDate(selected.cert_issued_at)}</span></div>}
            </div>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center" onClick={() => setShowCreate(false)}>
          <div className="bg-surface-v2 rounded-xl p-6 w-full max-w-md shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <select value={form.milestone_category} onChange={e => setForm(f => ({ ...f, milestone_category: e.target.value }))} className="w-full border rounded px-3 py-2 text-sm text-[var(--ink, #2d3748)]">
