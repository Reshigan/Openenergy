...
const STATUS_COLORS: Record<string, string> = {
  application_submitted:  'bg-[var(--s2, oklch(0.94_0.008_250))] text-[var(--ink-2, oklch(0.40_0.009_250))]',
  defects_outstanding:    'bg-orange-100 text-orange-800',
  inspection_scheduled:   'bg-[var(--s2, oklch(0.94_0.008_250))] text-[var(--ink-2, oklch(0.40_0.009_250))]',
  inspection_complete:    'bg-[var(--s2, oklch(0.90_0.015_250))] text-[var(--ink, oklch(0.17_0.010_250))]',
  ...
  withdrawn:              'bg-[var(--s2, #eef2f7)] text-[var(--ink-2, #6b7685)]',
    ...
    <div className="p-4 space-y-4">
      ...
      {kpis && (
        <div className="grid grid-cols-2 gap-3">
          <div className="rounded-lg p-3 border border-orange-200 bg-orange-50">
            <div className="text-xs text-[var(--ink-2, #6b7685)]">Pending Retention</div>
          ...
          <div className="rounded-lg p-3 border border-green-200 bg-green-50">
            <div className="text-xs text-[var(--ink-2, #6b7685)]">Retention Released</div>
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        {['application_submitted','inspection_scheduled','snag_list_issued','fcc_issued','retention_released','disputed'].map(s => (
          <button type="button" key={s} onClick={() => { setFilterStatus(s); load(s, filterTier); }} className={`px-3 py-1 rounded text-xs border ${filterStatus === s ? 'bg-[var(--ink, #1e2a38)] text-white' : 'bg-surface-v2 text-[var(--ink-2, #3d4756)] border-[var(--border-subtle, #dde4ec)]'}`}>
            {STATUS_LABELS[s] ?? s}
          </button>
        ))}
      ...
      {loading ? <div className="text-sm text-[var(--ink-2, #9aa5b4)] py-4">Loading…</div> : (
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {items.map(fc => (
                <tr key={fc.id} className="border-b hover:bg-[var(--s2, #eef2f7)] cursor-pointer" onClick={() => setSelected(fc)}>
                  <td className="py-2 pr-4 text-xs font-medium text-[var(--ink, #2d3748)] max-w-[160px] truncate">{fc.description?.slice(0, 50) ?? fc.project_id}</td>
                  ...
                  <td className="py-2 pr-4 text-xs text-[var(--ink-2, #6b7685)]">{fmtDate(fc.dlp_end_date)}</td>
                  <td className="py-2 pr-4">
                  ...
                  <td className={`py-2 pr-4 text-xs ${fc.sla_breached ? 'text-red-600 font-semibold' : 'text-[var(--ink-2, #6b7685)]'}`}>
              ...
              {items.length === 0 && (
                <tr><td colSpan={9} className="py-6 text-center text-[var(--ink-2, #9aa5b4)] text-sm">No FCC applications found</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <div className="bg-surface-v2 w-full max-w-xl h-full overflow-y-auto p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <div className="text-xs text-[var(--ink-2, #6b7685)] mt-1">{selected.contract_tier} · {selected.project_id}</div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm mb-4">
              <div><span className="text-[var(--ink-2, #6b7685)]">Contract Value:</span> <span className="font-bold">{fmt(selected.contract_value_zar)}</span></div>
              <div><span className="text-[var(--ink-2, #6b7685)]">Retention:</span> <span className="font-bold text-orange-700">{fmt(selected.retention_amount_zar)}</span></div>
              <div><span className="text-[var(--ink-2, #6b7685)]">Status:</span> <span className={`px-2 py-0.5 rounded text-xs ${STATUS_COLORS[selected.chain_status]}`}>{STATUS_LABELS[selected.chain_status]}</span></div>
              <div><span className="text-[var(--ink-2, #6b7685)]">Snags:</span> {selected.snag_count ?? '—'}</div>
              <div><span className="text-[var(--ink-2, #6b7685)]">Practical Completion:</span> {fmtDate(selected.practical_completion_date)}</div>
              <div><span className="text-[var(--ink-2, #6b7685)]">DLP End:</span> {fmtDate(selected.dlp_end_date)}</div>
              {selected.fcc_issued_at && <div><span className="text-[var(--ink-2, #6b7685)]">FCC Issued:</span> <span className="text-green-700 font-semibold">{fmtDate(selected.fcc_issued_at)}</span></div>}
              {selected.retention_released_at && <div><span className="text-[var(--ink-2, #6b7685)]">Retention Released:</span> <span className="text-green-700 font-semibold">{fmtDate(selected.retention_released_at)}</span></div>}
            </div>
          ...
          <div className="bg-surface-v2 rounded-xl p-6 w-full max-w-md shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-xs text-[var(--ink-2, #6b7685)] mb-1 block">Practical Completion *</label>
                  <input type="date" value={form.practical_completion_date}
                    onChange={e => setForm(f => ({ ...f, practical_completion_date: e.target.value }))}
                    className="w-full border rounded px-3 py-2 text-sm" />
                </div>
                <div>
                  <label className="text-xs text-[var(--ink-2, #6b7685)] mb-1 block">DLP End Date *</label>
