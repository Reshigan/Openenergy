        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {items.map(item => {
                ...
                return (
                  <tr key={item.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-2 pr-4 text-xs text-gray-700">{CATEGORY_LABELS[item.om_contract_category] ?? item.om_contract_category}</td>
                    <td className="py-2 pr-4 text-xs tabular-nums text-gray-700">{fmtZar(item.annual_om_value_zar)}</td>
                    ...
                    <td className="py-2 pr-4 text-xs text-gray-500">{item.preferred_bidder_name ?? '—'}</td>
                    <td className="py-2 pr-4">
                      {regulator && (
              ...
              {items.length === 0 && (
                <tr><td colSpan={9} className="py-10 text-center text-gray-400 text-sm">No O&M contracts found</td></tr>
              )}
            </tbody>
          </table>
        </div>
