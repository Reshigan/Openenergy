...
interface CccNegotiation {
  ...
  project_id: string;
  ccc_category: 'line_extension' | 'substation_upgrade' | 'protection_relay' | 'reactive_compensation' | 'metering_telecoms' | 'combined';
  ccc_amount_zar: number;
  ccc_tier: 'minor' | 'moderate' | 'significant' | 'major' | 'material';
  network_operator: string;
  grid_connection_ref: string | null;
  chain_status: string;
  sla_due_at: string;
  sla_breached: number;
  expert_appointed_at: string | null;
  provisional_agreement_at: string | null;
  ccc_agreed_at: string | null;
  ccc_rejected_at: string | null;
  nersa_referral_at: string | null;
  created_at: string;
  updated_at: string;
}

const STATUS_COLORS: Record<string, string> = {
  ccc_initiated:             'bg-gray-100 text-gray-500',
...
const CATEGORY_LABELS: Record<string, string> = {
  ...
  reactive_compensation:'Reactive Comp.',
  metering_telecoms:    'Metering/Telecoms',
  combined:             'Combined',
};

const TERMINAL_STATUSES = new Set(['ccc_agreed', 'ccc_rejected', 'regulatory_determination']);
...
const fmtZar = (n: number) => n >= 1e8 ? `R${(n / 1e6).toFixed(0)}M` : `R${(n / 1e6).toFixed(1)}M`;

function hasRegulatorFlag(row: CccNegotiation): boolean {
  if (row.ccc_rejected_at !== null) return true;
  if (row.nersa_referral_at !== null) return true;
  if (row.ccc_agreed_at !== null && (row.ccc_tier === 'major' || row.ccc_tier === 'material')) return true;
  return false;
}
...
export function IppCccTab() {
  const [items, setItems] = useState<CccNegotiation[]>([]);
  ...
  async function load(status = filterStatus, tier = filterTier, category = filterCategory) {
    ...
    try {
      ...
      const res = await fetch(`/api/ipp-ccc?${params}`, {
  ...
  const agreed  = items.filter(i => i.ccc_agreed_at !== null).length;
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KpiChip label="Total Negotiations" value={total} />
        <KpiChip label="Open"               value={open}     mode={open > 0 ? 'alert' : 'neutral'} />
        <KpiChip label="Agreed"             value={agreed}   mode={agreed > 0 ? 'good' : 'neutral'} />
          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="border-b text-left text-xs text-gray-500">
                ...
                <th className="pb-2 pr-4">CCC Amount</th>
                <th className="pb-2 pr-4">Tier</th>
                <th className="pb-2 pr-4">Status</th>
                <th className="pb-2 pr-4">SLA Due</th>
                <th className="pb-2 pr-4">Grid Ref</th>
            ...
            <tbody>
              {items.map(item => {
                ...
                return (
                  <tr key={item.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-2 pr-4 text-xs text-gray-700">{item.network_operator}</td>
                    <td className="py-2 pr-4 text-xs text-gray-700">{CATEGORY_LABELS[item.ccc_category] ?? item.ccc_category}</td>
                    <td className="py-2 pr-4 text-xs tabular-nums text-gray-700">{fmtZar(item.ccc_amount_zar)}</td>
                    ...
                    <td className="py-2 pr-4 text-xs text-gray-500">{item.grid_connection_ref ?? '—'}</td>
                    <td className="py-2 pr-4">
                      {regulator && (
              ...
              {items.length === 0 && (
                <tr><td colSpan={9} className="py-10 text-center text-gray-400 text-sm">No CCC negotiations found</td></tr>
              )}
            </tbody>
          </table>
        </div>
