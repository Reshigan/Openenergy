...
const STATUS_COLORS: Record<string, string> = {
  assessment_due:        'bg-gray-100 text-gray-500',
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {items.map(item => {
                ...
                return (
                  <tr key={item.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-2 pr-4 text-xs text-gray-700">{item.project_id}</td>
                    <td className="py-2 pr-4 text-xs text-gray-700">{CATEGORY_LABELS[item.compliance_category] ?? item.compliance_category}</td>
                    <td className="py-2 pr-4 text-xs tabular-nums text-gray-700">{item.assessment_year}</td>
              ...
              {items.length === 0 && (
                <tr><td colSpan={9} className="py-10 text-center text-gray-400 text-sm">No grid compliance assessments found</td></tr>
              )}
            </tbody>
          </table>
        </div>
