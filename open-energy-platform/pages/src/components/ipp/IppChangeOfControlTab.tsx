...
const STATUS_COLORS: Record<string, string> = {
  notification_submitted:   'bg-[var(--s2, #eef2f7)] text-[var(--ink-2, #6b7685)]',
  completeness_check:       'bg-[var(--s2, #eef2f7)]',
  foreign_ownership_screen: 'bg-purple-100 text-purple-700',
  competition_screen:       'bg-[var(--s2, #eef2f7)]',
  ...
  appeal_determined:        'bg-[var(--border-subtle, #e8ecf0)] text-[var(--ink-2, #3d4756)]',
};
...
const TIER_COLORS: Record<string, string> = {
  ...
  moderate:    'bg-[var(--s2, #eef2f7)]',
  significant: 'bg-[var(--s2, #eef2f7)]',
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        {(['all', 'domestic', 'foreign'] as const).map(f => (
          <button type="button"
            ...
            className={`px-2 py-1 rounded text-xs border ${filterForeign === f ? 'bg-[#c2873a] text-white border-[#c2873a]' : 'bg-surface-v2 text-[var(--ink-2, #3d4756)] border-[var(--border-subtle, #dde4ec)]'}`}
          >
                ...
                return (
                  ...
                  >
                    <td className="py-2 pr-4 text-xs max-w-[200px]">
                      <div className="truncate font-medium text-[var(--ink, #1e2a38)]">{item.acquirer_name}</div>
                      <div className="text-[var(--ink-2, #9aa5b4)] truncate font-mono">{item.project_id}</div>
                    </td>
                    ...
                    <td className="py-2 pr-4">
                      <span className={`px-2 py-0.5 rounded text-xs ${TIER_COLORS[item.ownership_tier] ?? 'bg-[var(--s2, #eef2f7)] text-[var(--ink-2, #6b7685)]'}`} style={TIER_STYLE[item.ownership_tier] ?? {}}>
                        {item.ownership_tier}
                      </span>
                    </td>
                    <td className="py-2 pr-4 text-xs">
                      {isForeign ? <span title={item.foreign_ownership_flag}>🌍</span> : <span className="text-[var(--ink-2, #9aa5b4)]">—</span>}
                    </td>
                    <td className="py-2 pr-4">
                      <span className={`px-2 py-0.5 rounded text-xs ${STATUS_COLORS[item.chain_status] ?? 'bg-[var(--s2, #eef2f7)] text-[var(--ink-2, #6b7685)]'}`} style={STATUS_STYLE[item.chain_status] ?? {}}>
          ...
          <div className="bg-surface-v2 w-full max-w-xl h-full overflow-y-auto p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <div className="text-xs text-[var(--ink-2, #6b7685)] mt-1">
              ...
              <button type="button" onClick={() => setSelected(null)} className="text-[var(--ink-2, #9aa5b4)] hover:text-[var(--ink, #2d3748)] text-lg leading-none">✕</button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm mb-4">
              <div>
                <span className="text-[var(--ink-2, #6b7685)] text-xs">Status</span>
              ...
              <div>
                <span className="text-[var(--ink-2, #6b7685)] text-xs">Acquirer</span>
                <div className="text-xs font-medium">{selected.acquirer_name}</div>
              </div>
              <div>
                <span className="text-[var(--ink-2, #6b7685)] text-xs">Project</span>
                <div className="text-xs font-mono">{selected.project_id}</div>
              </div>
              {selected.capacity_mw != null && (
                <div>
                  <span className="text-[var(--ink-2, #6b7685)] text-xs">Capacity</span>
                  <div className="font-medium">{selected.capacity_mw} MW</div>
                </div>
              )}
              <div>
                <span className="text-[var(--ink-2, #6b7685)] text-xs">Ownership</span>
                <div className="text-xs">{selected.foreign_ownership_flag}</div>
              </div>
              <div>
                <span className="text-[var(--ink-2, #6b7685)] text-xs">SLA due</span>
              ...
              {selected.approval_granted_at && (
                <div>
                  <span className="text-[var(--ink-2, #6b7685)] text-xs">Approval granted</span>
                  <div className="text-green-700">{fmtDate(selected.approval_granted_at)}</div>
                </div>
              )}
              <div>
                <span className="text-[var(--ink-2, #6b7685)] text-xs">Created</span>
