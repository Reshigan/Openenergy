...
const STATUS_COLORS: Record<string, string> = {
  hra_triggered:              'bg-[var(--s2, #eef2f7)] text-[var(--ink-2, #6b7685)]',
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {pageItems.map(item => {
                ...
                const tierColor = TIER_COLORS[item.hra_capacity_tier] ?? 'var(--ink-2, #6b7280)';
                return (
                  <tr key={item.id} className="border-b hover:bg-[var(--s2, #eef2f7)]">
                    ...
                    <td className="py-2 pr-4 text-xs text-[var(--ink, #2d3748)]">{item.sahra_reference ?? '—'}</td>
                    <td className="py-2 pr-4 text-xs text-[var(--ink, #2d3748)]">{item.heritage_consultant ?? '—'}</td>
