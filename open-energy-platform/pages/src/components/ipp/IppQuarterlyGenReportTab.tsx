...
const STATUS_COLORS: Record<string, string> = {
  report_quarter_opened:           'bg-[var(--s2, #eef2f7)] text-[var(--ink-2, #6b7685)]',
      ...
      {showCreate && (
        ...
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            ...
            <div>
              <label className="block text-xs text-[var(--ink-2, #3d4756)] mb-1">MWh Contracted (optional)</label>
              <input
                type="number"
            ...
            <div>
              <label className="block text-xs text-[var(--ink-2, #3d4756)] mb-1">MWh Actual (optional)</label>
              <input
                type="number"
                value={formMwhActual}
            ...
            <div>
              <label className="block text-xs text-[var(--ink-2, #3d4756)] mb-1">Availability % (optional)</label>
              <input
                type="number"
            ...
            <tbody>
              {pageItems.map(item => {
                ...
                return (
                  ...
                  >
                    <td className="py-2 pr-3 text-xs font-mono text-[var(--ink, #2d3748)]">{item.report_quarter}</td>
                    ...
                    <td className="py-2 pr-3 text-xs tabular-nums text-[var(--ink, #2d3748)]">{fmtMw(item.project_mw)}</td>
                    <td className="py-2 pr-3">
                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${TIER_COLORS[item.project_tier] ?? 'bg-[var(--s2, #eef2f7)] text-[var(--ink-2, #6b7685)]'}`}>
                        {item.project_tier.charAt(0).toUpperCase() + item.project_tier.slice(1)}
                      </span>
                    </td>
                    <td className="py-2 pr-3 text-xs tabular-nums text-[var(--ink-2, #3d4756)]">{fmtMwh(item.mwh_contracted)}</td>
                    <td className="py-2 pr-3 text-xs tabular-nums text-[var(--ink-2, #3d4756)]">{fmtMwh(item.mwh_actual)}</td>
                    <td className="py-2 pr-3 text-xs tabular-nums text-[var(--ink-2, #3d4756)]">{fmtPct(item.availability_pct)}</td>
          ...
          <div className="bg-surface-v2 h-full w-full max-w-lg shadow-2xl overflow-y-auto flex flex-col">
            <div className="flex items-center justify-between px-5 py-4 border-b">
              <div>
                ...
                <div className="text-xs text-[var(--ink-2, #6b7685)] mt-0.5">{detailItem.project_ref}</div>
              </div>
              <button type="button"
            ...
            <div className="flex-1 p-5 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-x-6 gap-y-3 text-xs">
                <div>
                  <div className="text-[var(--ink-2, #9aa5b4)] mb-0.5">Project Ref</div>
                  <div className="font-mono text-[var(--ink, #1e2a38)]">{detailItem.project_ref}</div>
                </div>
                <div>
                  <div className="text-[var(--ink-2, #9aa5b4)] mb-0.5">Report Quarter</div>
                  <div className="text-[var(--ink, #1e2a38)]">{detailItem.report_quarter}</div>
                </div>
                <div>
                  <div className="text-[var(--ink-2, #9aa5b4)] mb-0.5">Project MW</div>
                  <div className="text-[var(--ink, #1e2a38)]">{fmtMw(detailItem.project_mw)}</div>
                </div>
                <div>
                  <div className="text-[var(--ink-2, #9aa5b4)] mb-0.5">Tier</div>
                  <span className={`px-2 py-0.5 rounded text-xs font-medium ${TIER_COLORS[detailItem.project_tier] ?? 'bg-[var(--s2, #eef2f7)] text-[var(--ink-2, #6b7685)]'}`}>
                    {detailItem.project_tier.charAt(0).toUpperCase() + detailItem.project_tier.slice(1)}
                  </span>
                </div>
                <div>
                  <div className="text-[var(--ink-2, #9aa5b4)] mb-0.5">MWh Contracted</div>
                  <div className="tabular-nums text-[var(--ink, #1e2a38)]">{fmtMwh(detailItem.mwh_contracted)}</div>
                </div>
                <div>
                  <div className="text-[var(--ink-2, #9aa5b4)] mb-0.5">MWh Actual</div>
                  <div className="tabular-nums text-[var(--ink, #1e2a38)]">{fmtMwh(detailItem.mwh_actual)}</div>
                </div>
                <div>
                  <div className="text-[var(--ink-2, #9aa5b4)] mb-0.5">Availability %</div>
                  <div className="tabular-nums text-[var(--ink, #1e2a38)]">{fmtPct(detailItem.availability_pct)}</div>
                </div>
                <div>
                  <div className="text-[var(--ink-2, #9aa5b4)] mb-0.5">Capacity Factor %</div>
                  <div className="tabular-nums text-[var(--ink, #1e2a38)]">{fmtPct(detailItem.capacity_factor_pct)}</div>
                </div>
                <div>
                  <div className="text-[var(--ink-2, #9aa5b4)] mb-0.5">ED Spend QTD (ZAR)</div>
                  <div className="tabular-nums text-[var(--ink, #1e2a38)]">{fmtZar(detailItem.ed_spend_qtd_zar)}</div>
                </div>
                <div>
                  <div className="text-[var(--ink-2, #9aa5b4)] mb-0.5">SED Spend QTD (ZAR)</div>
                  <div className="tabular-nums text-[var(--ink, #1e2a38)]">{fmtZar(detailItem.sed_spend_qtd_zar)}</div>
                </div>
                <div>
      ...
      {actionItem && (
        <div onMouseDown={(e) => { if (e.target === e.currentTarget) setActionItem(null); }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-surface-v2 rounded-xl shadow-xl p-6 w-full max-w-md mx-4">
            <div className="text-sm font-semibold text-[var(--ink, #1e2a38)] mb-1">DMRE Quarterly Report Action</div>
