...
const STATUS_COLORS: Record<string, string> = {
  tpa_application_submitted:  'bg-[var(--s2, #eef2f7)] text-[var(--ink-2, #6b7685)]',
    ...
    <div className="p-4 space-y-4">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        {['minor','moderate','significant','major','material'].map(t => (
          <button type="button" key={t} onClick={() => { const nt = filterTier === t ? '' : t; setFilterTier(nt); load(filterStatus, nt); }} className={`px-2 py-1 rounded text-xs border ${filterTier === t ? 'bg-[oklch(0.40_0.15_55)] text-white' : 'bg-surface-v2 text-[var(--ink-2, #3d4756)] border-[var(--border-subtle, #dde4ec)]'}`}>{t}</button>
        ))}
      ...
      {loading ? <div className="text-sm text-[var(--ink-2, #9aa5b4)] py-4">Loading…</div> : (
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {items.map(tpa => (
                <tr key={tpa.id} className="border-b hover:bg-[var(--s2, #eef2f7)] cursor-pointer" onClick={() => setSelected(tpa)}>
                  ...
                  <td className="py-2 pr-4 text-xs text-[var(--ink-2, #6b7685)]">{tpa.tpa_category ? CATEGORY_LABELS[tpa.tpa_category] ?? tpa.tpa_category : '—'}</td>
                  <td className="py-2 pr-4 text-xs text-[var(--ink-2, #6b7685)] max-w-[120px] truncate">{tpa.network_owner ?? '—'}</td>
                  ...
                  <td className={`py-2 pr-4 text-xs ${tpa.sla_breached ? 'text-red-600 font-semibold' : 'text-[var(--ink-2, #6b7685)]'}`}>
                    {tpa.sla_breached ? '⚠ BREACHED' : fmtDate(tpa.sla_due_at)}
                  </td>
                  <td className="py-2 text-xs text-[oklch(0.46_0.16_55)]">View →</td>
                </tr>
              ))}
              {items.length === 0 && <tr><td colSpan={9} className="py-6 text-center text-[var(--ink-2, #9aa5b4)] text-sm">No TPA agreement records found</td></tr>}
            </tbody>
          </table>
        </div>
      )}
          ...
          <div className="bg-surface-v2 w-full max-w-xl h-full overflow-y-auto p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <div className="text-xs text-[var(--ink-2, #6b7685)] mt-1">{selected.capacity_tier} · {selected.wheeling_capacity_mw} MW</div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm mb-4">
              ...
              <div><span className="text-[var(--ink-2, #6b7685)]">Network:</span> {selected.tpa_category ? CATEGORY_LABELS[selected.tpa_category] ?? selected.tpa_category : '—'}</div>
              {selected.network_owner && <div className="col-span-2"><span className="text-[var(--ink-2, #6b7685)]">Network Owner:</span> {selected.network_owner}</div>}
              {selected.offtaker_reference && <div><span className="text-[var(--ink-2, #6b7685)]">Off-taker Ref:</span> <span className="font-mono text-xs">{selected.offtaker_reference}</span></div>}
              {selected.agreement_reference && <div><span className="text-[var(--ink-2, #6b7685)]">Agreement Ref:</span> <span className="font-mono text-green-700">{selected.agreement_reference}</span></div>}
              {selected.tpa_signed_at && <div><span className="text-[var(--ink-2, #6b7685)]">Signed:</span> <span className="text-green-700">{fmtDate(selected.tpa_signed_at)}</span></div>}
              {selected.wheeling_activated_at && <div><span className="text-[var(--ink-2, #6b7685)]">Activated:</span> <span className="text-green-700">{fmtDate(selected.wheeling_activated_at)}</span></div>}
            </div>
              ...
              <div className="space-y-2">
                ...
                {ACTION_MAP[selected.chain_status].map(a => (
                  <button type="button" key={a.action} disabled={actionPending}
                    ...
                    className={`w-full text-left px-3 py-2 rounded border text-sm ${a.danger ? 'border-red-200 text-red-600 hover:bg-red-50' : 'border-[var(--border-subtle, #dde4ec)] text-[var(--ink, #2d3748)] hover:bg-[oklch(0.97_0.003_250)] hover:border-indigo-300'}`}>
                    {a.label}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center" onClick={() => setShowCreate(false)}>
          <div className="bg-surface-v2 rounded-xl p-6 w-full max-w-md shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <select value={form.tpa_category} onChange={e => setForm(f => ({ ...f, tpa_category: e.target.value }))} className="w-full border rounded px-3 py-2 text-sm text-[var(--ink, #2d3748)]">
