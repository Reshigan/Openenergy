...
const STATUS_COLORS: Record<string, string> = {
  tpa_application_submitted:  'bg-gray-100 text-gray-500',
...
export function IppTpaTab() {
  ...
  if (!loaded) return (
    <div className="p-6">
      <button type="button" onClick={() => load()} className="px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 text-sm">Load TPA Agreements</button>
    </div>
  );
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <button type="button" onClick={() => setShowCreate(true)} className="ml-auto px-3 py-1 bg-indigo-600 text-white rounded text-xs hover:bg-indigo-700">+ New TPA</button>
      ...
      {loading ? <div className="text-sm text-gray-400 py-4">Loading…</div> : (
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {items.map(tpa => (
                <tr key={tpa.id} className="border-b hover:bg-gray-50 cursor-pointer" onClick={() => setSelected(tpa)}>
                  ...
                  <td className="py-2 pr-4 text-xs text-gray-500">{tpa.tpa_category ? CATEGORY_LABELS[tpa.tpa_category] ?? tpa.tpa_category : '—'}</td>
                  <td className="py-2 pr-4 text-xs text-gray-500 max-w-[120px] truncate">{tpa.network_owner ?? '—'}</td>
                  ...
                  <td className={`py-2 pr-4 text-xs ${tpa.sla_breached ? 'text-red-600 font-semibold' : 'text-gray-500'}`}>
                    {tpa.sla_breached ? '⚠ BREACHED' : fmtDate(tpa.sla_due_at)}
                  </td>
                  <td className="py-2 text-xs text-indigo-600">View →</td>
                </tr>
              ))}
              {items.length === 0 && <tr><td colSpan={9} className="py-6 text-center text-gray-400 text-sm">No TPA agreement records found</td></tr>}
            </tbody>
          </table>
        </div>
      )}
          ...
          <div className="bg-white w-full max-w-xl h-full overflow-y-auto p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <div className="text-xs text-gray-500 mt-1">{selected.capacity_tier} · {selected.wheeling_capacity_mw} MW</div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm mb-4">
              ...
              <div><span className="text-gray-500">Network:</span> {selected.tpa_category ? CATEGORY_LABELS[selected.tpa_category] ?? selected.tpa_category : '—'}</div>
              {selected.network_owner && <div className="col-span-2"><span className="text-gray-500">Network Owner:</span> {selected.network_owner}</div>}
              {selected.offtaker_reference && <div><span className="text-gray-500">Off-taker Ref:</span> <span className="font-mono text-xs">{selected.offtaker_reference}</span></div>}
              {selected.agreement_reference && <div><span className="text-gray-500">Agreement Ref:</span> <span className="font-mono text-green-700">{selected.agreement_reference}</span></div>}
              {selected.tpa_signed_at && <div><span className="text-gray-500">Signed:</span> <span className="text-green-700">{fmtDate(selected.tpa_signed_at)}</span></div>}
              {selected.wheeling_activated_at && <div><span className="text-gray-500">Activated:</span> <span className="text-green-700">{fmtDate(selected.wheeling_activated_at)}</span></div>}
            </div>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center" onClick={() => setShowCreate(false)}>
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-3">
              ...
              <select value={form.tpa_category} onChange={e => setForm(f => ({ ...f, tpa_category: e.target.value }))} className="w-full border rounded px-3 py-2 text-sm text-gray-700">
            ...
            <div className="flex gap-2 mt-4">
              <button type="button" onClick={createRecord} disabled={!form.wheeling_capacity_mw} className="px-4 py-2 bg-indigo-600 text-white rounded text-sm hover:bg-indigo-700 disabled:opacity-50">Create</button>
