...
const STATUS_COLORS: Record<string, string> = {
  ...
  submitted_to_om:        'bg-indigo-100 text-indigo-800',
  om_review:              'bg-indigo-200 text-indigo-900',
      ...
      {loading ? <div className="text-sm text-gray-400 py-4">Loading…</div> : (
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {items.map(h => (
                <tr key={h.id} className="border-b hover:bg-gray-50 cursor-pointer" onClick={() => setSelected(h)}>
                  <td className="py-2 pr-4 text-xs font-medium max-w-[200px] truncate">{h.title}</td>
                  <td className="py-2 pr-4 text-xs text-gray-600">{CATEGORY_LABELS[h.category] ?? h.category}</td>
                  <td className="py-2 pr-4"><span className={`px-2 py-0.5 rounded text-xs ${TIER_COLORS[h.capacity_tier]}`}>{h.capacity_tier}</span></td>
                  <td className="py-2 pr-4 text-xs text-gray-600">{h.capacity_mw} MW</td>
                  <td className="py-2 pr-4 text-xs text-gray-500">{h.document_count ?? '—'}</td>
                  ...
                  <td className={`py-2 pr-4 text-xs ${h.sla_breached ? 'text-red-600 font-semibold' : 'text-gray-500'}`}>
                    {h.sla_breached ? '⚠ BREACHED' : fmtDate(h.sla_due_at)}
                  </td>
                  <td className="py-2 text-xs text-indigo-600">View →</td>
                </tr>
              ))}
              {items.length === 0 && <tr><td colSpan={8} className="py-6 text-center text-gray-400 text-sm">No handover packs found</td></tr>}
            </tbody>
          </table>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex justify-end" onClick={() => setSelected(null)}>
          <div className="bg-white w-full max-w-xl h-full overflow-y-auto p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm mb-4">
              ...
              <div><span className="text-gray-500">Documents:</span> {selected.document_count ?? '—'}</div>
              {selected.deficiency_count != null && <div><span className="text-gray-500">Deficiencies:</span> <span className="text-orange-700 font-semibold">{selected.deficiency_count}</span></div>}
              {selected.conditions && <div className="col-span-2"><span className="text-gray-500">Conditions:</span> <span className="text-yellow-700 text-xs">{selected.conditions}</span></div>}
              {selected.accepted_at && <div><span className="text-gray-500">Accepted:</span> <span className="text-green-700">{fmtDate(selected.accepted_at)}</span></div>}
            </div>
