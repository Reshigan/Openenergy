...
const STATUS_COLORS: Record<string, string> = {
  draft:                    'bg-[var(--s2, #eef2f7)] text-[var(--ink-2, #6b7685)]',
      ...
      {kpis && (
        <div className="grid grid-cols-3 gap-3">
          <div className="rounded-lg p-3 border border-[var(--border-subtle, #dde4ec)] bg-surface-v2">
            <div className="text-xs text-[var(--ink-2, #6b7685)]">Open Claims</div>
          ...
          <div className="rounded-lg p-3 border border-green-200 bg-green-50">
            <div className="text-xs text-[var(--ink-2, #6b7685)]">Total Paid</div>
          ...
          <div className="rounded-lg p-3 border border-orange-200 bg-orange-50">
            <div className="text-xs text-[var(--ink-2, #6b7685)]">Outstanding (Certified)</div>
      ...
      {loading ? <div className="text-sm text-[var(--ink-2, #9aa5b4)] py-4">Loading…</div> : (
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {items.map(pc => (
                <tr key={pc.id} className="border-b hover:bg-[var(--s2, #eef2f7)] cursor-pointer" onClick={() => setSelected(pc)}>
                  <td className="py-2 pr-4 font-mono text-xs font-semibold">{pc.cert_number}</td>
                  <td className="py-2 pr-4 text-xs text-[var(--ink-2, #3d4756)]">{TYPE_LABELS[pc.claim_type] ?? pc.claim_type}</td>
                  ...
                  <td className={`py-2 pr-4 text-xs ${pc.sla_breached || pc.chain_status === 'lapsed' ? 'text-red-600 font-semibold' : 'text-[var(--ink-2, #6b7685)]'}`}>
              ...
              {items.length === 0 && (
                <tr><td colSpan={9} className="py-6 text-center text-[var(--ink-2, #9aa5b4)] text-sm">No payment certificates found</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <div className="bg-surface-v2 w-full max-w-xl h-full overflow-y-auto p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <div className="text-xs text-[var(--ink-2, #6b7685)] mt-1">{TYPE_LABELS[selected.claim_type] ?? selected.claim_type} · {selected.value_tier}</div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm mb-4">
              <div><span className="text-[var(--ink-2, #6b7685)]">Claimed:</span> <span className="font-bold">{fmt(selected.claimed_value_zar)}</span></div>
              <div><span className="text-[var(--ink-2, #6b7685)]">Certified:</span> <span className="font-bold text-green-700">{fmt(selected.certified_value_zar)}</span></div>
              <div><span className="text-[var(--ink-2, #6b7685)]">Status:</span> <span className={`px-2 py-0.5 rounded text-xs ${STATUS_COLORS[selected.chain_status]}`}>{selected.chain_status}</span></div>
              <div><span className="text-[var(--ink-2, #6b7685)]">Payment Due:</span> <span className={selected.chain_status === 'lapsed' ? 'text-red-600 font-bold' : 'text-[var(--ink, #2d3748)]'}>{selected.chain_status === 'lapsed' ? '⚠ LAPSED' : fmtDate(selected.payment_due_at)}</span></div>
              {selected.period_from && <div className="col-span-2"><span className="text-[var(--ink-2, #6b7685)]">Period:</span> {fmtDate(selected.period_from)} – {fmtDate(selected.period_to)}</div>}
              <div><span className="text-[var(--ink-2, #6b7685)]">Submitted:</span> {fmtDate(selected.submitted_at)}</div>
              {selected.certified_at && <div><span className="text-[var(--ink-2, #6b7685)]">Certified:</span> {fmtDate(selected.certified_at)}</div>}
