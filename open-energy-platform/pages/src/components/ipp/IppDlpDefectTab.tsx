...
function slaLabel(d: DlpDefect) {
  ...
  return <span className="text-gray-500 text-xs">{Math.round(hrs / 24)}d left</span>;
}
...
export function IppDlpDefectTab() {
  ...
  if (loading) return <div className="p-6 text-gray-400">Loading DLP defects…</div>;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-4 md:grid-cols-8 gap-3">
        {[
          { label: 'Total', value: summary?.total ?? 0, color: 'text-gray-800' },
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100">
            {filtered.map(d => (
              <tr key={d.id} onClick={() => setSelected(d)}
                ...
                <td className="py-2 pr-4">
                  <div className="font-medium text-gray-900">{d.defect_ref ?? d.id.slice(-6)}</div>
                  <div className="text-xs text-gray-400">{fmt(d.identified_at)}</div>
                </td>
                <td className="py-2 pr-4 text-gray-600 text-xs max-w-[120px] truncate">{d.project_name ?? d.project_id}</td>
                <td className="py-2 pr-4">
                  <span className={`px-2 py-0.5 rounded text-xs font-medium ${SEVERITY_COLORS[d.severity_class] ?? 'bg-gray-100 text-gray-700'}`}>
                    {d.severity_class}
                  </span>
                </td>
                <td className="py-2 pr-4 text-gray-700 max-w-[200px] truncate">{d.description}</td>
                <td className="py-2 pr-4">
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={7} className="py-8 text-center text-gray-400">No defects match this filter</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex" onClick={() => setSelected(null)}>
          <div className="ml-auto w-full max-w-lg bg-white shadow-xl border-l border-gray-200 overflow-y-auto"
            ...
            <div className="p-6 space-y-4">
              ...
              {selected.location_description && (
                <div>
                  ...
                  <p className="text-sm text-gray-700">{selected.location_description}</p>
                </div>
              )}

              <div className="grid grid-cols-2 gap-3 text-sm">
                <div><span className="text-xs text-gray-500">Type</span><br />{selected.defect_type ?? '—'}</div>
                <div><span className="text-xs text-gray-500">Contractor</span><br />{selected.responsible_contractor ?? '—'}</div>
                <div><span className="text-xs text-gray-500">SLA ({selected.sla_hours}h)</span><br />{slaLabel(selected)}</div>
                <div><span className="text-xs text-gray-500">DLP Expires</span><br />{fmt(selected.dlp_end_date)}</div>
                {selected.ncr_ref && <div><span className="text-xs text-gray-500">NCR Ref</span><br />{selected.ncr_ref}</div>}
                {selected.ei_ref && <div><span className="text-xs text-gray-500">EI Ref</span><br />{selected.ei_ref}</div>}
                {selected.si_ref && <div><span className="text-xs text-gray-500">SI Ref</span><br />{selected.si_ref}</div>}
              </div>
      ...
      {showCreate && (
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center" onClick={() => setShowCreate(false)}>
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 space-y-4" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-semibold text-gray-900">Log Defect</h3>

            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                <label className="text-xs text-gray-600 block mb-1">Description *</label>
                <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                  className="w-full border border-gray-200 rounded px-3 py-1.5 text-sm" rows={2} />
              </div>
              <div>
              ...
              <div>
                <label className="text-xs text-gray-600 block mb-1">Responsible contractor</label>
