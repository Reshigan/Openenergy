...
const STATUS_COLORS: Record<string, string> = {
  bfs_triggered:             'bg-gray-100 text-gray-500',
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {items.map(item => {
                ...
                const tierColor = TIER_COLORS[item.bfs_capacity_tier] ?? '#6b7280';
                return (
                  <tr key={item.id} className="border-b hover:bg-gray-50">
                    <td className="py-2 pr-4 text-xs font-mono text-gray-600">{item.id.slice(0, 12)}</td>
                    <td className="py-2 pr-4 text-xs text-gray-700">{item.ie_firm_name ?? '—'}</td>
                    <td className="py-2 pr-4 text-xs text-gray-700">{TRIGGER_LABELS[item.trigger_category] ?? item.trigger_category}</td>
                    <td className="py-2 pr-4 text-xs tabular-nums text-gray-700">{item.capacity_mw.toFixed(1)}</td>
                    <td className="py-2 pr-4 text-xs tabular-nums text-gray-700">{item.p50_yield_gwh != null ? item.p50_yield_gwh.toFixed(1) : '—'}</td>
              ...
              {items.length === 0 && (
                <tr><td colSpan={9} className="py-10 text-center text-gray-400 text-sm">No BFS studies found</td></tr>
              )}
            </tbody>
          </table>
        </div>
