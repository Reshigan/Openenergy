        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {items.map(item => {
                ...
                return (
                  <tr key={item.id} className="border-b hover:bg-[#eef2f7]">
                    ...
                    <td className="py-2 pr-4 text-xs text-[#2d3748]">{item.ie_firm_name ?? '—'}</td>
                    <td className="py-2 pr-4 text-xs text-[#2d3748]">{TRIGGER_LABELS[item.trigger_category] ?? item.trigger_category}</td>
                    <td className="py-2 pr-4 text-xs tabular-nums text-[#2d3748]">{item.capacity_mw.toFixed(1)}</td>
                    <td className="py-2 pr-4 text-xs tabular-nums text-[#2d3748]">{item.p50_yield_gwh != null ? item.p50_yield_gwh.toFixed(1) : '—'}</td>
              ...
              {items.length === 0 && (
                <tr><td colSpan={9} className="py-10 text-center text-[#9aa5b4] text-sm">No BFS studies found</td></tr>
              )}
            </tbody>
          </table>
        </div>
