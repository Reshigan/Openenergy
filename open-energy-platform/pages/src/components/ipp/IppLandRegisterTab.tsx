...
const STATUS_COLORS: Record<string, string> = {
  survey_commissioned:   'bg-[#eef2f7] text-[#6b7685]',
  ...
  superseded:            'bg-[#e8ecf0] text-[#6b7685]',
};
      ...
      {loading ? <div className="text-sm text-[#9aa5b4] py-4">Loading…</div> : (
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {items.map(lr => (
                <tr key={lr.id} className="border-b hover:bg-[#eef2f7] cursor-pointer" onClick={() => setSelected(lr)}>
              ...
              {items.length === 0 && <tr><td colSpan={9} className="py-6 text-center text-[#9aa5b4] text-sm">No land register records found</td></tr>}
            </tbody>
          </table>
        </div>
      )}
          ...
          <div className="bg-white w-full max-w-xl h-full overflow-y-auto p-6 shadow-xl" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <div className="text-xs text-[#6b7685] mt-1">{selected.area_tier} · {selected.area_ha} ha</div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3 text-sm mb-4">
              ...
              {selected.deeds_reference && <div><span className="text-[#6b7685]">Deeds Ref:</span> <span className="font-mono text-green-700">{selected.deeds_reference}</span></div>}
              {selected.deeds_registered_at && <div><span className="text-[#6b7685]">Registered:</span> <span className="text-green-700">{fmtDate(selected.deeds_registered_at)}</span></div>}
            </div>
