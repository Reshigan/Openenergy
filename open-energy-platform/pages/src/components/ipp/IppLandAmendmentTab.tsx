...
const STATUS_COLORS: Record<string, string> = {
  amendment_requested:  'bg-gray-100 text-gray-500',
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {items.map(item => {
                ...
                const tierColor = TIER_COLORS[item.area_tier] ?? '#6b7280';
                return (
                  <tr key={item.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-2 pr-4 text-xs text-gray-700">{CATEGORY_LABELS[item.amendment_category] ?? item.amendment_category}</td>
                    <td className="py-2 pr-4 text-xs tabular-nums text-gray-700">{item.land_area_hectares.toLocaleString('en-ZA')}</td>
              ...
              {items.length === 0 && (
                <tr><td colSpan={9} className="py-10 text-center text-gray-400 text-sm">No land amendments found</td></tr>
              )}
            </tbody>
          </table>
        </div>
