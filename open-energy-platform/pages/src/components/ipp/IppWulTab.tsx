...
interface WulApplication {
  ...
  section21_category:
    ...
    | 's21_g_discharge'
    | 's21_h_disposal';
  wul_capacity_tier: 'small' | 'medium' | 'large' | 'utility' | 'strategic';
  chain_status: string;
...
const STATUS_COLORS: Record<string, string> = {
  wul_application_triggered:   'bg-gray-100 text-gray-500',
  ...
  dws_final_review:             'bg-amber-100 text-amber-700',
  wul_granted:                  'bg-green-100 text-green-700',
  wul_refused:                  'bg-red-100 text-red-700',
  wul_lapsed:                   'bg-orange-100 text-orange-700',
};

const STATUS_LABELS: Record<string, string> = {
  wul_application_triggered:   'WUL Application Triggered',
  site_assessment:              'Site Assessment',
  application_preparation:      'Application Preparation',
  application_submitted:        'Application Submitted',
  dws_completeness_review:      'DWS Completeness Review',
  public_participation_open:    'Public Participation Open',
  public_participation_closed:  'Public Participation Closed',
  technical_assessment:         'Technical Assessment',
  dws_final_review:             'DWS Final Review',
  wul_granted:                  'WUL Granted',
  wul_refused:                  'WUL Refused',
  wul_lapsed:                   'WUL Lapsed',
};
...
const SECTION21_LABELS: Record<string, string> = {
  s21_a_diversion:     'S21(a) Diversion',
  s21_b_storage:       'S21(b) Storage',
  s21_c_impeding_flow: 'S21(c) Impeding Flow',
  s21_g_discharge:     'S21(g) Discharge',
  s21_h_disposal:      'S21(h) Disposal',
};

const TERMINAL_STATUSES = new Set(['wul_granted', 'wul_refused', 'wul_lapsed']);
...
function hasRegulatorFlag(row: WulApplication): boolean {
  if (row.chain_status === 'wul_refused') return true;
  if (
    row.chain_status === 'wul_lapsed' &&
    (row.wul_capacity_tier === 'utility' || row.wul_capacity_tier === 'strategic')
  ) return true;
  if (
    row.chain_status === 'wul_granted' &&
    (row.wul_capacity_tier === 'utility' || row.wul_capacity_tier === 'strategic')
  ) return true;
  return false;
}
...
export function IppWulTab() {
  const [items, setItems]               = useState<WulApplication[]>([]);
  ...
  async function load(
    ...
    try {
      ...
      const res = await fetch(`/api/ipp-wul?${params}`, {
  ...
  const lapsed   = items.filter(i => i.chain_status === 'wul_lapsed').length;
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KpiChip label="Total Applications" value={total} />
          ...
          <table className="min-w-full text-sm">
            <thead>
              <tr className="border-b text-left text-xs text-gray-500">
                ...
                <th className="pb-2 pr-4">Section 21 Category</th>
                <th className="pb-2 pr-4">Capacity (MW)</th>
                <th className="pb-2 pr-4">Status</th>
                <th className="pb-2 pr-4">DWS Reference</th>
                <th className="pb-2 pr-4">Water Consultant</th>
              ...
              {pageItems.map(item => {
                ...
                return (
                  <tr key={item.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="py-2 pr-4 text-xs text-gray-700">{item.dws_reference ?? '—'}</td>
                    <td className="py-2 pr-4 text-xs text-gray-700">{item.water_consultant ?? '—'}</td>
