...
export function OnboardingTour({ scope, steps }: Props) {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 max-w-sm widget-card shadow-lg border border-[var(--border-subtle, #dde3ee)] bg-surface-v2 p-4">
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1">
          <div className="text-[11px] uppercase tracking-wider text-[var(--ink-2, #5b6b85)]">Quick tour · {cursor + 1} of {remaining.length}</div>
        ...
        <button type="button" onClick={() => setSkip(true)} aria-label="Skip tour" className="text-[var(--ink-2, #5b6b85)] hover:text-[#0e1726] flex-none">
          <X size={14}/>
        </button>
      </div>
      <div className="flex justify-end mt-3 gap-2">
        <button type="button" onClick={() => setSkip(true)} className="h-8 px-3 text-[11px] text-[var(--ink-2, #5b6b85)] hover:text-[#0e1726]">Skip</button>
