...
export function RegulatorAiHub() {
  ...
  return (
    <section className="rounded-xl border bg-white overflow-hidden" style={{ borderColor: '#dde4ec' }}>
      <header className="px-5 py-4 border-b flex items-center gap-3" style={{ borderColor: '#e5ebf2' }}>
        <div className="w-9 h-9 rounded-lg flex items-center justify-center text-white"
             style={{ background: 'linear-gradient(135deg,#1a8a5b 0%,#3b82c4 100%)' }}>
          <FileBarChart size={18} />
        </div>
        <div className="leading-tight">
          <h2 className="text-[15px] font-semibold" style={{ color: '#0f1c2e' }}>Regulator compliance copilot</h2>
      ...
      <div className="p-5 space-y-3">
        <div className="flex flex-wrap gap-3 items-end">
          <label className="flex flex-col gap-1">
            <span className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: '#6b7685' }}>Filing type</span>
            <select
              value={filingType}
              onChange={(e) => setFilingType(e.target.value as FilingType)}
              className="h-9 px-3 rounded-md border text-[13px]"
              style={{ borderColor: '#d5d6d8', color: '#0f1c2e' }}
            >
          ...
          <label className="flex flex-col gap-1">
            <span className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: '#6b7685' }}>Period</span>
            <input
              value={period}
        ...
        {error && (
          <div className="text-[13px] rounded-md px-3 py-2" style={{ background: '#fde0db', color: '#c0392b' }}>{error}</div>
        )}
        {result && (
