...
const MONITORING_TIER_COLOR: Record<MonitoringTier, string> = {
  ...
  baseline: 'bg-[var(--s2, #eef2f7)] text-[var(--ink-2, #3d4756)]',
};
...
const STATUS_COLOR: Record<EnvMonitoringStatus, string> = {
  scheduled: 'bg-[var(--s2, #eef2f7)] text-[var(--ink, #2d3748)]',
  sampling: 'rounded',
  ...
  cancelled: 'bg-[var(--border-subtle, #e8ecf0)] text-[var(--ink-2, #9aa5b4)]',
};
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="text-xs text-[var(--ink-2, #9aa5b4)] ml-auto">{filtered.length} monitoring records</span>
        {!readOnly && (
      ...
      {loading && <div className="text-xs text-[var(--ink-2, #9aa5b4)]">Loading environmental monitoring register…</div>}
        ...
        <div className="overflow-x-auto rounded-lg border border-[var(--border-subtle, #dde4ec)]">
          <table className="w-full text-xs">
            <thead className="bg-[var(--s1, #f8fafc)]">
              <tr>
                <th className="text-left px-3 py-2 font-medium text-[var(--ink-2, #6b7685)]">Ref</th>
                ...
                <th className="text-left px-3 py-2 font-medium text-[var(--ink-2, #6b7685)]">Measurement</th>
            ...
            <tbody>
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={readOnly ? 8 : 9} className="px-3 py-6 text-center text-[var(--ink-2, #9aa5b4)]">
              ...
              {filtered.map(row => {
                ...
                return (
                  <tr
                    key={row.id}
                    className={`border-t border-[var(--s2, #eef2f7)] hover:bg-[var(--s2, #eef2f7)] cursor-pointer ${exceedance ? 'bg-red-50/40' : ''}`}
                    onClick={() => setSelected(row)}
                  >
                    <td className="px-3 py-2 font-mono text-[var(--ink-2, #9aa5b4)]">{row.monitoring_ref ?? row.id}</td>
                    <td className="px-3 py-2 max-w-[200px]">
                      <span className="text-[var(--ink, #1e2a38)] block truncate">{row.monitoring_title}</span>
                      ...
                      {row.measured_value != null ? (
                        <span className={exceedance ? 'text-red-700 font-semibold' : ''}>
                          ...
                          {row.permit_limit_max != null && (
                            <span className="text-[var(--ink-2, #9aa5b4)]"> / {row.permit_limit_max}</span>
                          )}
                        ...
                        <span className="text-[var(--ink-2, #9aa5b4)]">{row.parameter_name ?? '—'}</span>
                      )}
                    </td>
                    <td className="px-3 py-2">
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <h3 className="font-semibold text-[var(--ink, #0f1c2e)] text-sm">{selected.monitoring_title}</h3>
            ...
            {selected.measured_value != null && (
              <div className="mb-4 p-3 rounded-lg bg-[var(--s1, #f8fafc)] border border-[var(--border-subtle, #dde4ec)]">
                <p className="text-xs font-semibold text-[var(--ink, #0f1c2e)] mb-2">Measurement</p>
                <div className="grid grid-cols-3 gap-3 text-xs">
                  <div>
                    <span className="text-[var(--ink-2, #6b7685)]">Parameter</span>
                    <p className="font-medium">{selected.parameter_name ?? '—'}</p>
                  </div>
                  <div>
                    <span className="text-[var(--ink-2, #6b7685)]">Measured</span>
                  ...
                  <div>
                    <span className="text-[var(--ink-2, #6b7685)]">Limit (max)</span>
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-2 text-xs mb-4">
              <div>
                <span className="text-[var(--ink-2, #6b7685)]">EIA condition ref</span>
                <p className="font-medium text-[var(--ink, #1e2a38)]">{selected.eia_condition_ref ?? '—'}</p>
              </div>
              <div>
                <span className="text-[var(--ink-2, #6b7685)]">Sampling location</span>
                <p className="font-medium text-[var(--ink, #1e2a38)]">{selected.sampling_location ?? '—'}</p>
              </div>
              <div>
                <span className="text-[var(--ink-2, #6b7685)]">Frequency</span>
                <p className="font-medium text-[var(--ink, #1e2a38)]">{selected.monitoring_frequency ?? '—'}</p>
              </div>
              <div>
              ...
              {selected.lab_sample_ref && (
                <div>
                  <span className="text-[var(--ink-2, #6b7685)]">Lab sample ref</span>
                  <p className="font-medium text-[var(--ink, #1e2a38)]">{selected.lab_sample_ref}</p>
                </div>
              )}
              {selected.corrective_action_deadline && (
                <div>
                  <span className="text-[var(--ink-2, #6b7685)]">Corrective action deadline</span>
                  <p className="font-medium text-orange-700">{selected.corrective_action_deadline}</p>
                </div>
              )}
              {selected.sla_breach_count > 0 && (
                <div>
                  <span className="text-[var(--ink-2, #6b7685)]">SLA breach count</span>
                  <p className="font-medium text-red-800">{selected.sla_breach_count}</p>
                </div>
              )}
              {selected.regulator_ref && (
                <div>
                  <span className="text-[var(--ink-2, #6b7685)]">Regulator ref</span>
                  <p className="font-medium text-[var(--ink, #1e2a38)]">{selected.regulator_ref}</p>
                </div>
              )}
            </div>
            ...
            {selected.findings && (
              <div className="mb-4">
                <p className="text-xs text-[var(--ink-2, #6b7685)] mb-1">Findings</p>
                <p className="text-xs text-[var(--ink, #1e2a38)] whitespace-pre-wrap">{selected.findings}</p>
              </div>
            )}
            ...
            {selected.corrective_actions && (
              <div className="mb-4">
                <p className="text-xs text-[var(--ink-2, #6b7685)] mb-1">Corrective actions</p>
                <p className="text-xs text-[var(--ink, #1e2a38)] whitespace-pre-wrap">{selected.corrective_actions}</p>
              </div>
            )}
            ...
            {selected.report_title && (
              <div className="mb-4">
                <p className="text-xs text-[var(--ink-2, #6b7685)] mb-1">Report</p>
                <p className="text-xs text-[var(--ink, #1e2a38)]">{selected.report_title}</p>
                {selected.report_submitted_to && <p className="text-xs text-[var(--ink-2, #3d4756)] mt-0.5">Submitted to: {selected.report_submitted_to}</p>}
              </div>
            )}
      ...
      {showCreate && !readOnly && (
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold text-[var(--ink, #0f1c2e)]">Schedule environmental monitoring</h3>
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-[var(--ink-2, #6b7685)] block mb-1">Monitoring title *</label>
                ...
                <div>
                  <label className="text-xs text-[var(--ink-2, #6b7685)] block mb-1">Monitoring ref</label>
                ...
                <div>
                  <label className="text-xs text-[var(--ink-2, #6b7685)] block mb-1">Monitoring tier *</label>
                ...
                <div>
                  <label className="text-xs text-[var(--ink-2, #6b7685)] block mb-1">EIA condition ref</label>
                ...
                <div>
                  <label className="text-xs text-[var(--ink-2, #6b7685)] block mb-1">Sampling location</label>
                ...
                <div>
                  <label className="text-xs text-[var(--ink-2, #6b7685)] block mb-1">Monitoring frequency</label>
                ...
                <div>
                  <label className="text-xs text-[var(--ink-2, #6b7685)] block mb-1">Parameter name</label>
                ...
                <div>
                  <label className="text-xs text-[var(--ink-2, #6b7685)] block mb-1">Permit limit (max)</label>
                ...
                <div>
                  <label className="text-xs text-[var(--ink-2, #6b7685)] block mb-1">Lab name</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-xs text-[var(--ink-2, #6b7685)] block mb-1">HSE incident ref</label>
                ...
                <div>
                  <label className="text-xs text-[var(--ink-2, #6b7685)] block mb-1">Method statement ref</label>
                ...
                <div>
                  <label className="text-xs text-[var(--ink-2, #6b7685)] block mb-1">Stage gate ref</label>
