...
const STATE_TONE: Record<ChainStatus, { bg: string; fg: string; label: string }> = {
  ...
  assessment_review: { bg: '#dbecfb', fg: '#1a3a5c', label: 'Assessment review' },
...
const TIER_TONE: Record<Tier, { bg: string; fg: string; label: string }> = {
  ...
  small:  { bg: '#dbecfb', fg: '#1a3a5c', label: 'Small (<R1m)' },
          ...
          <table className="w-full text-[12px]">
            <thead className="bg-[#f3f5f9]">
              <tr className="text-left">
                <th className="px-3 py-2 font-semibold text-[#1a3a5c]">Levy #</th>
                <th className="px-3 py-2 font-semibold text-[#1a3a5c]">Licensee</th>
                <th className="px-3 py-2 font-semibold text-[#1a3a5c]">Sector</th>
                <th className="px-3 py-2 font-semibold text-[#1a3a5c]">Tier</th>
                <th className="px-3 py-2 font-semibold text-[#1a3a5c] text-right">Assessed</th>
              ...
              {filtered.map((r) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-right tabular-nums text-[#1a3a5c]">{fmtZar(r.assessed_amount)}</td>
    ...
    <div className="fixed inset-0 z-30 bg-black/40" onClick={onClose}>
      ...
      >
        ...
        <section className="px-5 py-4 border-b border-[#e3e7ec]">
          ...
          {row.invoice_basis && (
            <BasisBlock label="Invoice basis" tone="#1a3a5c" text={row.invoice_basis} />
          )}
          {row.objection_basis && (
            <BasisBlock label="Objection basis (licensee)" tone="#a06200" text={row.objection_basis} />
          )}
          {row.payable_basis && (
            <BasisBlock label="Payable basis" tone="#1a3a5c" text={row.payable_basis} />
          )}
          {row.payment_basis && (
