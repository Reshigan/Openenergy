...
export function PasskeysPage() {
  ...
  return (
    ...
    >
      <div className="p-4 mb-4" style={{ background: 'oklch(0.99 0.002 80)', border: '1px solid oklch(0.87 0.006 250)', borderRadius: '12px' }}>
        <div className="text-[13px] font-semibold mb-1" style={{ color: 'oklch(0.17 0.010 250)' }}>Add a new passkey</div>
        <p className="text-[12px] mb-2" style={{ color: 'oklch(0.40 0.009 250)' }}>
        ...
        {!supported && <div className="text-[12px] mb-2" style={{ color: 'oklch(0.48 0.20 20)' }}>Your browser doesn't support WebAuthn.</div>}
        ...
        {err && <div className="text-[12px] mt-2 inline-flex items-center gap-1" style={{ color: 'oklch(0.48 0.20 20)' }}><AlertCircle size={13} className="inline mr-1"/>{err}</div>}
        {ack && <div className="text-[12px] mt-2 inline-flex items-center gap-1" style={{ color: 'oklch(0.45 0.15 150)' }}><CheckCircle2 size={13} className="inline mr-1"/>{ack}</div>}
      </div>

      <div style={{ background: 'oklch(0.99 0.002 80)', border: '1px solid oklch(0.87 0.006 250)', borderRadius: '12px', overflow: 'hidden' }}>
        <header className="px-4 py-3 flex items-center" style={{ borderBottom: '1px solid oklch(0.87 0.006 250)', background: 'oklch(0.96 0.003 250)' }}>
          <div className="text-[13px] font-semibold" style={{ color: 'oklch(0.17 0.010 250)' }}>Your passkeys</div>
          <button type="button" onClick={load} className="ml-auto h-8 px-2 rounded text-[11px] inline-flex items-center gap-1" style={{ border: '1px solid oklch(0.87 0.006 250)', color: 'oklch(0.40 0.009 250)', background: 'oklch(0.99 0.002 80)' }}><RefreshCw size={11}/>Refresh</button>
        </header>
        <ul>
          {creds.map((c) => (
            <li key={c.id} className="px-4 py-3 flex items-center gap-3" style={{ borderTop: '1px solid oklch(0.91 0.005 250)' }}>
              <Fingerprint size={16} style={{ color: c.revoked_at ? 'oklch(0.60 0.007 250)' : 'oklch(0.46 0.16 55)' }}/>
              <div className="flex-1">
                <div className="font-semibold text-[13px]" style={{ color: 'oklch(0.17 0.010 250)' }}>{c.device_name}</div>
                <div className="text-[11px]" style={{ color: 'oklch(0.60 0.007 250)' }}>
              ...
              {!c.revoked_at && (
                <button type="button" onClick={() => revoke(c.id)} className="text-[11px] underline inline-flex items-center gap-1" style={{ color: 'oklch(0.48 0.20 20)' }}>
                  <X size={11}/> Revoke
                </button>
              )}
            </li>
          ))}
          {creds.length === 0 && <li className="px-4 py-6 text-center text-[12px]" style={{ color: 'oklch(0.60 0.007 250)' }}>No passkeys enrolled yet.</li>}
        </ul>
      </div>
    </StitchPage>
  );
