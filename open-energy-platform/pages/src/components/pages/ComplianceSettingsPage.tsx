    ...
    <div className="mt-3 space-y-3">
      <section className="widget-card p-4">
        <div className="flex items-center gap-2">
          <Lock size={16} style={{ color: 'oklch(0.46 0.16 55)' }} />
          <span className="text-[14px] font-semibold" style={{ color: 'oklch(0.17 0.010 250)' }}>Two-factor authentication</span>
          {state?.verified
        ...
        <p className="text-[12px] mt-2" style={{ color: 'oklch(0.40 0.009 250)' }}>
        ...
        {otpauth && secret && (
          <div className="mt-4 grid grid-cols-1 md:grid-cols-[200px_1fr] gap-4 items-start">
            ...
            <div className="space-y-2 text-[12px]">
              <div>
                <div className="text-[11px] uppercase tracking-wider font-bold" style={{ color: 'oklch(0.40 0.009 250)' }}>1. Scan with your authenticator app</div>
                <div className="text-[11px] mt-1" style={{ color: 'oklch(0.40 0.009 250)' }}>Or paste this secret manually:</div>
                <code className="block mt-1 font-mono text-[11px] px-2 py-1 rounded break-all"
                      style={{ background: 'oklch(0.96 0.003 250)', border: '1px solid oklch(0.87 0.006 250)' }}>{secret}</code>
              </div>
              <div>
                <div className="text-[11px] uppercase tracking-wider font-bold" style={{ color: 'oklch(0.40 0.009 250)' }}>2. Enter the 6-digit code</div>
              ...
              {error && <div className="text-[11px]" style={{ color: 'oklch(0.48 0.20 20)' }}>{error}</div>}
            </div>
          </div>
        )}

        {recoveryCodes && (
          <div className="mt-4 rounded-lg p-3" style={{ border: '1px solid oklch(0.45 0.15 150)', background: 'oklch(0.97 0.04 150)' }}>
            ...
            <p className="text-[11px] mt-1" style={{ color: 'oklch(0.40 0.009 250)' }}>
        ...
        {state?.verified && !otpauth && (
          <div className="mt-3 space-y-2 text-[12px]" style={{ color: 'oklch(0.40 0.009 250)' }}>
            ...
            <button type="button" onClick={reset} className="h-8 px-3 rounded text-[12px] font-semibold inline-flex items-center gap-1"
                    style={{ background: 'oklch(0.99 0.002 80)', border: '1px solid oklch(0.48 0.20 20)', color: 'oklch(0.48 0.20 20)' }}>
        ...
        <section className="widget-card">
          ...
          <div className="p-3 grid grid-cols-1 md:grid-cols-3 gap-2 items-center">
            ...
            <label className="h-9 px-3 rounded text-[12px] inline-flex items-center justify-center gap-1 cursor-pointer"
                   style={{ border: '1px dashed oklch(0.87 0.006 250)' }}>
            ...
            {busy && <span className="text-[11px] inline-flex items-center gap-1" style={{ color: 'oklch(0.40 0.009 250)' }}><Loader2 size={11} className="animate-spin" /> Uploading…</span>}
          </div>
        </section>
      )}
      <section className="widget-card">
        ...
        <div className="p-3 overflow-x-auto">
          <table className="w-full text-[12px]">
            ...
            <tbody>
                ...
                <tr key={r.id}>
                  ...
                  {adminView && (
                    <td className="text-right space-x-2">
                      {r.status === 'pending' && (
                        <>
                          <button type="button" onClick={() => decide(r.id, 'approved')} className="text-[11px] font-semibold" style={{ color: 'oklch(0.45 0.15 150)' }}>Approve</button>
                          <button type="button" onClick={() => decide(r.id, 'rejected')} className="text-[11px] font-semibold" style={{ color: 'oklch(0.48 0.20 20)' }}>Reject</button>
                        </>
                      )}
                    </td>
                  )}
              ...
              {!rows.length && <tr><td colSpan={adminView ? 6 : 5} className="italic py-3" style={{ color: 'oklch(0.40 0.009 250)' }}>No submissions.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>
  ...
  return (
    <div className="mt-3 space-y-3">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <section className="widget-card p-4">
          <div className="flex items-center gap-2">
            <Download size={16} style={{ color: 'oklch(0.46 0.16 55)' }} />
            <span className="text-[14px] font-semibold" style={{ color: 'oklch(0.17 0.010 250)' }}>Section 23 — Right of access</span>
          </div>
          <p className="text-[12px] mt-2" style={{ color: 'oklch(0.40 0.009 250)' }}>Download all personal information we hold about you in a portable JSON archive. Ready within seconds.</p>
          <button type="button" onClick={requestExport} disabled={busy}
                  className="mt-3 h-9 px-3 rounded text-white text-[12px] font-semibold disabled:opacity-50"
                  style={{ background: 'oklch(0.46 0.16 55)' }}>
            {busy ? 'Generating…' : 'Request data export'}
          </button>
          <ul className="mt-3 text-[11px]" style={{ borderTop: '1px solid oklch(0.87 0.006 250)' }}>
            {data.exports.map((e) => (
              <li key={e.id} className="py-2 flex items-center gap-2" style={{ borderBottom: '1px solid oklch(0.87 0.006 250)' }}>
                ...
                {e.status === 'ready' && (
                  <button type="button" onClick={() => download(e.id)} className="ml-auto" style={{ color: 'oklch(0.46 0.16 55)' }}>Download</button>
                )}
              </li>
            ))}
            {!data.exports.length && <li className="py-2 italic" style={{ color: 'oklch(0.40 0.009 250)' }}>No requests yet.</li>}
          </ul>
        </section>
        <section className="widget-card p-4">
          <div className="flex items-center gap-2">
            <Trash2 size={16} style={{ color: 'oklch(0.48 0.20 20)' }} />
            <span className="text-[14px] font-semibold" style={{ color: 'oklch(0.17 0.010 250)' }}>Section 24 — Right to erasure</span>
          </div>
          <p className="text-[12px] mt-2" style={{ color: 'oklch(0.40 0.009 250)' }}>Request deletion of your account. 30-day cooling-off before execution. Audit chains are preserved per POPIA s.14(2)(d) (legal obligation).</p>
              ...
              <li key={d.id} className="py-2 flex items-center gap-2" style={{ borderBottom: '1px solid oklch(0.87 0.006 250)' }}>
                ...
                {d.status === 'cooling_off' && (
                  <>
                    ...
                    <button type="button" onClick={() => cancelErasure(d.id)} className="ml-auto" style={{ color: 'oklch(0.46 0.16 55)' }}>Cancel</button>
                  </>
                )}
              </li>
            ))}
            {!data.deletions.length && <li className="py-2 italic" style={{ color: 'oklch(0.40 0.009 250)' }}>No requests yet.</li>}
          </ul>
        </section>
      </div>
    </div>
        ...
        <section className="widget-card">
          ...
          <div className="p-3 flex gap-2 items-center">
            ...
            <button type="button" onClick={generateNersa} className="h-9 px-3 rounded text-white text-[12px] font-semibold"
                    style={{ background: 'oklch(0.46 0.16 55)' }}>Generate</button>
          </div>
          <ul className="text-[11px]" style={{ borderTop: '1px solid oklch(0.87 0.006 250)' }}>
            {nersa.map((r) => (
              <li key={r.id} className="px-3 py-2 flex items-center gap-2" style={{ borderBottom: '1px solid oklch(0.87 0.006 250)' }}>
                ...
                <span className="ml-auto font-mono" style={{ color: 'oklch(0.40 0.009 250)' }}>{r.r2_key}</span>
              </li>
            ))}
            {!nersa.length && <li className="px-3 py-2 italic" style={{ color: 'oklch(0.40 0.009 250)' }}>No quarterly returns yet.</li>}
          </ul>
        </section>
        <section className="widget-card">
          <header className="widget-card-header">
          ...
          <ul className="text-[11px]" style={{ borderTop: '1px solid oklch(0.87 0.006 250)' }}>
            ...
            {!sars.length && <li className="px-3 py-2 italic" style={{ color: 'oklch(0.40 0.009 250)' }}>No SARS packs yet.</li>}
          </ul>
        </section>
      </div>
    </div>
