    ...
    <div className="mt-3 space-y-3">
      <Section title="New algo execution">
        <div className="p-3 grid grid-cols-2 md:grid-cols-6 gap-2 items-end">
          <select value={draft.algo_type} onChange={(e) => setDraft({ ...draft, algo_type: e.target.value })} className="h-9 px-2 rounded border text-[12px]" style={{ borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' }}>
          ...
          <select value={draft.energy_type} onChange={(e) => setDraft({ ...draft, energy_type: e.target.value })} className="h-9 px-2 rounded border text-[12px]" style={{ borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' }}>
          ...
          <select value={draft.side} onChange={(e) => setDraft({ ...draft, side: e.target.value })} className="h-9 px-2 rounded border text-[12px]" style={{ borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' }}>
            <option value="buy">Buy</option><option value="sell">Sell</option>
          </select>
          <input type="number" placeholder="Volume MWh" value={draft.total_volume_mwh} onChange={(e) => setDraft({ ...draft, total_volume_mwh: Number(e.target.value) })} className="h-9 px-2 rounded border text-[12px] font-mono" style={{ borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' }} />
          <input type="datetime-local" value={draft.start_at} onChange={(e) => setDraft({ ...draft, start_at: e.target.value })} className="h-9 px-2 rounded border text-[12px]" style={{ borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' }} />
          <input type="datetime-local" value={draft.end_at}   onChange={(e) => setDraft({ ...draft, end_at: e.target.value })}   className="h-9 px-2 rounded border text-[12px]" style={{ borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' }} />
        </div>
        <div className="px-3 pb-3"><button type="button" onClick={submit} disabled={submitting} className="h-9 px-3 rounded text-white text-[12px] font-semibold disabled:opacity-50" style={{ background: 'var(--accent, oklch(0.46 0.16 55))' }}>{submitting ? 'Submitting…' : 'Submit algo'}</button></div>
      </Section>
          ...
          {rows.map((r) => (
            <tr key={r.id}>
              ...
              <td className="text-right space-x-2">
                ...
                {!['completed','cancelled'].includes(r.status) && <button type="button" onClick={async () => { await api.post(`/trading-deep/algos/${r.id}/cancel`, {}); void load(); }} className="text-[11px]" style={{ color: 'var(--bad, oklch(0.48 0.20 20))' }}>Cancel</button>}
              </td>
            </tr>
          ))}
        ...
        {rows.map((r) => (
          <tr key={r.id}>
            ...
            <td className="text-right space-x-1">
              {r.status === 'open'           && <button type="button" onClick={() => action(r.id, 'net')}     className="text-[11px]" style={{ color: 'var(--accent, oklch(0.46 0.16 55))' }}>Net</button>}
              {r.status === 'net_calculated' && <button type="button" onClick={() => action(r.id, 'novate')}  className="text-[11px]" style={{ color: 'var(--accent, oklch(0.46 0.16 55))' }}>Novate</button>}
        ...
        {rows.map((r) => (
          <React.Fragment key={r.id}>
            <tr>
              ...
              <td><Pill status={r.status} /></td>
              <td className="text-right"><button type="button" onClick={() => open(r.id)} className="text-[11px]" style={{ color: 'var(--accent, oklch(0.46 0.16 55))' }}>{expanded === r.id ? 'Hide CPs' : 'View CPs'}</button></td>
            </tr>
            {expanded === r.id && (
              <tr><td colSpan={5} className="px-3 py-2" style={{ background: 'var(--s1, oklch(0.96 0.003 250))' }}>
                <ul className="space-y-1 text-[11px]">
                  {cps.map((cp) => (
                    <li key={cp.id} className="flex items-center gap-2">
                      <Pill status={cp.status} />
                      <span className="font-mono">{cp.cp_type}</span>
                      <span style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>{cp.description}</span>
                    </li>
                  ))}
                </ul>
              </td></tr>
        ...
        {rows.map((r) => {
          ...
          return (
            <tr key={r.id}>
              ...
              <td><Pill status={r.status} /></td>
              <td className="text-right">{r.status === 'accruing' && <button type="button" onClick={() => accrue(r.id)} className="text-[11px]" style={{ color: 'var(--accent, oklch(0.46 0.16 55))' }}>Accrue now</button>}</td>
            </tr>
          );
        })}
