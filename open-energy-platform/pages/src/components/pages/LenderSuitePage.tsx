...
export function LenderSuitePage() {
  const tabs: TabSpec[] = [
    ...
    },
    {
      ...
      endpoint: '',
      description: 'DSCR monitoring & cure chain · LMA covenant schedule + SARB IFRS 9 Stage 2/3 trigger + Basel III. The rolling MONITOR counterpart to the periodic covenant CERTIFICATE. Tests DSCR/LLCR/PLCR each test date and routes the loan through clean → watch → breach → lock-up → cure (propose/execute/validate) → acceleration (loan-default pickup) or waiver. URGENT tier SLA — the LOWER the current DSCR the TIGHTER every window; tier is RE-DERIVED on every transition. Live coverage-defense battery on every record (severity index, headroom to lock-up, cure runway, equity-cure/DSRA coverage, forward DSCR, LLCR, PLCR, cross-default flag, urgency). declare_acceleration crosses regulator at every tier (IFRS 9 Stage 3 trigger), waive_breach + enter_lock_up + SLA breach for material+severe only.',
      columns: [],
