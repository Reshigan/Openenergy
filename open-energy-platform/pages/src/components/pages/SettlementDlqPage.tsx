  ...
  return (
    <div className="p-6 lg:p-10 space-y-4 min-h-screen" style={{ background: 'var(--oe-surface)' }}>
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="inline-flex items-center gap-2 text-[11px] uppercase tracking-wider text-[var(--ink-2, #6b7685)] bg-surface-v2 border border-[var(--border-subtle, #dde4ec)] rounded-full px-3 py-1">
            Settlement · DLQ
          </div>
          ...
          <p className="text-[13px] text-[var(--ink-2, #3d4756)]">Dead-letter queue for failed settlement runs. Click 'Explain' to get a deterministic cause + suggested action; accept logs the decision for audit.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={() => navigate('/admin/monitoring')} className="h-9 px-3 rounded-md border border-[var(--border-subtle, #dde4ec)] bg-surface-v2 text-[var(--ink-2, #3d4756)] text-[12px] font-semibold inline-flex items-center gap-1">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        <span className="text-[12px] text-[var(--ink-2, #6b7685)]">Status:</span>
        {(['open', 'retrying', 'resolved', 'abandoned'] as const).map(s => (
          <button type="button" key={s} onClick={() => setStatus(s)} className={`px-3 py-1 rounded-full text-[11px] capitalize ${status === s ? 'bg-[#c2873a] text-white' : 'bg-surface-v2 border border-[var(--border-subtle, #dde4ec)] text-[var(--ink-2, #3d4756)]'}`}>
            {s}
          </button>
        ))}
      </div>
            ...
            <tbody>
              {rows.map(r => (
                <React.Fragment key={r.id}>
                  <tr className="border-t border-[var(--border-subtle, #e5ebf2)] hover:bg-[var(--s1, #f8fafc)]">
                    ...
                    <td className="px-4 py-2 text-[11px] text-[var(--ink-2, #6b7685)]">{new Date(r.created_at).toLocaleString()}</td>
                    <td className="px-4 py-2">
                      {!explanations[r.id] && r.status !== 'resolved' && (
                        <button type="button" onClick={() => explain(r)} disabled={explaining === r.id} className="px-2 py-1 text-[11px] bg-amber-50 text-amber-800 rounded inline-flex items-center gap-1">
                      ...
                      <td colSpan={7} className="px-4 py-2 bg-amber-50/50 border-t border-amber-200/60">
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="text-[12px] font-semibold text-[var(--ink, #0f1c2e)]">
                              AI explanation
                              <span className="ml-2 text-[10px] text-[var(--ink-2, #6b7685)]">
                                confidence {(explanations[r.id].confidence * 100).toFixed(0)}% · source {explanations[r.id].source}
                              </span>
                            </div>
                            <p className="mt-1 text-[12px] text-[var(--ink-2, #3d4756)]">{explanations[r.id].explanation}</p>
                            <div className="mt-1 text-[12px] text-[var(--ink-2, #3d4756)]"><strong>Suggested:</strong> {explanations[r.id].suggested_action}</div>
                            <div className="mt-2 flex gap-2">
                              <button type="button" onClick={() => dismiss(r.id)} className="px-2 py-1 text-[11px] bg-surface-v2 border border-[var(--border-subtle, #dde4ec)] text-[var(--ink-2, #3d4756)] rounded inline-flex items-center gap-1">
                                <X size={10} /> Dismiss
                              </button>
                            </div>
                          </div>
