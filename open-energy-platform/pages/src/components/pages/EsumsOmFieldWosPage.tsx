...
const PRIORITY_STYLE: Record<string, React.CSSProperties> = {
  critical: { background: 'color-mix(in oklch, var(--bad, oklch(0.55 0.22 25)) 14%, var(--s1, oklch(0.97 0.04 20)))', color: 'var(--bad, oklch(0.48 0.20 20))', borderColor: 'var(--bad, oklch(0.85 0.08 20))' },
  high:     { background: 'var(--s1, oklch(0.96 0.003 250))', color: 'var(--accent, oklch(0.46 0.16 55))', borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' },
  medium:   { background: 'var(--s1, oklch(0.96 0.003 250))', color: 'var(--ink, oklch(0.17 0.010 250))', borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' },
  low:      { background: 'var(--s1, oklch(0.96 0.003 250))', color: 'var(--ink-2, oklch(0.60 0.007 250))', borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' },
};
...
export function EsumsOmFieldWosPage() {
  ...
  // ─── List view ───
  return (
    <div className="pb-20" style={{ minHeight: 'calc(100vh - 50px)', background: 'var(--s0, oklch(0.96 0.003 250))' }}>
      ...
      <div className="p-3 space-y-2">
        {rows.length === 0 ? (
          <div className="rounded-lg border p-6 text-center"
               style={{ background: 'oklch(0.99 0.002 80)', borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' }}>
            <Wrench size={28} className="mx-auto" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }} />
            <div className="mt-2 text-[14px] font-semibold" style={{ color: 'var(--ink, oklch(0.17 0.010 250))' }}>No assigned WOs</div>
            <div className="text-[12px] mt-1" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>You're caught up. Pull to refresh.</div>
          </div>
        ) : rows.map((w) => {
          const minsLeft = Math.round((new Date(w.sla_deadline).getTime() - Date.now()) / 60_000);
          const slaColor = minsLeft < 0 ? 'var(--bad, oklch(0.48 0.20 20))' : minsLeft < 60 ? 'var(--accent, oklch(0.46 0.16 55))' : 'var(--good, oklch(0.45 0.15 150))';
          return (
            <button type="button" key={w.id} onClick={() => setActive(w)} className="w-full text-left block">
              <div className="rounded-lg border p-3 shadow-sm"
                   style={{ background: 'oklch(0.99 0.002 80)', borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' }}>
                <div className="flex items-center justify-between gap-2">
                  <span className="font-mono text-[12px] font-bold" style={{ color: 'var(--ink, oklch(0.17 0.010 250))' }}>{w.wo_number}</span>
                  <span className="inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold uppercase border"
                        style={PRIORITY_STYLE[w.priority]}>
                    {w.priority}
                  </span>
                </div>
                <div className="mt-1 text-[14px] font-semibold" style={{ color: 'var(--ink, oklch(0.17 0.010 250))' }}>{w.title}</div>
                <div className="mt-1 text-[12px] inline-flex items-center gap-1" style={{ color: 'var(--ink-2, oklch(0.40 0.009 250))' }}>
                  <MapPin size={11} /> {w.site_name}
                </div>
                <div className="mt-2 flex items-center justify-between text-[11px]">
                  <span className="capitalize" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>{statusLabel(w.status).text}</span>
                  <span className="font-mono font-semibold" style={{ color: slaColor }}>
                    {minsLeft < 0 ? `⚠ ${Math.abs(minsLeft)}m over SLA` : `SLA ${minsLeft}m`}
                  </span>
                  <ChevronRight size={14} style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }} />
                </div>
              </div>
            </button>
          );
...
function FieldWoDetail({
  ...
  return (
    <div className="pb-32" style={{ minHeight: 'calc(100vh - 50px)', background: 'var(--s0, oklch(0.96 0.003 250))' }}>
      ...
      <div className="p-3 space-y-3">
        <section className="rounded-lg border p-4 shadow-sm"
                 style={{ background: 'oklch(0.99 0.002 80)', borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' }}>
          <div className="text-[16px] font-semibold leading-tight" style={{ color: 'var(--ink, oklch(0.17 0.010 250))' }}>{wo.title}</div>
          {wo.description && <p className="mt-1 text-[13px] leading-snug" style={{ color: 'var(--ink-2, oklch(0.40 0.009 250))' }}>{wo.description}</p>}
        ...
        <section className="rounded-lg border p-3 shadow-sm"
                 style={{ background: 'oklch(0.99 0.002 80)', borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' }}>
          <div className="text-[11px] uppercase tracking-wider font-bold" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>Photo evidence</div>
        ...
        <section className="rounded-lg border p-3 shadow-sm space-y-2"
                 style={{ background: 'oklch(0.99 0.002 80)', borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' }}>
          <div className="text-[11px] uppercase tracking-wider font-bold" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>Quick actions</div>
...
function Stat({ label, value, tone }: { label: string; value: string; tone?: 'good' | 'warn' | 'bad' }) {
  const toneColor = tone === 'bad' ? 'var(--bad, oklch(0.48 0.20 20))' : tone === 'warn' ? 'var(--accent, oklch(0.46 0.16 55))' : tone === 'good' ? 'var(--good, oklch(0.45 0.15 150))' : 'var(--ink, oklch(0.17 0.010 250))';
  return (
    <div>
      <div className="text-[10px] uppercase tracking-wider font-bold" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>{label}</div>
...
function PhotoButton({ label, onChoose }: { label: string; onChoose: (file: File) => Promise<void> }) {
  ...
  return (
    <label htmlFor={id} className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed py-4 cursor-pointer"
           style={{ borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' }}>
      <Camera size={20} style={{ color: 'var(--accent, oklch(0.46 0.16 55))' }} />
      <span className="mt-1 text-[11px] font-semibold" style={{ color: 'var(--ink-2, oklch(0.40 0.009 250))' }}>{label}</span>
