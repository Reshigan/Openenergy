...
export function EsumsSiteDetailPage() {
  ...
  if (!id) return <div className="p-6" style={{ color: 'var(--bad, oklch(0.48 0.20 20))' }}>No site id in URL.</div>;
  if (err && !detail) {
    return (
      <div className="p-6">
        <Link to="/esums" className="text-[12px] underline inline-flex items-center gap-1"
              style={{ color: 'var(--ink, oklch(0.17 0.010 250))' }}>
  ...
  if (!detail) return <div className="p-6 text-[12px]" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>Loading site…</div>;
    ...
    >
      <div className="flex items-center gap-2 mb-3">
        <Link to="/esums" className="text-[11px] inline-flex items-center gap-1"
              style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>
          <ArrowLeft size={12}/> Back to fleet
        </Link>
      ...
      <div className="border-b flex flex-wrap gap-1 mb-3" style={{ borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' }}>
        {([
  ...
  return (
    <section className="space-y-3">
      ...
      <div className="widget-card">
        ...
        <ul className="divide-y text-[12px]" style={{ borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' }}>
            ...
            <li key={f.id} className="px-4 py-2 flex items-center gap-3">
              <span className="px-2 py-0.5 rounded text-[10px] uppercase font-bold"
                    style={f.severity === 'critical'
                      ...
                      : { background: 'var(--s1, oklch(0.96 0.003 250))', color: 'var(--ink, oklch(0.17 0.010 250))' }}>{f.severity}</span>
              <span className="flex-1 truncate">{f.description}</span>
              <span className="font-mono text-[11px]" style={{ color: 'var(--bad, oklch(0.48 0.20 20))' }}>R{Math.round(Number(f.hourly_loss_zar || 0))}/h</span>
              <span className="font-mono text-[11px]" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>{new Date(f.detected_at).toLocaleDateString('en-ZA')}</span>
            </li>
          ))}
          {detail.recent_faults.filter((f) => !['resolved','closed'].includes(f.status)).length === 0 && (
            <li className="px-4 py-3 text-[12px] italic" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>No open faults — nice.</li>
          )}
        </ul>
      </div>
      <div className="widget-card">
        ...
        <ul className="divide-y text-[12px]" style={{ borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' }}>
          {detail.open_work_orders.slice(0, 10).map((w) => (
            <li key={w.id} className="px-4 py-2 flex items-center gap-3">
              ...
              <span className="px-2 py-0.5 rounded text-[10px] uppercase font-bold"
                    style={{ background: 'var(--s1, oklch(0.96 0.003 250))', color: 'var(--ink, oklch(0.17 0.010 250))' }}>{w.status}</span>
              <span className="font-mono text-[11px]" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>{w.sla_deadline ? new Date(w.sla_deadline).toLocaleDateString('en-ZA') : '—'}</span>
            </li>
          ))}
          {detail.open_work_orders.length === 0 && (
            <li className="px-4 py-3 text-[12px] italic" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>No open work orders.</li>
          )}
        </ul>
      </div>
    </section>
...
function LiveTab({ siteId }: { siteId: string }) {
  ...
  if (!data) return <div className="widget-card p-6 text-center text-[12px]" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>Loading live snapshot…</div>;
  ...
  return (
    <section className="space-y-3">
      <div className="flex items-center gap-2 text-[11px]" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>
        <Radio size={11} style={{ color: 'var(--good, oklch(0.45 0.15 150))' }} className={loading ? 'animate-pulse' : ''}/>
      ...
      <div className="widget-card">
        <header className="widget-card-header flex items-center gap-2">
          <AlertCircle size={13} style={{ color: data.open_faults.length > 0 ? 'var(--bad, oklch(0.48 0.20 20))' : 'var(--good, oklch(0.45 0.15 150))' }}/>
            ...
            <li key={f.id} className="px-4 py-2 flex items-center gap-3">
              <span className="px-2 py-0.5 rounded text-[10px] uppercase font-bold"
                    style={f.severity === 'critical'
                      ? { background: 'color-mix(in oklch, var(--bad, oklch(0.55 0.22 25)) 14%, var(--s1, oklch(0.97 0.04 20)))', color: 'var(--bad, oklch(0.48 0.20 20))' }
                      : f.severity === 'major'
                      ? { background: 'var(--s1, oklch(0.96 0.003 250))', color: 'var(--accent, oklch(0.46 0.16 55))' }
                      : { background: 'var(--s1, oklch(0.96 0.003 250))', color: 'var(--ink, oklch(0.17 0.010 250))' }}>{f.severity}</span>
              {f.fault_code && <span className="font-mono text-[10px]" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>{f.fault_code}</span>}
              <span className="flex-1 truncate">{f.description}</span>
              <span className="font-mono text-[11px]" style={{ color: 'var(--bad, oklch(0.48 0.20 20))' }}>R{Math.round(Number(f.hourly_loss_zar || 0))}/h</span>
              <span className="font-mono text-[10px]" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>{new Date(f.detected_at).toLocaleString('en-ZA', { timeZone: 'Africa/Johannesburg' })}</span>
            </li>
          ))}
          {data.open_faults.length === 0 && (
            <li className="px-4 py-3 text-[12px] italic" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>No open faults — fleet is healthy.</li>
          )}
        </ul>
      </div>
      ...
      <div className="widget-card">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-[11px]">
            <thead><tr className="text-left border-b" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))', borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' }}>
            ...
            <tbody>
              {data.connector_runs.slice(0, 10).map((r) => (
                <tr key={r.id} className="border-b" style={{ borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' }}>
                  ...
                  <td className="px-3 py-1.5">
                    <span className="px-2 py-0.5 rounded text-[10px] uppercase font-bold"
                          style={r.status === 'ok'
                            ...
                            : r.status === 'failed'
                            ? { background: 'color-mix(in oklch, var(--bad, oklch(0.55 0.22 25)) 14%, var(--s1, oklch(0.97 0.04 20)))', color: 'var(--bad, oklch(0.48 0.20 20))' }
                            : { background: 'var(--s1, oklch(0.96 0.003 250))', color: 'var(--ink, oklch(0.17 0.010 250))' }}>{r.status}</span>
                  </td>
                  ...
                  <td className="px-3 py-1.5 text-right font-mono" style={{ color: 'var(--bad, oklch(0.48 0.20 20))' }}>{r.rows_rejected ?? 0}</td>
                  <td className="px-3 py-1.5 text-[10px] truncate max-w-[200px]" style={{ color: 'var(--bad, oklch(0.48 0.20 20))' }} title={r.error_sample || ''}>{r.error_sample || ''}</td>
                </tr>
              ))}
              {data.connector_runs.length === 0 && (
                <tr><td colSpan={7} className="px-3 py-3 text-[12px] italic text-center" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>No connector runs yet — push telemetry to POST /api/esums-ingest/telemetry with a site ingest key.</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
function DevicesTab({ devices }: { devices: Device[] }) {
  ...
  if (devices.length === 0) return <div className="widget-card p-6 text-center text-[12px]" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>No devices registered at this site yet.</div>;
  return (
    <section className="space-y-3">
        ...
        <div key={type} className="widget-card">
          ...
          <div className="p-3 overflow-x-auto">
            <table className="w-full text-[11px]">
              ...
              <tbody>
                {list.map((d) => (
                  <tr key={d.id} className="border-b" style={{ borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' }}>
                    ...
                    <td className="py-1.5"><span className="px-2 py-0.5 rounded text-[10px] uppercase font-bold" style={{ background: 'var(--s1, oklch(0.96 0.003 250))', color: 'var(--ink, oklch(0.17 0.010 250))' }}>{d.status}</span></td>
                    <td className="py-1.5 font-mono text-[10px]" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>{d.last_seen_at ? new Date(d.last_seen_at).toLocaleString('en-ZA', { timeZone: 'Africa/Johannesburg' }) : '—'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function ChartsTab({ perf }: { perf: Perf | null }) {
  if (!perf || perf.series.length === 0) {
    return <div className="widget-card p-6 text-center text-[12px]" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>No telemetry yet — connect an OEM adapter or push data to /api/esums/telemetry to start populating charts.</div>;
  }
    ...
    <section className="space-y-3">
            ...
            {perf.series.map((s) => {
              ...
              return (
                <div key={s.day} className="flex-1 group relative" title={`${s.day} · ${Math.round(s.kwh).toLocaleString()} kWh · R${Math.round(s.revenue_zar).toLocaleString()}`}>
                  <div className="rounded-t" style={{ height: `${Math.max(2, h)}%`, background: 'var(--accent, oklch(0.46 0.16 55))' }}/>
                </div>
              );
            })}
          </div>
          <div className="mt-2 text-[10px] flex justify-between font-mono" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>
      ...
      <div className="widget-card">
        ...
        <div className="p-4">
          ...
          <div className="text-[10px] flex justify-between" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>
...
function SoilingCalculator({ capMw, tariff }: { capMw: number; tariff: number }) {
  ...
  return (
    <div className="widget-card p-4">
      <div className="inline-flex items-center gap-1 text-[13px] font-semibold mb-2" style={{ color: 'var(--ink, oklch(0.17 0.010 250))' }}>
        <Wrench size={13}/> Soiling cleaning — payback
      </div>
      <p className="text-[11px] mb-3" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>Compare cleaning cost against avoided generation loss.</p>
...
function BatteryPaybackCalculator({ capMw, tariff }: { capMw: number; tariff: number }) {
  ...
  return (
    <div className="widget-card p-4">
      ...
      <p className="text-[11px] mb-3" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>Simple energy-arbitrage payback at the PPA tariff.</p>
...
function ModuleReplacementCalculator({ capMw, tariff, perf }: { capMw: number; tariff: number; perf: Perf | null }) {
  ...
  return (
    <div className="widget-card p-4">
      ...
      <p className="text-[11px] mb-3" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>Replace under-performing modules. Current PR seeded from last-7-day average.</p>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-[11px]">
...
function RecCertCalculator({ capMw, perf }: { capMw: number; perf: Perf | null }) {
  ...
  return (
    <div className="widget-card p-4">
      ...
      <p className="text-[11px] mb-3" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>Project the annual REC stream from observed generation × current price.</p>
...
function PrintTab({ site }: { site: Site }) {
  return (
    <section className="space-y-3">
      <div className="widget-card p-4">
        <div className="text-[13px] font-semibold" style={{ color: 'var(--ink, oklch(0.17 0.010 250))' }}>Lender pack</div>
        <p className="text-[12px] mt-1 mb-2" style={{ color: 'var(--ink-2, oklch(0.40 0.009 250))' }}>
      ...
      <div className="widget-card p-4">
        <div className="text-[13px] font-semibold" style={{ color: 'var(--ink, oklch(0.17 0.010 250))' }}>Regulator pack</div>
        <p className="text-[12px] mt-1 mb-2" style={{ color: 'var(--ink-2, oklch(0.40 0.009 250))' }}>
      ...
      <div className="widget-card p-4">
        <div className="text-[13px] font-semibold" style={{ color: 'var(--ink, oklch(0.17 0.010 250))' }}>Daily audit summary</div>
        <p className="text-[12px] mt-1 mb-2" style={{ color: 'var(--ink-2, oklch(0.40 0.009 250))' }}>
...
function Kpi({ label, value, caption }: { label: string; value: React.ReactNode; caption?: string }) {
  return (
    <div className="widget-card p-3">
      <div className="text-[11px] uppercase tracking-wider" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>{label}</div>
      <div className="text-[20px] font-bold" style={{ color: 'var(--ink, oklch(0.17 0.010 250))' }}>{value}</div>
      {caption && <div className="text-[10px]" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>{caption}</div>}
    </div>
  );
}
function KpiSm({ label, value, sub }: { label: string; value: React.ReactNode; sub?: string }) {
  return (
    <div className="widget-card p-3">
      <div className="text-[11px] uppercase tracking-wider" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>{label}</div>
      <div className="text-[16px] font-bold" style={{ color: 'var(--ink, oklch(0.17 0.010 250))' }}>{value}</div>
      {sub && <div className="text-[10px]" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>{sub}</div>}
    </div>
  );
}
function NumIn({ label, value, onChange, step }: { label: string; value: number; onChange: (v: number) => void; step?: number }) {
  return (
    <label className="block">
      <div className="text-[10px] uppercase" style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>{label}</div>
...
function ResultRow({ label, value, tone }: { label: string; value: React.ReactNode; tone?: 'good' | 'bad' }) {
  const valueColor = tone === 'good' ? 'var(--good, oklch(0.45 0.15 150))' : tone === 'bad' ? 'var(--bad, oklch(0.48 0.20 20))' : 'var(--ink, oklch(0.17 0.010 250))';
  return (
    <div className="flex justify-between border-t py-1.5 text-[12px]" style={{ borderColor: 'var(--border-subtle, oklch(0.87 0.006 250))' }}>
      <span style={{ color: 'var(--ink-2, oklch(0.60 0.007 250))' }}>{label}</span>
