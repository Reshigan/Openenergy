      ...
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <div className="flex items-center gap-2 text-[12px]" style={{ color: 'var(--ink-2, #6b7685)' }}>
            ...
            <span>/</span>
            <span style={{ color: 'var(--ink, #0f1c2e)', fontWeight: 600 }}>Operations</span>
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={() => navigate(`/projects/${id}/lifecycle`)} className="h-9 px-3 rounded-md border border-[var(--border-subtle, #dde4ec)] bg-surface-v2 text-[var(--ink-2, #3d4756)] text-[12px] font-semibold inline-flex items-center gap-1">
            Lifecycle
          </button>
          <button type="button" onClick={() => navigate(`/projects/${id}`)} className="h-9 px-3 rounded-md border border-[var(--border-subtle, #dde4ec)] bg-surface-v2 text-[var(--ink-2, #3d4756)] text-[12px] font-semibold inline-flex items-center gap-1">
        ...
        {workOrders.length === 0 ? <Empty label="No work orders. Schedule preventive + corrective via the O&M page." /> : (
          <Table headers={['When', 'Type', 'Status', 'Asset', 'Notes']}>
            {workOrders.slice(0, 50).map(w => (
              <tr key={w.id} className="border-t border-[var(--border-subtle, #e5ebf2)]">
                <td className="px-4 py-2 text-[11px] text-[var(--ink-2, #6b7685)]">{w.opened_at ? new Date(w.opened_at).toLocaleDateString() : '—'}</td>
        ...
        {spares.length === 0 ? <Empty label="No spares stocked. EPC + O&M will list critical spares here." /> : (
          <Table headers={['Part', 'Qty', 'Reorder level', 'Location']}>
            {spares.slice(0, 50).map(s => (
              <tr key={s.id} className="border-t border-[var(--border-subtle, #e5ebf2)]">
        ...
        {nominations.length === 0 ? <Empty label="No nominations scheduled." /> : (
          <Table headers={['Period', 'MWh nominated', 'Status']}>
            {nominations.slice(0, 50).map(n => (
              <tr key={n.id} className="border-t border-[var(--border-subtle, #e5ebf2)]">
          ...
          <Table headers={['When', 'Severity', 'Status', 'Description']}>
            {faults.slice(0, 50).map(f => (
              <tr key={f.id} className="border-t border-[var(--border-subtle, #e5ebf2)]">
                <td className="px-4 py-2 text-[11px] text-[var(--ink-2, #6b7685)]">{f.detected_at ? new Date(f.detected_at).toLocaleString() : '—'}</td>
