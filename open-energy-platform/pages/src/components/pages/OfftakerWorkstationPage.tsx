...
const OFFTAKER_REPORTS: ReportConfig[] = [
  {
    ...
    mailSubject: 'Open Energy — Offtaker PPA Contracts Report',
  },
  {
    ...
    mailSubject: 'Open Energy — Offtaker Statutory Reports',
  },
  {
    ...
    mailSubject: 'Open Energy — Green Tariff Disclosures',
  },
  {
    ...
    mailSubject: 'Open Energy — Scope 2 Emissions Report',
  },
];

const OFFTAKER_WIZARDS: WizardSpec[] = [
  {
    id: 'offtaker-complete-setup',
    title: 'Set up your offtaker workstation',
    subtitle: 'Configure PPA contracts, operations, Scope 2 compliance, and all procurement workflows',
    steps: [
      {
        title: 'PPA contracts',
        description: 'Set up PPA contracts, Tariff indexation, Change in law, PPA termination, Payment security, and Wheeling access workflows.',
        aiHint: 'Each PPA you execute should be registered here. Tariff indexation (Wave W39) runs automatically annually — it compares CPI to your base tariff and flags the repricing amount for your approval. Payment security instruments (Wave W54) must be maintained with sufficient headroom or the generator can declare a payment default.',
        fields: [
          { key: 'ppa_count', label: 'Number of active PPAs', type: 'select', options: [{ value: '1', label: '1' }, { value: '2_5', label: '2–5' }, { value: '6_plus', label: '6 or more' }] },
          { key: 'standard_payment_security', label: 'Standard payment security type', type: 'select', options: [{ value: 'letter_of_credit', label: 'Letter of credit' }, { value: 'bank_guarantee', label: 'Bank guarantee' }, { value: 'cash_escrow', label: 'Cash escrow' }] },
          { key: 'default_take_or_pay_pct', label: 'Typical take-or-pay obligation (%)', type: 'number', placeholder: 'e.g. 90' },
        ],
      },
      {
        title: 'Operations & billing',
        description: 'Set up PPA nominations, Annual reconciliation, Consumption sites, Energy bills, Curtailment claims, Take-or-pay, and Wheeling charges.',
        aiHint: 'Day-ahead nominations must be submitted by 12:00 for the following day. The system reminds you at 10:00 if you have an active PPA with no nomination for the next day. Curtailment claims (Wave W46) must be lodged within 5 business days — the system creates a draft automatically when the SO records a curtailment event.',
        fields: [
          { key: 'consumption_site_count', label: 'Number of consumption sites', type: 'number', placeholder: 'e.g. 3' },
          { key: 'nomination_auto_submit', label: 'Auto-submit nominations?', type: 'select', options: [{ value: 'yes', label: 'Yes — use previous day\'s profile' }, { value: 'no', label: 'No — manual approval required' }] },
          { key: 'billing_contact', label: 'Energy billing contact email', type: 'text', placeholder: 'procurement@company.co.za' },
        ],
      },
      {
        title: 'Scope 2 & sustainability',
        description: 'Configure RECs & GoOs, Scope 2 reporting, Green tariff disclosure, SLB KPIs, and REC lifecycle workflows.',
        aiHint: 'GHG Protocol Scope 2 requires market-based accounting using RECs. I-RECs must be from the same country and same calendar year as consumption. For SBTi alignment, you need RECs covering 100% of your electricity consumption — the system tracks your coverage gap monthly.',
        fields: [
          { key: 'scope2_reporting_standard', label: 'Scope 2 reporting standard', type: 'select', options: [{ value: 'ghg_protocol', label: 'GHG Protocol Scope 2 Guidance' }, { value: 'cdp', label: 'CDP Climate questionnaire' }, { value: 'sbti', label: 'SBTi 1.5°C pathway' }, { value: 'none', label: 'Not currently reporting' }] },
          { key: 'annual_consumption_mwh', label: 'Annual electricity consumption (MWh)', type: 'number', placeholder: 'e.g. 50000' },
          { key: 'rec_target_pct', label: 'REC coverage target (%)', type: 'number', placeholder: 'e.g. 100 for SBTi, 50 for basic reporting' },
        ],
      },
    ],
    submitLabel: 'Save offtaker setup',
    onSubmit: async (values) => {
      const token = localStorage.getItem('token') || '';
      await fetch('/api/preferences', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ role: 'offtaker', ...values }) }).catch(() => {});
    },
  },
  {
    id: 'offtaker-ppa',
    title: 'Execute a new PPA',
    subtitle: 'NERSA Section 34 — power purchase agreement',
    steps: [
      {
        title: 'Generator',
        description: 'Identify the generator you are contracting with.',
        aiHint: 'The IPP entity name must match the NERSA licence exactly. The system cross-checks this against the licence registry when you submit.',
        fields: [
          { key: 'generator_name', label: 'Generator / IPP name', type: 'text', required: true, placeholder: 'e.g. Saldanha Wind Energy (Pty) Ltd' },
          { key: 'technology', label: 'Technology', type: 'select', required: true, options: [{ value: 'solar_pv', label: 'Solar PV' }, { value: 'wind', label: 'Wind' }, { value: 'hydro', label: 'Hydro' }, { value: 'gas', label: 'Gas' }] },
          { key: 'contracted_mw', label: 'Contracted capacity (MW)', type: 'number', required: true, placeholder: 'e.g. 100' },
        ],
      },
      {
        title: 'Tariff',
        description: 'Set the energy price and escalation terms.',
        aiHint: 'CPI escalation is the standard REIPPPP structure. Fixed price PPAs are simpler but expose you to inflation risk. The tariff indexation chain (W39) monitors annual adjustments.',
        fields: [
          { key: 'base_tariff_zar_mwh', label: 'Base tariff (R/MWh)', type: 'number', required: true, placeholder: 'e.g. 680.00' },
          { key: 'escalation_type', label: 'Price escalation', type: 'select', options: [{ value: 'cpi', label: 'CPI-linked (annual)' }, { value: 'fixed', label: 'Fixed price' }, { value: 'hybrid', label: 'Hybrid (partial CPI)' }] },
          { key: 'ppa_term_years', label: 'PPA term (years)', type: 'number', required: true, placeholder: 'e.g. 20' },
        ],
      },
      {
        title: 'Payment security',
        description: 'What credit support are you providing to the generator?',
        aiHint: 'Most REIPPPP generators require a payment security instrument — letter of credit, bank guarantee, or cash escrow — before financial close. This seeds the payment security workflow (W54).',
        fields: [
          { key: 'payment_security_type', label: 'Payment security type', type: 'select', options: [{ value: 'letter_of_credit', label: 'Letter of credit' }, { value: 'bank_guarantee', label: 'Bank guarantee' }, { value: 'cash_escrow', label: 'Cash escrow' }, { value: 'pcg', label: 'Parent company guarantee' }] },
          { key: 'security_amount_zar', label: 'Security amount (ZAR)', type: 'number', placeholder: 'e.g. 50000000' },
        ],
      },
      {
        title: 'Commencement',
        description: 'When does the PPA start and what is the take-or-pay obligation?',
        aiHint: 'Take-or-pay percentage (usually 80–95%) is the fraction of contracted MWh you must pay for whether or not you actually use the energy. The take-or-pay chain (W32) monitors this monthly.',
        fields: [
          { key: 'commencement_date', label: 'Commencement date (COD)', type: 'date', required: true },
          { key: 'take_or_pay_pct', label: 'Take-or-pay obligation (%)', type: 'number', placeholder: 'e.g. 90' },
          { key: 'notes', label: 'Notes', type: 'textarea', placeholder: 'Special conditions, amendments, or comments…' },
        ],
      },
    ],
    submitLabel: 'Create PPA record',
    onSubmit: async (values) => {
      const token = localStorage.getItem('token') || '';
      const res = await fetch('/api/ppa-contracts', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(values) });
      if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error((e as any).error || 'PPA creation failed'); }
    },
  },
  {
    id: 'offtaker-rec-retirement',
    title: 'Retire RECs for Scope 2 disclosure',
    subtitle: 'I-REC / SAREC — GHG Protocol market-based accounting',
    steps: [
      {
        title: 'REC selection',
        description: 'Choose which renewable energy certificates to retire.',
        aiHint: 'I-RECs must be retired in the same calendar year as the electricity consumption they represent. Retiring after year-end still counts but requires a "vintage year" adjustment in your GHG report.',
        fields: [
          { key: 'rec_type', label: 'Certificate type', type: 'select', required: true, options: [{ value: 'i_rec', label: 'I-REC (international)' }, { value: 'sarec', label: 'SAREC (South African)' }, { value: 'eu_go', label: 'EU Guarantee of Origin' }] },
          { key: 'certificate_ids', label: 'Certificate IDs / batch reference', type: 'text', required: true, placeholder: 'e.g. I-REC-ZA-2025-WIND-001 to 100' },
          { key: 'volume_mwh', label: 'Volume (MWh)', type: 'number', required: true, placeholder: 'e.g. 5000' },
        ],
      },
      {
        title: 'Consumption period',
        description: 'Match the retirement to the consumption period for your GHG report.',
        aiHint: 'SBTi and CDP require the consumption period to match the certificate vintage. If your consumption was in 2025 use 2025 RECs — not 2024 surplus certificates.',
        fields: [
          { key: 'consumption_year', label: 'Consumption year', type: 'select', required: true, options: [{ value: '2024', label: '2024' }, { value: '2025', label: '2025' }, { value: '2026', label: '2026' }] },
          { key: 'reporting_standard', label: 'Reporting standard', type: 'select', options: [{ value: 'ghg_protocol', label: 'GHG Protocol Scope 2' }, { value: 'cdp', label: 'CDP disclosure' }, { value: 'sbti', label: 'SBTi Science-Based Target' }] },
          { key: 'notes', label: 'Retirement notes', type: 'textarea', placeholder: 'Optional reference for auditor…' },
        ],
      },
    ],
    submitLabel: 'Retire certificates',
    onSubmit: async (values) => {
      const token = localStorage.getItem('token') || '';
      const res = await fetch('/api/rec-lifecycle', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(values) });
      if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error((e as any).error || 'REC retirement failed'); }
    },
  },
  {
    id: 'offtaker-curtailment',
    title: 'Lodge a curtailment claim',
    subtitle: 'IEC 61724 — deemed-energy compensation under PPA',
    steps: [
      {
        title: 'Curtailment event',
        description: 'Identify the curtailment event and the affected PPA.',
        aiHint: 'Curtailment by the SO (grid operator) or by your instruction entitles the generator to deemed-energy payment under most REIPPPP PPAs. You must notify within 5 business days of the curtailment.',
        fields: [
          { key: 'ppa_reference', label: 'PPA reference', type: 'text', required: true, placeholder: 'PPA contract ID' },
          { key: 'curtailment_date', label: 'Curtailment date', type: 'date', required: true },
          { key: 'curtailment_cause', label: 'Curtailment cause', type: 'select', required: true, options: [{ value: 'grid_constraint', label: 'Grid constraint (SO instruction)' }, { value: 'buyer_instruction', label: 'Buyer instruction' }, { value: 'force_majeure', label: 'Force majeure' }, { value: 'scheduled', label: 'Scheduled maintenance window' }] },
        ],
      },
      {
        title: 'Quantum',
        description: 'Estimate the deemed energy volume and claim value.',
        aiHint: 'Deemed energy is calculated from the P90 generation profile for the curtailed period. The platform uses actual SCADA data where available, otherwise meteorological forecast data.',
        fields: [
          { key: 'curtailed_hours', label: 'Curtailed hours', type: 'number', required: true, placeholder: 'e.g. 4.5' },
          { key: 'estimated_mwh', label: 'Estimated deemed MWh', type: 'number', placeholder: 'Platform will calculate if left blank' },
          { key: 'claim_value_zar', label: 'Estimated claim value (ZAR)', type: 'number', placeholder: 'Platform will calculate from tariff if left blank' },
          { key: 'supporting_ref', label: 'Supporting evidence reference', type: 'text', placeholder: 'SCADA export file, met data report…' },
        ],
      },
    ],
    ...
    onSubmit: async (values) => {
      const token = localStorage.getItem('token') || '';
      const res = await fetch('/api/curtailment-claims', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(values) });
      if (!res.ok) { const e = await res.json().catch(() => ({})); throw new Error((e as any).error || 'Curtailment claim failed'); }
    },
  },
  {
    id: 'offtaker-rec-retire',
    title: 'Issue REC retirement',
    subtitle: 'Wave W70 — renewable attribute certificate retirement for Scope 2',
    steps: [
      {
        title: 'REC details',
        fields: [
          { key: 'rec_standard', label: 'Certificate standard', type: 'select', required: true, options: [{ value: 'i_rec', label: 'I-REC' }, { value: 'sarec', label: 'SAREC' }, { value: 'eu_go', label: 'EU Guarantee of Origin' }, { value: 'tigr', label: 'TIGR' }, { value: 'm_rets', label: 'M-RETS' }] },
          ...
          { key: 'generator_country', label: 'Generator country', type: 'text', required: true, placeholder: 'ISO 2-letter code, e.g. ZA' },
        ],
      },
      {
        title: 'Retirement purpose',
        fields: [
          { key: 'retirement_reason', label: 'Retirement reason', type: 'select', required: true, options: [{ value: 'scope2_market_based', label: 'Scope 2 — market-based' }, { value: 'green_tariff_claim', label: 'Green tariff claim' }, { value: 'sbti_aligned_target', label: 'SBTi-aligned target' }, { value: 'voluntary_pledge', label: 'Voluntary pledge' }] },
          { key: 'fiscal_year', label: 'Fiscal year', type: 'number', required: true, placeholder: 'e.g. 2025' },
          { key: 'claim_ref', label: 'Claim reference', type: 'text', placeholder: 'GHG report or CDP disclosure reference' },
        ],
      },
    ],
    submitLabel: 'Issue retirement',
    onSubmit: async (values) => {
      const token = localStorage.getItem('token') || '';
      await fetch('/api/offtaker/rec-lifecycle', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(values) }).catch(() => {});
    },
  },
  {
    id: 'offtaker-curtailment-new',
    title: 'Submit PPA curtailment claim',
    subtitle: 'Wave W46 — deemed-energy compensation',
    steps: [
      {
        title: 'Curtailment event',
        fields: [
          { key: 'ppa_contract_ref', label: 'PPA contract reference', type: 'text', required: true },
          { key: 'curtailment_date', label: 'Curtailment date', type: 'date', required: true },
          { key: 'curtailed_mwh', label: 'Curtailed MWh', type: 'number', required: true },
          { key: 'curtailment_cause', label: 'Curtailment cause', type: 'select', required: true, options: [{ value: 'so_instruction', label: 'SO instruction' }, { value: 'congestion', label: 'Congestion' }, { value: 'system_constraint', label: 'System constraint' }, { value: 'force_majeure', label: 'Force majeure' }] },
        ],
      },
      {
        title: 'Compensation',
    ...
    onSubmit: async (values) => {
      const token = localStorage.getItem('token') || '';
      await fetch('/api/offtaker/curtailment-claims', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(values) }).catch(() => {});
    },
  },
  {
    id: 'offtaker-top',
    title: 'Submit take-or-pay notice',
    subtitle: 'Wave W32 — IFRS 16 + NERSA s34 monthly TOP monitoring',
    steps: [
      {
    ...
    submitLabel: 'Submit TOP notice',
    onSubmit: async (values) => {
      const token = localStorage.getItem('token') || '';
      await fetch('/api/offtaker/take-or-pay', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(values) }).catch(() => {});
    },
  },
  {
    id: 'offtaker-tariff-indexation',
    title: 'Request tariff indexation',
    subtitle: 'Wave W39 — NERSA ERA §4 + IFRS 16 annual repricing',
    steps: [
      {
        title: 'Indexation basis',
        fields: [
          { key: 'ppa_contract_ref', label: 'PPA contract reference', type: 'text', required: true },
          { key: 'indexation_type', label: 'Indexation type', type: 'select', required: true, options: [{ value: 'cpi_linked', label: 'CPI-linked' }, { value: 'ppi_linked', label: 'PPI-linked' }, { value: 'nersa_mypd', label: 'NERSA MYPD determination' }, { value: 'fixed_escalation', label: 'Fixed escalation' }] },
          { key: 'reference_year', label: 'Reference year', type: 'number', required: true, placeholder: 'e.g. 2025' },
        ],
      },
      {
        title: 'Application',
        fields: [
          { key: 'new_tariff_zar_per_mwh', label: 'New tariff (ZAR/MWh)', type: 'number', required: true },
          { key: 'effective_from', label: 'Effective from', type: 'date', required: true },
          { key: 'nersa_determination_ref', label: 'NERSA determination reference', type: 'text', placeholder: 'e.g. NERSA/MYPD-4/2025' },
        ],
      },
    ],
    submitLabel: 'Submit indexation',
    onSubmit: async (values) => {
      const token = localStorage.getItem('token') || '';
      await fetch('/api/offtaker/tariff-indexation', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(values) }).catch(() => {});
    },
  },
  {
    id: 'offtaker-change-in-law',
    title: 'Notify PPA change in law event',
    steps: [
      {
        title: 'Event',
        fields: [
          { key: 'ppa_ref', label: 'PPA reference', type: 'text', required: true },
          { key: 'change_type', label: 'Change type', type: 'select', required: true, options: [{ value: 'legislative', label: 'Legislative' }, { value: 'regulatory', label: 'Regulatory' }, { value: 'tax', label: 'Tax' }, { value: 'grid_code', label: 'Grid code' }] },
      ...
      },
      {
        ...
        fields: [
          { key: 'estimated_tariff_impact_pct', label: 'Estimated tariff impact (%)', type: 'number', placeholder: '+2.5 or -1.0' },
          { key: 'cure_period_days', label: 'Cure period (days)', type: 'number', placeholder: '90 per PPA default' },
          { key: 'legal_opinion_attached', label: 'Legal opinion attached', type: 'select', options: [{ value: 'yes', label: 'Yes' }, { value: 'no', label: 'No' }] },
          { key: 'notice_to_generator_sent', label: 'Notice to generator sent', type: 'select', required: true, options: [{ value: 'yes', label: 'Yes' }, { value: 'no', label: 'No' }] },
        ],
      },
    ],
  ...
  },
  {
    ...
    title: 'Submit PPA energy nomination',
    steps: [
      {
        title: 'Nomination',
        fields: [
          { key: 'ppa_ref', label: 'PPA reference', type: 'text', required: true },
          { key: 'delivery_period', label: 'Delivery period', type: 'text', required: true, placeholder: 'e.g. 2026-06-10' },
          { key: 'nominated_mwh', label: 'Nominated MWh', type: 'number', required: true },
          { key: 'peak_mw', label: 'Peak MW', type: 'number', required: true },
          { key: 'flexible_tolerance_pct', label: 'Flexible tolerance (%)', type: 'number', placeholder: '±10 = 10% flex' },
  ...
  },
  {
    ...
    title: 'Run PPA annual reconciliation',
    steps: [
      {
        title: 'Period',
        fields: [
      ...
      },
      {
        title: 'Settlement',
        fields: [
          { key: 'shortfall_mwh', label: 'Shortfall MWh', type: 'number' },
          { key: 'take_or_pay_penalty_zar', label: 'Take-or-pay penalty (ZAR)', type: 'number' },
          { key: 'surplus_mwh', label: 'Surplus MWh', type: 'number' },
          { key: 'banker_confirmed', label: 'Banker confirmed', type: 'select', options: [{ value: 'yes', label: 'Yes' }, { value: 'no', label: 'No' }] },
  ...
  },
  {
    ...
    title: 'Apply for wheeling access (W219)',
    steps: [
      {
        title: 'Application',
        fields: [
          { key: 'wheel_tier', label: 'Wheeling tier', type: 'select', required: true, options: [{ value: 'small_embedded', label: 'Small embedded' }, { value: 'distribution', label: 'Distribution' }, { value: 'sub_transmission', label: 'Sub-transmission' }, { value: 'bulk_transmission', label: 'Bulk transmission' }] },
          { key: 'requested_capacity_mw', label: 'Requested capacity (MW)', type: 'number', required: true },
          { key: 'ipp_ref', label: 'IPP reference', type: 'text', required: true },
          { key: 'wheeling_route_description', label: 'Wheeling route description', type: 'textarea', required: true },
          { key: 'ppa_ref', label: 'PPA reference', type: 'text' },
        ],
      },
      {
        title: 'Technical',
        fields: [
          { key: 'delivery_point', label: 'Delivery point', type: 'text', required: true },
          { key: 'voltage_level_kv', label: 'Voltage level (kV)', type: 'number', required: true },
          { key: 'wheeling_distance_km', label: 'Wheeling distance (km)', type: 'number' },
          { key: 'gca_ref', label: 'GCA reference', type: 'text', placeholder: 'GCA from IPP if available' },
          { key: 'annual_wheeling_charge_zar', label: 'Annual wheeling charge (ZAR)', type: 'number' },
        ],
      },
    ],
  ...
  {
    id: 'offtaker-vppa',
    title: 'Create virtual PPA settlement instruction',
    steps: [
      {
        title: 'VPPA details',
        fields: [
          { key: 'contract_ref', label: 'Contract reference', type: 'text', required: true },
          { key: 'generator_id', label: 'Generator ID', type: 'text', required: true },
          { key: 'settlement_period', label: 'Settlement period', type: 'text', required: true, placeholder: 'e.g. 2026-Q2' },
      ...
      },
      {
        title: 'Settlement',
        fields: [
          { key: 'reference_index', label: 'Reference index', type: 'select', required: true, options: [{ value: 'spot_vwap', label: 'Spot VWAP' }, { value: 'day_ahead_auction', label: 'Day-ahead auction' }, { value: 'bilaterally_agreed', label: 'Bilaterally agreed' }] },
          { key: 'reference_price_zar_per_mwh', label: 'Reference price (ZAR/MWh)', type: 'number' },
          { key: 'settlement_direction', label: 'Settlement direction', type: 'select', options: [{ value: 'generator_pays', label: 'Generator pays' }, { value: 'offtaker_pays', label: 'Offtaker pays' }, { value: 'net_zero', label: 'Net zero' }] },
          { key: 'settlement_bank', label: 'Settlement bank', type: 'text' },
        ],
      },
    ],
  ...
  },
  {
    ...
    title: 'Register PPA payment security (W54)',
    steps: [
      {
        title: 'Instrument',
        fields: [
          { key: 'ppa_contract_ref', label: 'PPA contract reference', type: 'text', required: true },
          { key: 'security_type', label: 'Security type', type: 'select', required: true, options: [{ value: 'bank_guarantee', label: 'Bank guarantee' }, { value: 'lc', label: 'Letter of credit' }, { value: 'parent_guarantee', label: 'Parent guarantee' }, { value: 'escrow', label: 'Escrow' }, { value: 'cash_deposit', label: 'Cash deposit' }] },
      ...
      },
      {
        ...
        fields: [
          ...
          { key: 'renewal_trigger_days', label: 'Renewal trigger (days before expiry)', type: 'number', placeholder: '60 = renew 60 days before expiry' },
          { key: 'generator_consent_required', label: 'Generator consent required', type: 'select', options: [{ value: 'yes', label: 'Yes' }, { value: 'no', label: 'No' }] },
          { key: 'notes', label: 'Notes', type: 'textarea' },
        ],
      },
    ],
  ...
  {
    id: 'offtaker-slb-kpi',
    title: 'Record SLB KPI achievement (W204)',
    steps: [
      {
        title: 'KPI',
        fields: [
          { key: 'facility_ref', label: 'Facility reference', type: 'text', required: true },
          { key: 'kpi_period', label: 'KPI period', type: 'text', required: true, placeholder: '2026-H1' },
          ...
          { key: 'kpi_unit', label: 'KPI unit', type: 'text', required: true },
        ],
      },
      {
        title: 'Performance',
        fields: [
          ...
          { key: 'ratchet_basis_points', label: 'Ratchet (basis points)', type: 'number' },
          { key: 'kpi_data_source', label: 'KPI data source', type: 'text' },
  ...
  },
  {
    ...
    title: 'Submit green tariff disclosure (W210)',
    steps: [
      {
        title: 'Disclosure period',
        fields: [
          { key: 'disclosure_period', label: 'Disclosure period', type: 'text', required: true, placeholder: 'e.g. 2026' },
          { key: 'green_tariff_class', label: 'Green tariff class', type: 'select', required: true, options: [{ value: 'sbti_aligned', label: 'SBTi aligned' }, { value: 'corporate_ppa', label: 'Corporate PPA' }, { value: 'i_rec_backed', label: 'I-REC backed' }, { value: 'voluntary', label: 'Voluntary' }] },
          { key: 'ppa_ref', label: 'PPA reference', type: 'text' },
          { key: 'consumption_mwh', label: 'Consumption MWh', type: 'number', required: true },
          { key: 'contracted_green_mwh', label: 'Contracted green MWh', type: 'number', required: true },
        ],
      },
      {
        title: 'RECs & labelling',
        fields: [
          { key: 'matched_rec_mwh', label: 'Matched REC MWh', type: 'number' },
          { key: 'irec_registry', label: 'I-REC registry', type: 'select', options: [{ value: 'i_rec', label: 'I-REC' }, { value: 'sarec', label: 'SAREC' }, { value: 'rego', label: 'REGO' }, { value: 'other', label: 'Other' }] },
          { key: 'rec_serial_from', label: 'REC serial (from)', type: 'text' },
          { key: 'rec_serial_to', label: 'REC serial (to)', type: 'text' },
          { key: 'cdp_submission_ref', label: 'CDP submission reference', type: 'text' },
  ...
  },
  {
    ...
    title: 'Initiate PPA termination (W62)',
    steps: [
      {
        title: 'Termination notice',
        fields: [
          { key: 'ppa_contract_ref', label: 'PPA contract reference', type: 'text', required: true },
          { key: 'termination_cause', label: 'Termination cause', type: 'select', required: true, options: [{ value: 'voluntary', label: 'Voluntary' }, { value: 'generator_default', label: 'Generator default' }, { value: 'force_majeure', label: 'Force majeure' }, { value: 'change_in_law', label: 'Change in law' }, { value: 'regulatory_invalidity', label: 'Regulatory invalidity' }] },
          { key: 'notice_date', label: 'Notice date', type: 'date', required: true },
          { key: 'effective_termination_date', label: 'Effective termination date', type: 'date', required: true },
        ],
      },
      {
        title: 'Buy-out calculation',
        fields: [
          { key: 'eta_basis', label: 'ETA basis', type: 'select', required: true, options: [{ value: 'book_value', label: 'Book value' }, { value: 'market_to_market', label: 'Mark-to-market' }, { value: 'agreed_formula', label: 'Agreed formula' }] },
          { key: 'estimated_eta_zar', label: 'Estimated ETA (ZAR)', type: 'number', placeholder: 'Early termination amount' },
          { key: 'eta_dispute_likely', label: 'ETA dispute likely', type: 'select', options: [{ value: 'yes', label: 'Yes' }, { value: 'no', label: 'No' }] },
  ...
  },
  {
    ...
    title: 'Log unserved energy claim',
    steps: [
      {
        title: 'Unserved event',
        fields: [
          { key: 'ppa_ref', label: 'PPA reference', type: 'text', required: true },
          { key: 'event_date', label: 'Event date', type: 'date', required: true },
          { key: 'curtailment_cause', label: 'Curtailment cause', type: 'select', required: true, options: [{ value: 'grid_constraint', label: 'Grid constraint' }, { value: 'so_curtailment', label: 'SO curtailment' }, { value: 'force_majeure', label: 'Force majeure' }, { value: 'planned_maintenance', label: 'Planned maintenance' }] },
          { key: 'curtailed_hours', label: 'Curtailed hours', type: 'number', required: true },
          { key: 'contracted_mw', label: 'Contracted MW', type: 'number', required: true },
        ],
      },
      {
        title: 'Quantum',
        fields: [
          { key: 'unserved_energy_mwh', label: 'Unserved energy (MWh)', type: 'number', required: true },
          { key: 'applicable_tariff_zar_per_mwh', label: 'Applicable tariff (ZAR/MWh)', type: 'number', required: true },
          { key: 'claim_amount_zar', label: 'Claim amount (ZAR)', type: 'number', required: true },
          { key: 'evidence_ref', label: 'Evidence reference', type: 'text', required: true },
          { key: 'generator_notified', label: 'Generator notified', type: 'select', required: true, options: [{ value: 'yes', label: 'Yes' }, { value: 'no', label: 'No' }] },
        ],
      },
    ],
