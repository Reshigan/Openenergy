...
const URGENCY_COLOR: Record<QueryUrgency, string> = {
  ...
  information_only: 'bg-[var(--s2, #eef2f7)] text-[var(--ink-2, #3d4756)]',
};
...
const STATUS_COLOR: Record<TqStatus, string> = {
  raised:                  'bg-[var(--s2, #eef2f7)] text-[var(--ink, #2d3748)]',
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="text-xs text-[var(--ink-2, #9aa5b4)] ml-auto">{filtered.length} TQs</span>
        {!readOnly && (
      ...
      {loading && <div className="text-xs text-[var(--ink-2, #9aa5b4)]">Loading technical queries…</div>}
        ...
        <div className="overflow-x-auto rounded-lg border border-[var(--border-subtle, #dde4ec)]">
          <table className="w-full text-xs">
            <thead className="bg-[var(--s1, #f8fafc)]">
              <tr>
                <th className="text-left px-3 py-2 font-medium text-[var(--ink-2, #6b7685)]">TQ No.</th>
                <th className="text-left px-3 py-2 font-medium text-[var(--ink-2, #6b7685)]">Title</th>
                <th className="text-left px-3 py-2 font-medium text-[var(--ink-2, #6b7685)]">Discipline</th>
                <th className="text-left px-3 py-2 font-medium text-[var(--ink-2, #6b7685)]">Urgency</th>
                <th className="text-left px-3 py-2 font-medium text-[var(--ink-2, #6b7685)]">Designer</th>
                <th className="text-left px-3 py-2 font-medium text-[var(--ink-2, #6b7685)]">Response type</th>
            ...
            <tbody>
              ...
              {filtered.map(row => (
                <tr
                  key={row.id}
                  className={`border-t border-[var(--s2, #eef2f7)] hover:bg-[var(--s2, #eef2f7)] cursor-pointer ${
                ...
                >
                  ...
                  <td className="px-3 py-2 max-w-[200px]">
                    <span className="text-[var(--ink, #1e2a38)] block truncate font-medium">{row.tq_title}</span>
                    {row.project_name && <span className="text-[var(--ink-2, #9aa5b4)] truncate block">{row.project_name}</span>}
                  </td>
                  <td className="px-3 py-2">
                    {row.discipline && (
                      <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${DISCIPLINE_COLOR[row.discipline] ?? 'bg-[var(--s2, #eef2f7)] text-[var(--ink-2, #3d4756)]'}`}>
                  ...
                  <td className="px-3 py-2">
                    <span className="text-[var(--ink, #2d3748)] block truncate">{row.assigned_designer ?? '—'}</span>
                    {row.design_company && <span className="text-[var(--ink-2, #9aa5b4)] text-[10px] block truncate">{row.design_company}</span>}
                  </td>
                  <td className="px-3 py-2">
                    {row.response_type ? (
                      <span className="text-[var(--ink-2, #3d4756)]">{RESPONSE_TYPE_LABEL[row.response_type] ?? row.response_type}</span>
          ...
          <div className="bg-surface-v2 rounded-xl shadow-2xl w-full max-w-3xl p-6 space-y-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-[var(--ink, #2d3748)] mt-0.5">{selected.tq_title}</p>
                {selected.project_name && <p className="text-xs text-[var(--ink-2, #9aa5b4)]">{selected.project_name}</p>}
              </div>
            ...
            {/* Query content */}
            <div className="rounded-lg border border-[var(--border-subtle, #dde4ec)] p-4 space-y-3">
              <h3 className="text-xs font-semibold text-[var(--ink, #2d3748)]">Query</h3>
              <p className="text-xs text-[var(--ink, #1e2a38)] leading-relaxed">{selected.query_description}</p>
              {selected.drawing_ref && (
                <div className="text-xs"><span className="font-medium text-[var(--ink-2, #6b7685)]">Drawing ref:</span> <span className="text-[var(--ink, #2d3748)]">{selected.drawing_ref}</span></div>
              )}
              {selected.specification_ref && (
                <div className="text-xs"><span className="font-medium text-[var(--ink-2, #6b7685)]">Specification ref:</span> <span className="text-[var(--ink, #2d3748)]">{selected.specification_ref}</span></div>
              )}
            ...
            {(selected.assigned_designer || selected.design_company) && (
              <div className="text-xs text-[var(--ink-2, #3d4756)] flex gap-4">
            ...
            {(selected.rfi_ref || selected.ncr_ref || selected.ms_ref || selected.submittal_ref) && (
              <div className="flex flex-wrap gap-2 text-xs text-[var(--ink-2, #6b7685)]">
                {selected.rfi_ref && <span className="rounded bg-[var(--s2, #eef2f7)] px-2 py-0.5">RFI: {selected.rfi_ref}</span>}
                {selected.ncr_ref && <span className="rounded bg-[var(--s2, #eef2f7)] px-2 py-0.5">NCR: {selected.ncr_ref}</span>}
                {selected.ms_ref && <span className="rounded bg-[var(--s2, #eef2f7)] px-2 py-0.5">MS: {selected.ms_ref}</span>}
                {selected.submittal_ref && <span className="rounded bg-[var(--s2, #eef2f7)] px-2 py-0.5">Submittal: {selected.submittal_ref}</span>}
              </div>
            )}
      ...
      {showCreate && (
        <div onMouseDown={(e) => { if (e.target === e.currentTarget) setShowCreate(false); }} className="fixed inset-0 bg-black/40 z-50 flex items-start justify-center overflow-y-auto py-8 px-4">
          <div className="bg-surface-v2 rounded-xl shadow-2xl w-full max-w-2xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-base font-semibold text-[var(--ink, #0f1c2e)]">New technical query (TQ)</h2>
            ...
            <div className="grid grid-cols-2 gap-4 text-xs">
              <label className="flex flex-col gap-1">
                <span className="text-[var(--ink-2, #3d4756)] font-medium">TQ title *</span>
              ...
              <label className="flex flex-col gap-1">
                <span className="text-[var(--ink-2, #3d4756)] font-medium">TQ number</span>
              ...
              <label className="flex flex-col gap-1">
                <span className="text-[var(--ink-2, #3d4756)] font-medium">Discipline *</span>
                <select className="border rounded px-2 py-1.5" value={newDiscipline} onChange={e => setNewDiscipline(e.target.value)}>
                  {ALL_DISCIPLINES.map(d => <option key={d} value={d}>{DISCIPLINE_LABEL[d]}</option>)}
                </select>
              </label>
              <label className="flex flex-col gap-1">
                <span className="text-[var(--ink-2, #3d4756)] font-medium">Query urgency *</span>
                ...
                <span className="text-[10px] text-[var(--ink-2, #9aa5b4)] italic">Higher urgency = tighter SLA (URGENT polarity)</span>
              </label>
              <label className="col-span-2 flex flex-col gap-1">
                <span className="text-[var(--ink-2, #3d4756)] font-medium">Query description *</span>
              ...
              <label className="flex flex-col gap-1">
                <span className="text-[var(--ink-2, #3d4756)] font-medium">Drawing ref</span>
              ...
              <label className="flex flex-col gap-1">
                <span className="text-[var(--ink-2, #3d4756)] font-medium">Specification ref</span>
              ...
              <label className="col-span-2 flex flex-col gap-1">
                <span className="text-[var(--ink-2, #3d4756)] font-medium">Contractor's proposed solution (optional)</span>
              ...
              <label className="flex flex-col gap-1">
                <span className="text-[var(--ink-2, #3d4756)] font-medium">Contractor ref</span>
            ...
            <div className="space-y-2 pt-2 border-t border-[var(--s2, #eef2f7)]">
              <p className="text-xs font-medium text-[var(--ink-2, #3d4756)]">Technical flags</p>
