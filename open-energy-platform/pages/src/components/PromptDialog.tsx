...
export function PromptHost() {
  ...
  return createPortal(
    ...
    >
      ...
      >
        <h3 id="prompt-dialog-title" className="mb-3 text-sm font-semibold text-[#0c2a4d]">{active.message}</h3>
        {active.kind === 'prompt' ? (
          <>
            <input
              autoFocus
              ...
              className="mb-4 w-full rounded border border-[#cbd5e1] px-3 py-2 text-sm text-[#0c2a4d] focus:border-[#1a3a5c] focus:outline-none"
            />
            <div className="flex justify-end gap-2">
              <button type="button" onClick={() => closePrompt(null)} className="rounded px-3 py-1.5 text-sm text-[#4a5568] hover:bg-[#f1f5f9]">
                Cancel
              </button>
              <button type="button" onClick={() => closePrompt(value)} className="rounded bg-[#0c2a4d] px-3 py-1.5 text-sm font-medium text-white hover:bg-[#163b66]">
                OK
              </button>
            </div>
          </>
          ...
          <div className="flex justify-end gap-2">
            ...
            <button type="button" onClick={() => closeConfirm(true)} className="rounded bg-[#0c2a4d] px-3 py-1.5 text-sm font-medium text-white hover:bg-[#163b66]">
              Yes
            </button>
          </div>
        )}
