    ...
    <section className={`rounded-xl bg-surface-v2 border border-[var(--border-subtle, #dde4ec)] ${className ?? ''}`}>
      <header className="flex items-center justify-between px-4 py-2.5 border-b border-[var(--s2, #eef2f7)]">
        <div className="flex items-center gap-2 text-[var(--ink, #0f1c2e)]">
          ...
          {label && <span className="text-[11px] text-[var(--ink-2, #6b7685)] truncate max-w-[8rem]">{label}</span>}
        </div>
        <button
      ...
      <div className="p-3 space-y-3">
        ...
        {empty && <p className="text-[11px] text-[var(--ink-2, #6b7685)] px-1 py-4 text-center">No activity on this chain yet.</p>}
        ...
        {data && !empty && (
          <>
            ...
            {data.throughput.length > 1 && (
              <div className="rounded-lg bg-[var(--s1, #f8fafc)] border border-[var(--border-subtle, #e5ebf2)] p-3">
                <p className="text-[10px] uppercase tracking-wide text-[var(--ink-2, #6b7685)] mb-1">Events / day (30d)</p>
                <Sparkline points={data.throughput.map((p) => p.events)} />
              </div>
            )}

            {data.bottleneck && (
              <p className="text-[11px] text-[var(--ink-2, #3d4756)] px-1">
                Bottleneck:{' '}
                <span className="font-medium text-[var(--ink, #0f1c2e)]">{data.bottleneck.status.replaceAll('_', ' ')}</span>{' '}
          ...
          <article key={card.key} className="rounded-lg border border-amber-200 bg-gradient-to-br from-[color-mix(in_oklch,var(--warn,oklch(0.65_0.18_75))_14%,var(--s1,#fffdf3))] to-[color-mix(in_oklch,var(--warn,oklch(0.65_0.18_75))_14%,var(--s1,#fff7e3))] p-3">
            <div className="flex items-start gap-2">
              ...
              <div className="min-w-0">
                <p className="text-[12px] font-semibold text-[var(--ink, #0f1c2e)] leading-snug">{card.title}</p>
                <p className="mt-1 text-[11px] text-[var(--ink-2, #6b7685)] leading-snug">{card.why}</p>
                {card.accept?.href && (
                  <button
...
function Kpi({ label, value, tone }: { label: string; value: string | number; tone?: 'warn' }) {
  return (
    <div className="rounded-lg bg-[var(--s1, #f8fafc)] border border-[var(--border-subtle, #e5ebf2)] px-3 py-2">
      <p className="text-[10px] uppercase tracking-wide text-[var(--ink-2, #6b7685)]">{label}</p>
      <p className={`text-[15px] font-semibold ${tone === 'warn' ? 'text-amber-600' : 'text-[var(--ink, #0f1c2e)]'}`}>{value}</p>
    </div>
  );
}
