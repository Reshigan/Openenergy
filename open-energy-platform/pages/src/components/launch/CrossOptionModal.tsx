  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-[#0b1c30]/40 backdrop-blur-sm" onClick={onClose}>
      <div
        ...
        ref={dialogRef}
        className="w-full sm:max-w-md bg-surface-v2 border border-[var(--border-subtle, #dde4ec)] rounded-t-2xl sm:rounded-xl shadow-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <header className="flex items-start justify-between px-5 py-3 border-b border-[var(--s2, #eef2f7)]">
          <div>
            <p className="text-[10px] uppercase tracking-wider font-semibold" style={{ color: 'oklch(0.55 0.008 250)' }}>Suggested next step</p>
            <h2 className="text-[14px] font-display font-semibold text-[var(--ink, #0f1c2e)] mt-0.5">{action.title}</h2>
          </div>
        ...
        <div className="px-5 py-4 text-[13px] text-[var(--ink-2, #3d4756)] space-y-1">
          <p>
            From <span className="font-medium text-[var(--ink, #0f1c2e)]">{action.source_chain_key ?? action.source_entity_type}</span>
            {' · '}{action.source_entity_id}
          </p>
          <p className="text-[12px] text-[var(--ink-2, #6b7685)]">
            Completing this opens <span className="text-[var(--ink, #0f1c2e)] font-medium">{co.target_route}</span>.
          </p>
        </div>

        <footer className="flex items-center justify-between gap-2 px-5 py-3 border-t border-[var(--s2, #eef2f7)]">
          <button type="button" onClick={onClose} disabled={busy}
            className="text-[12px] text-[var(--ink-2, #6b7685)] hover:text-[var(--ink, #0f1c2e)] px-3 py-1.5 rounded-md hover:bg-[var(--s2, #eef2f7)] disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#c2873a]">
            Later
          </button>
