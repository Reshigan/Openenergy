...
const STATE_TONE: Record<RpmStatus, { bg: string; fg: string; label: string }> = {
  ...
  survival_dataset_bound:   { bg: 'color-mix(in oklab, var(--warn) 18%, var(--s1))', fg: 'var(--warn)', label: 'Surv. ds' },
  ...
  failover_to_ols:          { bg: 'color-mix(in oklab, var(--warn) 15%, var(--s1))', fg: 'var(--warn)', label: 'OLS failover' },
};
  ...
  return (
    <div className="text-[12px]" style={{ color: 'oklch(0.46 0.16 55)' }}>
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div>
          <h2 className="text-base font-semibold text-[var(--ink, #0c2a4d)]">RUL prediction ML model governance</h2>
    ...
    <div onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }} className="fixed inset-0 z-50 flex items-stretch justify-end bg-black/40">
      <div className="w-full max-w-3xl overflow-y-auto bg-[var(--s2, #f3f5f9)] p-4">
        ...
        <div className="mb-3 grid grid-cols-4 gap-2 rounded border border-[var(--border-subtle, #d8dde6)] bg-surface-v2 p-3 text-[11px]">
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">Harrell C</div>
            <div className={`font-mono text-[12px] ${(row.concordance_index ?? 0) >= 0.7 ? 'text-[var(--good, #1f5b3a)]' : (row.concordance_index ?? 0) >= 0.6 ? 'text-[#a06200]' : 'text-[var(--bad, #9b1f1f)] font-semibold'}`}>{row.concordance_index ?? '-'}</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">td-AUC</div>
            <div className="font-mono text-[12px] text-[var(--ink, #0c2a4d)]">{row.time_dependent_auc ?? '-'}</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">Brier</div>
            <div className={`font-mono text-[12px] ${(row.brier_score ?? 0) > 0.20 ? 'text-[var(--bad, #9b1f1f)] font-semibold' : (row.brier_score ?? 0) > 0.10 ? 'text-[#a06200]' : 'text-[var(--good, #1f5b3a)]'}`}>{row.brier_score ?? '-'}</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">Partial likelihood</div>
            <div className="font-mono text-[12px] text-[var(--ink, #0c2a4d)]">{row.partial_likelihood ?? '-'}</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">PH p-value</div>
            <div className={`font-mono text-[12px] ${(row.ph_assumption_pvalue ?? 1) < 0.01 ? 'text-[var(--bad, #9b1f1f)] font-semibold' : (row.ph_assumption_pvalue ?? 1) < 0.05 ? 'text-[#a06200]' : 'text-[var(--good, #1f5b3a)]'}`}>{row.ph_assumption_pvalue ?? '-'}</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">PH violated</div>
            <div className={`font-mono text-[12px] ${(row.ph_violated_count ?? 0) > 0 ? 'text-[#a06200]' : 'text-[var(--good, #1f5b3a)]'}`}>{row.ph_violated_count ?? 0}</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">KM lift vs OLS</div>
            <div className={`font-mono text-[12px] ${(row.kaplan_meier_lift_vs_ols ?? 0) >= 15 ? 'text-[var(--good, #1f5b3a)]' : (row.kaplan_meier_lift_vs_ols ?? 0) < 0 ? 'text-[var(--bad, #9b1f1f)] font-semibold' : 'text-[#a06200]'}`}>{row.kaplan_meier_lift_vs_ols ?? '-'}%</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">RUL p50 MAE</div>
            <div className="font-mono text-[12px] text-[var(--ink, #0c2a4d)]">{row.rul_p50_mae_days ?? '-'}d</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">RUL p10/p50/p90</div>
            <div className="font-mono text-[12px] text-[var(--ink, #0c2a4d)]">{row.rul_p10_days ?? '-'}/{row.rul_p50_days ?? '-'}/{row.rul_p90_days ?? '-'}d</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">Censoring rate</div>
            <div className="font-mono text-[12px] text-[var(--ink, #0c2a4d)]">{row.censoring_rate ?? '-'}</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">OLS recon. %</div>
            <div className="font-mono text-[12px] text-[var(--ink, #0c2a4d)]">{row.reconciliation_with_w71_ols_pct ?? '-'}</div>
          </div>
          <div>
        ...
        {/* Bridges */}
        <div className="mb-3 rounded border border-[var(--border-subtle, #d8dde6)] bg-surface-v2 p-3 text-[11px]">
          <div className="mb-2 text-[10px] uppercase tracking-wider text-[var(--ink-2, #4a5568)]">Cross-chain bridges (OLS NOT NULL + audit mandatory)</div>
          <div className="grid grid-cols-5 gap-2">
  ...
  return (
    <div onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="w-full max-w-2xl rounded bg-surface-v2 p-4 text-[12px]" style={{ color: 'oklch(0.46 0.16 55)' }}>
        <div className="mb-3 flex items-start justify-between">
          <div>
            <h3 className="text-base font-semibold text-[var(--ink, #0c2a4d)]">Propose RUL prediction ML model</h3>
            <p className="text-[11px] text-[var(--ink-2, #4a5568)]">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Model family">
            <select value={family} onChange={(e) => setFamily(e.target.value as RpmFamily)} className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]">
              {FAMILY_OPTIONS.map((p) => <option key={p.key} value={p.key}>{p.label}</option>)}
            </select>
          </Field>
          <Field label="Asset class">
            <select value={assetClass} onChange={(e) => setAssetClass(e.target.value as RpmAssetClass)} className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]">
          ...
          <Field label="Covariate / feature count">
            <input value={featureCount} onChange={(e) => setFeatureCount(e.target.value)} type="number" className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]" placeholder="32" />
          </Field>
          <Field label="Title">
            <input value={title} onChange={(e) => setTitle(e.target.value)} className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]" placeholder="Eastern Cape wind fleet Cox PH v1" />
          </Field>
          ...
          <Field label="Lender drawdown ref">
            <input value={w21} onChange={(e) => setW21(e.target.value)} className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]" placeholder="draw-2026-0007" />
          </Field>
          <Field label="Reserve account ref">
            <input value={w77} onChange={(e) => setW77(e.target.value)} className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]" placeholder="rac-2026-0011" />
          </Field>
          <Field label="Warranty recovery ref">
            <input value={w63} onChange={(e) => setW63(e.target.value)} className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]" placeholder="wrec-2026-0019" />
          </Field>
        </div>
