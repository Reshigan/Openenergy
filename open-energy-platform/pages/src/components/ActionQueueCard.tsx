...
const priorityColor: Record<string, { bg: string; text: string; dot: string }> = {
  urgent: { bg: '#fde7e9', text: 'var(--bad, #c0392b)', dot: 'var(--bad, #c0392b)' },
  high: { bg: '#fef3e6', text: '#b04e0f', dot: '#c97a14' },
  normal: { bg: '#d4e7f6', text: 'oklch(0.46 0.16 55)', dot: 'oklch(0.46 0.16 55)' },
  low: { bg: '#eef1f4', text: 'var(--ink-2, #6b7685)', dot: '#8c8f94' },
};
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="leading-tight">
            <h2 className="text-[15px] font-semibold" style={{ color: 'var(--ink, #0f1c2e)' }}>
              Awaiting your action
            </h2>
            <p className="text-[12px]" style={{ color: 'var(--ink-2, #6b7685)' }}>
        ...
        <div
          ...
          style={{ background: items.length ? '#d4e7f6' : '#eef1f4', color: items.length ? 'oklch(0.46 0.16 55)' : 'var(--ink-2, #6b7685)' }}
        >
          {loading ? '…' : `${items.length} open`}
        </div>
      </header>

      <div className="divide-y" style={{ borderColor: 'var(--border-subtle, #e5ebf2)' }}>
        {loading && (
          <div className="px-5 py-6 text-[13px]" style={{ color: 'var(--ink-2, #6b7685)' }}>
            Loading your queue…
          </div>
        )}
        {!loading && error && (
          <div className="px-5 py-6 text-[13px]" style={{ color: 'var(--bad, #c0392b)' }}>
            {error}
          </div>
        )}
        {!loading && !error && items.length === 0 && (
          <div className="px-5 py-10 text-center">
            <div
              ...
              style={{ background: '#e5f7ec', color: 'var(--good, #1a8a5b)' }}
            >
              <CheckCircle2 size={22} />
            </div>
            <div className="text-[13px] font-semibold" style={{ color: 'var(--ink, #0f1c2e)' }}>Inbox zero</div>
            <div className="text-[12px] mt-0.5" style={{ color: 'var(--ink-2, #6b7685)' }}>
        ...
        {!loading && !error && items.map((it) => {
          ...
          return (
            <div
              key={it.id}
              className="flex items-start gap-3 px-5 py-3 hover:bg-[var(--s2, #eef2f7)] transition-colors"
            >
              <div
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <div className="text-[13px] font-semibold truncate" style={{ color: 'var(--ink, #0f1c2e)' }}>
                    {it.title}
                  </div>
                  <span
                ...
                {it.description && (
                  <div className="text-[12px] mt-0.5 truncate" style={{ color: 'var(--ink-2, #6b7685)' }}>
                    {it.description}
                  </div>
                )}
                {it.due_date && (
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button type="button"
                  ...
                  className="text-[12px] font-semibold px-2.5 h-8 rounded-md flex items-center gap-1 hover:bg-[var(--border-subtle, #e5ebf2)]"
