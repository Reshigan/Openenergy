...
const STATE_TONE: Record<SscStatus, { bg: string; fg: string; label: string }> = {
  ...
  bic_validated:                { bg: 'color-mix(in oklab, var(--warn) 18%, var(--s1))', fg: 'var(--warn)', label: 'BIC validated' },
  bank_handshake_completed:     { bg: 'color-mix(in oklab, var(--warn) 18%, var(--s1))', fg: 'var(--warn)', label: 'Bank handshake' },
  iso20022_schemas_loaded:      { bg: 'color-mix(in oklab, var(--warn) 18%, var(--s1))', fg: 'var(--warn)', label: 'ISO 20022' },
  messaging_session_established:{ bg: 'color-mix(in oklab, var(--warn) 18%, var(--s1))', fg: 'var(--warn)', label: 'Session' },
  test_messages_validated:      { bg: 'color-mix(in oklab, var(--warn) 15%, var(--s1))', fg: 'var(--warn)', label: 'Test msgs' },
  reconciliation_account_bound: { bg: 'color-mix(in oklab, var(--warn) 15%, var(--s1))', fg: 'var(--warn)', label: 'Recon bound' },
  live_settlement_active:       { bg: 'color-mix(in oklab, var(--good) 15%, var(--s1))', fg: 'var(--good, #1f6b3a)', label: 'Live settlement' },
  cycle_reconciled:             { bg: 'color-mix(in oklab, var(--good) 15%, var(--s1))', fg: 'var(--good, #1f6b3a)', label: 'Cycle reconciled' },
...
const TIER_TONE: Record<SscTier, { bg: string; fg: string; label: string }> = {
  domestic_eft:    { bg: 'var(--s2, #eef1f5)', fg: 'var(--ink-2)',    label: 'Domestic EFT' },
  multi_bank_eft:  { bg: 'color-mix(in oklab, var(--warn) 18%, var(--s1))', fg: 'var(--warn)', label: 'Multi-bank EFT' },
  strate_csd:      { bg: 'color-mix(in oklab, var(--good) 15%, var(--s1))', fg: 'var(--good, #1f6b3a)', label: 'STRATE CSD' },
  samos_rtgs:      { bg: 'color-mix(in oklab, var(--warn) 15%, var(--s1))', fg: 'var(--warn)', label: 'SAMOS RTGS' },
  swift_global:    { bg: 'var(--bad, #7a0e0e)', fg: '#fff',    label: 'SWIFT global' },
};
  ...
  return (
    <div className="text-[12px]" style={{ color: 'oklch(0.46 0.16 55)' }}>
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div>
          <h2 className="text-base font-semibold text-[var(--ink, #0c2a4d)]">STRATE / SWIFT settlement connector</h2>
      ...
      <div className="mb-3 flex flex-wrap gap-x-4 gap-y-1 rounded border border-[var(--border-subtle, #d8dde6)] bg-surface-v2 px-3 py-2 text-[11px] text-[var(--ink-2, #4a5568)]">
        ...
        <span>Key {'<'}14d: <span className="font-semibold text-[var(--bad, #9b1f1f)]">{kpis.keys_expiring_within_14d}</span></span>
                ...
                return (
                  ...
                  >
                    <td className="px-3 py-2 font-mono text-[var(--ink, #0c2a4d)]">
                      ...
                      {r.floor_at_swift_global_live ? <span className="ml-1 text-[9px] font-semibold text-[var(--bad, #7a0e0e)]">SWIFT</span> : null}
                    </td>
                    <td className="px-3 py-2 text-[11px] text-[oklch(0.46_0.16_55)]">
                      {r.counterparty_name ?? '-'}
                      {r.bic ? <div className="text-[10px] text-[var(--ink-2, #6b7685)] font-mono">{r.bic}</div> : null}
                    </td>
                    <td className="px-3 py-2 text-[11px] font-mono text-[oklch(0.46_0.16_55)]">
                      {fmtProto(r.protocol)}
                      {r.iso20022_schema_version ? <div className="text-[10px] text-[var(--ink-2, #6b7685)]">{r.iso20022_schema_version}</div> : null}
                    </td>
                    ...
                    <td className="px-3 py-2 text-right tabular-nums text-[11px] text-[var(--ink, #0c2a4d)]">
                      {fmtZar(r.settlement_value_zar_per_cycle)}
                    </td>
                    <td className={`px-3 py-2 text-center tabular-nums ${quality >= 100 ? 'text-[var(--good, #1f5b3a)]' : quality >= 60 ? 'text-[#a06200]' : 'text-[var(--bad, #9b1f1f)] font-semibold'}`}>
                      {quality}/130
                    </td>
                    <td className={`px-3 py-2 text-center text-[10px] uppercase tracking-wider ${keyDays != null && keyDays < 14 ? 'text-[var(--bad, #9b1f1f)] font-semibold' : keyDays != null && keyDays < 60 ? 'text-[#a06200]' : 'text-[var(--ink-2, #4a5568)]'}`}>
    ...
    <div onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }} className="fixed inset-0 z-50 flex items-stretch justify-end bg-black/40">
      <div className="w-full max-w-3xl overflow-y-auto bg-[var(--s2, #f3f5f9)] p-4">
        ...
        <div className="mb-3 grid grid-cols-4 gap-2 rounded border border-[var(--border-subtle, #d8dde6)] bg-surface-v2 p-3 text-[11px]">
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">Msgs/min</div>
            <div className="font-mono text-[12px] text-[var(--ink, #0c2a4d)]">{row.settlement_messages_per_minute ?? '-'}</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">Successful 24h</div>
            <div className="font-mono text-[12px] text-[var(--ink, #0c2a4d)]">{row.successful_settlement_count_24h ?? '-'}</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">Failed 24h</div>
            <div className="font-mono text-[12px] text-[var(--ink, #0c2a4d)]">{row.failed_settlement_count_24h ?? '-'}</div>
          </div>
          <div>
          ...
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">Value 24h</div>
            <div className="font-mono text-[12px] text-[var(--ink, #0c2a4d)]">{fmtZar(row.settlement_value_zar_last_24h)}</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">Latency</div>
            <div className={`font-mono text-[12px] ${(row.average_settlement_latency_ms ?? 0) > 300 ? 'text-[var(--bad, #9b1f1f)] font-semibold' : (row.average_settlement_latency_ms ?? 0) > 150 ? 'text-[#a06200]' : 'text-[var(--ink, #0c2a4d)]'}`}>{row.average_settlement_latency_ms ?? '-'} ms</div>
          </div>
          <div>
          ...
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">Break ZAR</div>
            <div className="font-mono text-[12px] text-[var(--ink, #0c2a4d)]">{fmtZar(row.reconciliation_break_zar)}</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">LCR contrib</div>
            <div className="font-mono text-[12px] text-[var(--ink, #0c2a4d)]">{row.lcr_contribution_pct ?? '-'} %</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">NSFR contrib</div>
            <div className="font-mono text-[12px] text-[var(--ink, #0c2a4d)]">{row.nsfr_contribution_pct ?? '-'} %</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">ExCon</div>
            <div className={`font-mono text-[12px] ${row.excon_authorization_status === 'authorized' ? 'text-[var(--good, #1f5b3a)]' : row.excon_authorization_status === 'pending' ? 'text-[#a06200]' : row.excon_authorization_status === 'expired' ? 'text-[var(--bad, #9b1f1f)] font-semibold' : 'text-[var(--ink-2, #4a5568)]'}`}>{row.excon_authorization_status ?? '-'}</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">FIC Act KYC</div>
            <div className={`font-mono text-[12px] ${row.fic_act_kyc_status === 'clean' ? 'text-[var(--good, #1f5b3a)]' : row.fic_act_kyc_status === 'refresh_due' ? 'text-[#a06200]' : row.fic_act_kyc_status === 'flagged' ? 'text-[var(--bad, #9b1f1f)] font-semibold' : 'text-[var(--ink-2, #4a5568)]'}`}>{row.fic_act_kyc_status ?? '-'}</div>
          </div>
          <div>
          ...
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">PCI-DSS seg</div>
            <div className={`font-mono text-[12px] ${row.pci_dss_segmentation_ok ? 'text-[var(--good, #1f5b3a)]' : 'text-[#a06200]'}`}>{row.pci_dss_segmentation_ok ? 'OK' : 'NO'}</div>
          </div>
          <div>
            <div className="text-[9px] uppercase tracking-wider text-[var(--ink-2, #6b7685)]">Key expiry</div>
            <div className="font-mono text-[12px] text-[var(--ink, #0c2a4d)]">{fmtDate(row.swift_user_key_expiry_at)}</div>
          </div>
        </div>
        ...
        {/* Bridges */}
        <div className="mb-3 rounded border border-[var(--border-subtle, #d8dde6)] bg-surface-v2 p-3 text-[11px]">
          <div className="mb-2 text-[10px] uppercase tracking-wider text-[var(--ink-2, #4a5568)]">Cross-chain bridges (audit + reconciliation attestation mandatory)</div>
          <div className="grid grid-cols-5 gap-2">
...
function ProposeModal({
  ...
  return (
    <div onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="w-full max-w-2xl rounded bg-surface-v2 p-4 text-[12px] text-[oklch(0.46_0.16_55)]">
        <div className="mb-3 flex items-start justify-between">
          <div>
            <h3 className="text-base font-semibold text-[var(--ink, #0c2a4d)]">Propose STRATE / SWIFT connector</h3>
            <p className="text-[11px] text-[var(--ink-2, #4a5568)]">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Peer id (bank counterparty)">
            <input value={peerId} onChange={(e) => setPeerId(e.target.value)} className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]" placeholder="fnb-corporate-rail" />
          </Field>
          <Field label="Counterparty name">
            <input value={counterparty} onChange={(e) => setCounterparty(e.target.value)} className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]" placeholder="First National Bank" />
          </Field>
          <Field label="BIC (ISO 9362, 8 or 11 chars)">
            <input value={bic} onChange={(e) => setBic(e.target.value)} className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px] font-mono" placeholder="FIRNZAJJXXX" />
          </Field>
          <Field label="Settlement value ZAR / cycle">
            <input value={settlementValue} onChange={(e) => setSettlementValue(e.target.value)} type="number" className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]" placeholder="2500000" />
          </Field>
          <Field label="Protocol">
            <select value={protocol} onChange={(e) => setProtocol(e.target.value as SscProtocol)} className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]">
              {PROTOCOL_OPTIONS.map((p) => <option key={p.key} value={p.key}>{p.label}</option>)}
            </select>
          </Field>
          <Field label="Endpoint URL">
            <input value={endpointUrl} onChange={(e) => setEndpointUrl(e.target.value)} className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]" placeholder="https://swift-gateway.example.za" />
          </Field>
          <Field label="Title">
            <input value={title} onChange={(e) => setTitle(e.target.value)} className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]" placeholder="FNB ISO 20022 corporate EFT rail" />
          </Field>
          ...
          <Field label="Reconciliation attestation ref (mandatory)">
            <input value={w120} onChange={(e) => setW120(e.target.value)} className="w-full rounded border border-[var(--border-subtle, #d8dde6)] px-2 py-1 text-[12px]" placeholder="ratt-2026-0042" />
          </Field>
        ...
        <div className="mt-3 rounded border border-[var(--border-subtle, #d8dde6)] bg-[var(--s1, #f8fafc)] p-3 text-[11px]">
          <div className="mb-2 text-[10px] uppercase tracking-wider text-[var(--ink-2, #4a5568)]">Floor flags (FLOOR-AT-SAMOS-RTGS {'≥'}1, FLOOR-AT-SWIFT-GLOBAL {'≥'}3)</div>
        ...
        <div className="mt-3 flex justify-end gap-2">
          <button type="button" onClick={onClose} className="rounded bg-surface-v2 border border-[var(--border-subtle, #d8dde6)] px-3 py-1.5 text-[12px] text-[oklch(0.46_0.16_55)] hover:bg-[var(--s2, #f3f5f9)]">Cancel</button>
