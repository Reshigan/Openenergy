...
export default function MarketHaltSurface(_props: { role: string }) {
  ...
  return (
    <div>
      ...
      {allHalted && (
        <div className="rounded-lg border border-[var(--amber)] bg-[var(--amber-tint)] px-4 py-3 mb-4 text-[12px] text-[var(--amber-deep)]">
      ...
      {loadError && (
        <div className="rounded-lg border border-[var(--oxide)] bg-[var(--oxide-tint)] px-4 py-3 mb-4 text-[12px] text-[var(--oxide-deep)] flex items-center justify-between">
          <span>{loadError}</span>
          <button onClick={load} className="underline hover:no-underline">Retry</button>
        </div>
      )}
      {actionError && (
        <div className="rounded-lg border border-[var(--oxide)] bg-[var(--oxide-tint)] px-4 py-3 mb-4 text-[12px] text-[var(--oxide-deep)]">
          {actionError}
        </div>
      )}
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <button onClick={load} className="text-[12px] text-[var(--ink3)] hover:text-[var(--ink)]">Refresh</button>
      </div>

      <div className="rounded-lg border border-[var(--line)] overflow-hidden">
        <table className="w-full text-[13px]">
          <thead>
            <tr className="text-[var(--ink3)] border-b border-[var(--line)] bg-[var(--raised)]">
          ...
          <tbody>
            {loading && <tr><td colSpan={5} className="px-4 py-6 text-center text-[var(--ink3)]">Loading…</td></tr>}
            {!loading && loadError && rows.length === 0 && <tr><td colSpan={5} className="px-4 py-6 text-center text-[var(--oxide-deep)]">Unavailable.</td></tr>}
            {!loading && !loadError && rows.length === 0 && <tr><td colSpan={5} className="px-4 py-6 text-center text-[var(--ink3)]">No markets.</td></tr>}
            {!loading && rows.map((r) => (
              <tr key={r.scope} className="border-b border-[var(--line)] last:border-0">
                <td className="px-4 py-2 font-medium text-[var(--ink)] capitalize">{r.scope_label}</td>
                <td className="px-4 py-2">
                  <Pill tone={r.active ? 'bad' : 'good'}>{r.active ? r.state_label : 'Open'}</Pill>
                  {r.via === '_all' && <span className="ml-2 text-[11px] text-[var(--ink3)]">via All markets</span>}
                </td>
                <td className="px-4 py-2 text-[var(--ink2)] max-w-[280px] truncate" title={r.reason || ''}>{r.reason || '—'}</td>
                <td className="px-4 py-2 text-[var(--ink3)] text-[12px]">
                  {r.set_at ? new Date(r.set_at).toLocaleString() : '—'}
                </td>
                <td className="px-4 py-2 text-right">
                  {!canHalt && <span className="text-[var(--ink3)] text-[12px]">—</span>}
                  {canHalt && r.via === 'self' && (
                    <button onClick={() => lift(r.scope)} className="px-2.5 py-1 rounded-md text-[12px] bg-[var(--raised)] text-[var(--ink2)] hover:bg-[var(--line)]">Lift</button>
                  )}
                  {canHalt && r.via === '_all' && (
