  ...
  return (
    <div>
      <div className="flex justify-end mb-3">
        <button type="button" onClick={() => setInviting(true)} className="h-9 px-3 rounded-md bg-[oklch(0.46_0.16_55)] text-white text-[12px] font-semibold">
          + Invite user
        </button>
      </div>
      <ListingTable
        ...
        columns={[
          { key: 'email', label: 'User', render: (r) => (
            <div className="leading-tight">
              ...
              <div className="text-[10px] text-slate-500">{r.email}{r.company_name ? ` · ${r.company_name}` : ''}</div>
            </div>
          ) },
          ...
          { key: '_actions', label: '', render: (r) => (
            <button type="button" onClick={() => setManaging(r)} className="px-2 py-1 text-[11px] bg-[oklch(0.46_0.16_55)] text-white rounded">Manage</button>
          ) },
        ]}
      />
      {inviting && (
