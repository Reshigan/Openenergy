        ...
        <div className="flex gap-1">
          {TABS.map((t) => (
            <button key={t.key} type="button" onClick={() => setView(t.key)}
              className={view === t.key ? 'btn pri' : 'btn ghost'}>
              {t.label}
            </button>
          ))}
        </div>
        <div className="flex gap-2">
          <button type="button" onClick={() => setRecording(true)} className="btn pri">+ Record action</button>
          <button type="button" onClick={() => setTransitioning(true)} className="btn ghost">Transition action</button>
        </div>
      </div>
